...
import {appendBreadcrumbAtom, breadcrumbAtom} from "@/oss/lib/atoms/breadcrumb"
...
const EvaluationPage = memo(({evalType}: {evalType: "auto" | "human"}) => {
    ...
    const breadcrumbs = useAtomValue(breadcrumbAtom, {store: traceDrawerJotaiStore})
    const appendBreadcrumb = useSetAtom(appendBreadcrumbAtom, {store: traceDrawerJotaiStore})
    ...
    useEffect(() => {
        if (Object.keys(breadcrumbs || {}).length > 0 && (id || name)) {
            appendBreadcrumb({
                appPage: {
                    label: evalType === "auto" ? "auto evaluation" : "human annotation",
                    href: `/apps/${appId}/evaluations?selectedEvaluation=${evalType === "auto" ? "auto_evaluation" : "human_annotation"}`,
                },
                "eval-detail": {
