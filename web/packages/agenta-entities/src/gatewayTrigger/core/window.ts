 ...
 *
 * A schedule's start_time/end_time are stored as UTC ISO strings, but the picker editing them
 * works in the browser's local zone. These map a stored UTC instant onto the same local clock
 * face for display and back to UTC on write, so the user always picks the UTC wall-clock
 * directly (a schedule's cron is UTC).
 */
