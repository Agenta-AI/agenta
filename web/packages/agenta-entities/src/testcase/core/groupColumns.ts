 ...
 * for table display. This is used by both OSS and playground table components.
 */
...
export interface GroupColumnsOptions<T> {
    ...
    createCollapsedColumnDef?: (groupPath: string, childColumns: Column[]) => ColumnType<T>
...
function groupColumnsRecursive<T>(
    ...
    columns.forEach((col) => {
        ...
        if (!expanded && currentDepth === 0) {
            ...
            result.push({
                ...createColumnDef(col, col.label || col.key),
