 ...
 * metadata, input rows, chat history). Entity-agnostic — uses duck-typed interfaces.
 *
...
// ---------------------------------------------------------------------------
// Duck-typed interfaces — compatible with EnhancedVariant (OSS) and
// workflow entity data without importing either.
// ---------------------------------------------------------------------------
...
export interface TransformToRequestBodyParams {
    ...
    /** Pre-resolved ag_config from the caller (e.g. workflowMolecule.selectors.configuration).
 ...
 *
 * Accepts a duck-typed variant and builds the request body expected by the backend.
 */
export function transformToRequestBody({
    ...
    // When isChat is explicitly true (detected via molecule selectors),
