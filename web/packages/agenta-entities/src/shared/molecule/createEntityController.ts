...
export interface EntityControllerConfig<T, TDraft = Partial<T>> {
     ...
     */
    updateAtom: WritableAtom<unknown, [string, TDraft], void>

    /**
     * Discard atom - clears draft, reverting to server state
     * Signature: (id: string) => void
     */
    discardAtom: WritableAtom<unknown, [string], void>
...
/**
 * Schema selectors - optional, for entities that support schema-driven UI
 */
export interface EntitySchemaSelectors {
    /** Get schema at a specific path */
    schemaAtPath: (params: {id: string; path: (string | number)[]}) => Atom<SchemaProperty | null>
    /** Get the full root schema for an entity */
    rootSchema: (id: string) => Atom<SchemaProperty | null>
}
...
export interface EntityActions<TDraft> {
    ...
    update: WritableAtom<unknown, [string, TDraft], void>
...
/**
 * Schema property type for schema-driven UI rendering
 * Used to understand field types, enums, nested structures, etc.
 */
export interface SchemaProperty {
    ...
    required?: string[]
    /** Custom extension for prompt fields */
    "x-parameters"?: {
        prompt?: boolean
        [key: string]: unknown
    }
...
export interface DrillInConfig<T, TRootData = T> {
    ...
    /**
     * Optional: Get schema at a path for schema-driven UI
     * If provided, enables schema-aware rendering in DrillIn views
     * @param id - Entity ID (to fetch entity-specific schema)
     * @param path - Path within the data structure
     * @returns Schema at path, or null if not available
     */
    getSchemaAtPath?: (id: string, path: (string | number)[]) => SchemaProperty | null
}
...
export interface EntityDrillIn<T> {
     ...
     */
    setValueAtPathAtom: WritableAtom<unknown, [{id: string; path: string[]; value: unknown}], void>
    ...
    /**
     * Optional: Get schema at a path for schema-driven UI
     * Returns null if schema is not available for this entity type
     * @example drillIn.getSchemaAtPath?.("rev-123", ["prompt", "llm_config", "model"])
     */
    getSchemaAtPath?: (id: string, path: (string | number)[]) => SchemaProperty | null
}
...
function createEntityDrillIn<T, TRootData>(
    ...
    dataAtomFamily: (id: string) => Atom<T | null>,
    updateAtom: WritableAtom<unknown, [string, Partial<T>], void>,
): EntityDrillIn<T> {
    const {getRootData, setRootData, getRootItems, valueMode, getSchemaAtPath} = config
    ...
    return {
        ...
        getRootItems,
        valueMode,
        // Include schema getter if provided in config
...
export function createEntityController<T, TDraft = Partial<T>>(
    ...
    const drillIn = drillInConfig
        ? createEntityDrillIn(
              ...
              updateAtom as WritableAtom<unknown, [string, Partial<T>], void>,
          )
        : undefined
