...
export const openDeleteModalAtom = atom(
    null,
    (_get, set, options: EntityReference[] | OpenDeleteModalOptions) => {
        // Reset first to clear any previous state
        set(resetDeleteModalAtom)

        // Handle both array and options object for backwards compatibility
        const entities = Array.isArray(options) ? options : options.entities
        const onSuccess = Array.isArray(options) ? undefined : options.onSuccess

        // Set new entities
        set(deleteModalEntitiesAtom, entities)
        // Set callback if provided
        if (onSuccess) {
            set(deleteModalOnSuccessAtom, onSuccess)
        }
        // Open modal
