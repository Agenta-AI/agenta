...
const rev = (over: {uri?: string; slug?: string; flags?: Record<string, boolean>}) =>
    ({
        slug: over.slug,
...
describe("deriveWorkflowTypeFromRevision — agent", () => {
    it("resolves an agent URI kind to 'agent'", () => {
        ...
        expect(deriveWorkflowTypeFromRevision(rev({uri: "agenta:serve:agent:v0.1"}))).toBe("agent")
    })
    ...
    it("lets is_agent win over is_chat (back-compat flagging)", () => {
        expect(deriveWorkflowTypeFromRevision(rev({flags: {is_agent: true, is_chat: true}}))).toBe(
            "agent",
        )
    })
    ...
    it("still resolves chat / custom / completion unchanged", () => {
        ...
        expect(deriveWorkflowTypeFromRevision(rev({uri: "agenta:serve:chat:v0.1"}))).toBe("chat")
    })
})
