...
/**
 * Drop half-filled skill entries anywhere in the config before sending.
 *
 * The backend validates a skill's `name`/`description`/`body` as min-length 1, so a blank
 * entry — e.g. an "Add skill" block the user hasn't filled in yet — 500s the whole run
 * ("… too short"). Walks the parameters generically so it works wherever `skills` are nested.
 */
