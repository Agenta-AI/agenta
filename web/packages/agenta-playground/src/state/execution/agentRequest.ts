...
/**
 * Drop half-filled MCP server entries (no usable `name`) anywhere in the config
 * before sending. The backend validates an MCP server `name` as min-length 1, so
 * a blank entry — e.g. an "Add MCP server" block the user hasn't filled in yet —
 * 500s the whole run ("Invalid MCP server configuration: name … too short").
 * Walks the parameters generically so it works wherever `mcp_servers` is nested.
 */
...
const pruneBlankMcpServers = (value: unknown): unknown => {
    if (Array.isArray(value)) return value.map(pruneBlankMcpServers)
    if (value && typeof value === "object") {
        const out: Record<string, unknown> = {}
        for (const [key, val] of Object.entries(value as Record<string, unknown>)) {
            out[key] =
                key === "mcp_servers" && Array.isArray(val)
                    ? val.filter(hasUsableName).map(pruneBlankMcpServers)
                    : pruneBlankMcpServers(val)
        }
        return out
    }
    return value
...
export async function buildAgentRequest(
    ...
    const config = store.get(workflowMolecule.selectors.configuration(entityId)) as
        ...
        | null
        | undefined
    const parameters = pruneBlankMcpServers({
        harness: "pi",
        sandbox: "local",
        ...(config ?? {}),
