 ...
 * We use testsetMolecule to fetch the testset name when a revision is selected.
 */
...
export function useTestsetSelection(
    ...
    const revisionQuery = useAtomValue(
        useMemo(
            () => revisionMolecule.selectors.queryOptional(selectedRevisionId),
            [selectedRevisionId],
        ),
    )
    ...
    const revisionData = useAtomValue(
        useMemo(
            () => revisionMolecule.selectors.dataOptional(selectedRevisionId),
            [selectedRevisionId],
        ),
    )
    ...
    const testsetData = useAtomValue(
        useMemo(
            () => testsetMolecule.selectors.dataOptional(selectedTestsetId),
            [selectedTestsetId],
        ),
    )
    ...
    const setRevisionContext = useSetAtom(testcaseMolecule.actions.setRevisionContext)
