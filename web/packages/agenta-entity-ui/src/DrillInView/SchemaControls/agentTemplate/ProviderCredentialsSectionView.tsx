/**
 * ProviderCredentialsSectionView — the presentational half of `ProviderCredentialsSection`.
 *
 ...
 * right) over either (a) a provider rail + key form, or (b) a self-managed info card. The vault
 * secrets arrive as props (the container reads the atoms) so this file is storiable with plain
 * data; key WRITES still happen inside `ProviderKeyField`, because saves are immediate and
 * independent of the drawer draft (locked decision; design.md §3.1). Connections are always the
 * project default: there is no named-connection picker here (owner call, post-design-doc) — a
 * picked model can still auto-fill a vault connection's slug (the model picker threads a vault
 * option's `connectionSlug` metadata into `useModelHarness.writeModel`), just not through a Select.
 *
...
export interface ProviderCredentialsSectionViewProps {
    // data — the project vault, read from atoms by the container
    standardSecrets: LlmProvider[]
    customSecrets: LlmProvider[]
...
export function ProviderCredentialsSectionView({
    standardSecrets,
    customSecrets,
    mode,
    onModeChange,
    ...
    const segmentedClassName = cn(
        // h-[26px]: antd DERIVES the track height (item line-height 20 + 4px padding + this
        // call-site's 2px border); the primitive pins h-control-sm=24, which would squeeze the
        // items to 18 and lift everything below by 2px. Restate antd's rendered height.
        "!h-[26px] rounded-md border border-solid border-[var(--ag-colorBorder)] !bg-[var(--ag-colorFillTertiary)]",
        "[&_[data-slot=segmented-item][data-state=active]]:!text-[var(--ag-colorBgContainer)]",
        "[&_[data-slot=segmented-thumb]]:!bg-[var(--ag-colorText)] [&_[data-slot=segmented-thumb]]:!shadow-none",
    )
        ...
        <div className="flex flex-col items-start gap-3 rounded-[10px] border border-solid border-[var(--ag-colorBorderSecondary)] p-4">
            ...
            <div className="flex flex-col gap-1">
                ...
                <ul className="m-0 flex list-disc flex-col gap-0.5 pl-4">
                    ...
                    <li>
                        <span className="text-xs font-semibold leading-relaxed text-colorTextDescription">
