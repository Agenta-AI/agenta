 ...
 * (GroupedChoiceControl), the tool picker (ToolSelectorPopover + ToolItemControl), the skill
 * editor (SkillConfigControl), enum selects (harness, sandbox, permission policy), and a
 * textarea (agents_md). The field shape is the `agent_config` catalog type generated
...
import {SkillConfigControl} from "./SkillConfigControl"
...
export function AgentConfigControl({
    ...
    // Model selector. `config.model` is the default connection string the SDK coerces into a
    // ModelRef; the form edits it directly (kept byte-identical to today).
    ...
    // Skills are a sibling of tools: a flat array on the agent config. Each entry is
                        ...
                        {skills.map((skill, index) => {
                            const control = (
                                <SkillConfigControl
                                    ...
                                    onDelete={disabled ? undefined : () => handleSkillDelete(index)}
                                    disabled={disabled}
                                />
                            )
                            return EditorProvider ? (
