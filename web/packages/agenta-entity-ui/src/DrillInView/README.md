...
function TestcaseDetailView({testcaseId}: {testcaseId: string}) {
    return <MoleculeDrillInView entityId={testcaseId} molecule={testcaseAdapter} />
}
```
...
<MoleculeDrillInView
    slots={{
        // Custom breadcrumb
        breadcrumb: (props) => (
            <MyBreadcrumb path={props.path} onNavigate={props.onNavigateToIndex} />
        ),
        ...
        fieldActions: (props) => <ActionButtons {...props.actions} />,

        // Custom empty state
        empty: (props) => <EmptyState isRoot={props.isRoot} />,
    }}
/>
```
