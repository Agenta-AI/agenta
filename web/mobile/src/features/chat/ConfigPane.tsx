    ...
    return (
        <div className="ag-panel-raised ag-scroll-no-bar flex h-full min-h-0 w-full flex-col overflow-y-auto">
            <AgentBuildPanel
                ...
                storageHeader={<StorageFilesHeader revisionId={entityId} sessionId={sessionId} />}
                header={
                    <AgentConfigHeader
                        revisionId={entityId}
                        appId={agentId ?? undefined}
                        // The roster and the agent screens read the workflows list query; a commit
                        // mints a revision they would otherwise not see.
                        // A commit mints a revision: drop any pin so the workspace follows the new
                        // latest instead of staying on the one that was just committed from. The list
                        // query the roster and agent screens read needs the same nudge.
                        onAfterCommit={() => {
                            pinRevision(null)
