...
const meta = {
    title: "@agenta/ui/Presentational/ShortcutKeys",
    component: ShortcutKeys,
    parameters: {
        docs: {
            description: {
                component:
                    "Keycaps for one keyboard shortcut, printed the way the reader's own keyboard is labelled. The faces come from `PLAYGROUND_SHORTCUTS` in `@agenta/shared/utils`, so a hint can never name a key the handler does not bind.\n\nThe platform is read in a mount effect, never during render: the server has no platform and a guess mismatches on hydration. On Apple hardware the caps read `⌘ ⌥ ⌃ ⇧`; everywhere else they read `Ctrl Alt Shift`.",
            },
        },
    },
