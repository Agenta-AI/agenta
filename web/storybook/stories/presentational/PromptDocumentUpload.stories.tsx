...
// PromptDocumentUpload is a drag-and-drop PDF uploader (file + URL input). It was migrated off antd
...
const meta = {
    title: "@agenta/ui/Presentational/Attachments/PromptDocumentUpload",
    component: PromptDocumentUpload,
    parameters: {
        layout: "padded",
        docs: {
            description: {
                component:
                    "A drag-and-drop PDF uploader (file + URL input). Migrated off antd `Upload.Dragger`/`Input`/`Button` to a native drop-zone (`usePromptFileUpload` hook) + @agenta/ui primitives (InputAffix). Shown beside a reproduction of the pre-migration antd body.\n\n**Used in:** 1 place — the playground turn-message adapter (`@agenta/playground-ui` `adapters/TurnMessageAdapter.tsx`).",
            },
        },
    },
} satisfies Meta<typeof PromptDocumentUpload>
 ...
 */
const AntdPromptDocumentUpload = ({disabled}: {disabled?: boolean}) => (
    <Dragger
    ...
    >
        <div className="flex items-center gap-2 w-full">
            <div className="flex items-start gap-4 w-full">
                <FileArchive size={48} className="text-colorTextTertiary" />
            ...
            <Button type="text" icon={<MinusCircleOutlined />} disabled={disabled} />
        </div>
    </Dragger>
)
...
const AgentaControlled = ({disabled}: {disabled?: boolean}) => {
    ...
    return (
        <PromptDocumentUpload
            disabled={disabled}
            value={value}
            onFileChange={(fileData) => setValue(fileData)}
            onRemove={() => setValue("")}
        />
    )
}
...
export const AntdVsAgenta: Story = {
    render: () => (
        <div className="flex max-w-[900px] flex-col">
            <Row label="empty" a={<AntdPromptDocumentUpload />} s={<AgentaControlled />} />
            <Row
                label="disabled"
                a={<AntdPromptDocumentUpload disabled />}
