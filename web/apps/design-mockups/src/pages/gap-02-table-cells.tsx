            ...
            <MockupPageShell
                ...
                notes={
                    <>
                        ...
                        delegates to{" "}
                        <code>@agenta/ui/cell-renderers</code> —
                        <ul style={styles.list}>
                            <li>
                                <code>tryParseJson</code> +{" "}
                                <code>extractChatMessages</code> for type
                                detection
                            </li>
                            <li>
                                <code>JsonCellContent</code> (syntax-highlighted
                                JSON), <code>ChatMessagesCellContent</code>,{" "}
                                <code>TextCellContent</code> for renderers
                            </li>
                            <li>
                                <code>CellContentPopover</code> — hover popover
                                with full content + Copy button
                            </li>
                            <li>
                                Em-dash placeholder for{" "}
                                <code>null</code> / <code>undefined</code> /
                                empty string
                            </li>
                            <li>
                                <code>maxLines={"{10}"}</code> truncation in
                                the cell preview
                            </li>
                        </ul>
                        Functional today. Visible problem: a deeply-nested
                        object renders as ~10 lines of multi-line JSON inside
                        the cell — burns vertical space and reads slow.
                        <br />
                        <br />
                        <strong>What gap-02 actually proposes:</strong> a
                        denser <em>preview format</em>:
                        <ul style={styles.list}>
                            <li>
                                Line 1: <code>[obj]</code> chip +{" "}
                                <code>{"{ 4 props }"}</code> count
                            </li>
                            <li>
                                Line 2: comma-separated first 2-3 keys (or
                                values for arrays)
                            </li>
                            <li>
                                Cell stays ~2 lines tall regardless of nested
                                depth. Hover popover (existing){" "}
                                <code>CellContentPopover</code> still shows the
                                full structure.
                            </li>
                            <li>
                            ...
                            <li>
                                Mixed columns get the <code>[mixed]</code>{" "}
                                chip; dotted-key columns get{" "}
                                <code>[dotted-key]</code>; collision rows
                                stack <code>[⚠ collision]</code>.
                            </li>
                        </ul>
                        <br />
                        <strong>Relationship to gap-01:</strong> chips on cells
                        = gap-01 applied to a different surface. The format
                        proposal here (count, sample keys, popover-on-parsed)
                        is the cell-specific contribution. Both ship together
                        on{" "}
                        ...
                        <br />
                        <br />
                        <strong>Production reuse path:</strong> the dense
                        preview can ship as a new option inside{" "}
                        <code>JsonCellContent</code> (e.g.{" "}
                        <code>variant=&quot;summary&quot;</code>) so the rest
                        of the renderer pipeline doesn't change. Or as a new
                        sibling renderer (<code>SummaryCellContent</code>)
                        called by{" "}
                        <code>TestcaseCellContent</code> when the user toggles
                        density.
                    </>
                }
            >
                <Link href="/solutions-tables" style={styles.cta}>
                    ...
                    <span style={styles.ctaTitle}>
                        Solutions · Tables — full demo →
                    </span>
                    <span style={styles.ctaBlurb}>
                        Side-by-side fixture grid: production{" "}
                        <code>TestcaseCellContent</code> (with popover, syntax
                        highlighting, chat preview) on the left vs{" "}
                        <code>ProposedTableCell</code> (chip + count + sample
                        keys preview) on the right. Each row tagged with the
                        relevant gap.
                    </span>
                </Link>
                ...
                <div style={styles.crossLinks}>
                    ...
                    <Link href="/gap-01-type-chips" style={styles.link}>
                        gap-01 (chip vocabulary — most of what's "missing" on
                        cells today is the chip side)
                    </Link>{" "}
                    ·{" "}
                    <Link href="/gap-04-shape-preservation" style={styles.link}>
                        gap-04 (em-dash for missing keys — already in
                        production, gap-04 just adds the conceptual marker)
                    </Link>{" "}
                    ·{" "}
                    <Link href="/gap-05-dot-key-disambiguation" style={styles.link}>
                        gap-05 (dotted-key on column headers — composes with
                        cell rendering)
                    </Link>{" "}
                    ·{" "}
