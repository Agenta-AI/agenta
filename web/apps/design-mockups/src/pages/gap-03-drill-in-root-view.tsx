    ...
    return (
        <>
            ...
            <MockupPageShell
                ...
                notes={
                    <>
                        <strong>What production already has:</strong>{" "}
                        <code>DrillInContent</code> ships with{" "}
                        <code>collapsedFields</code> state, a per-field caret
                        button (<code>DrillInFieldHeader</code>), and the{" "}
                        <code>showFieldCollapse</code> prop. Each field can be
                        collapsed and re-expanded individually after first
                        render. Messages rendering at any level uses{" "}
                        <code>ChatMessageList</code> automatically (line 1284
                        of <code>DrillInContent.tsx</code>). All of this works
                        today.
                        <br />
                        <br />
                        <strong>What's missing:</strong> auto-expand on first
                        render. The default state is "collapsed cards at the
                        root" — for testcases with nested{" "}
                        <code>inputs</code> or <code>outputs</code>, the user
                        sees{" "}
                        <code>
                            inputs <span style={styles.code}>[json-object]</span>{" "}
                            <span style={styles.code}>[Drill In]</span>
                        </code>{" "}
                        instead of the second-level fields. They have to click
                        ...
                        <ul style={styles.list}>
                            <li>
                                <strong>gap-06</strong>: messages render at
                                root without an extra click. Production already
                                renders <code>ChatMessageList</code> for
                                messages-shaped fields — just at the level the
                                user is currently focused. Auto-expand brings
                                that to root.
                            </li>
                            <li>
                                <strong>gap-04</strong>: the union of authored
                                vs not-authored keys becomes visible at first
