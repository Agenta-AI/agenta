/**
 * ProposalV2FormView — recursive form rendering of an object value.
 *
 * Layout (per the competitor reference shared in feedback):
 *
 *   Top of the section, a vertical stack of form fields. Each field has a
 *   bold label on top and a body below. Fields are separated by ~20px of
 *   vertical space. Children of a nested object are indented behind a
 *   2px light-gray vertical rail.
 *
 *   - String leaf: a small card (1px gray border, 8px radius, white bg).
 *     A compact `Text ▾` dropdown sits in the top-left of the card. Below
 *     it, a `SharedEditor` with line numbers — same chrome the rest of the
 *     code editors get, but with the white-gutter override scoped under
 *     `.proposal-v2-editor`.
 *   - Number leaf: a single-line `InputNumber` with placeholder text.
 *     No card, no dropdown.
 *   - Boolean leaf: a `Switch`. No card.
 *   - Null leaf: italic muted "null".
 *   - Object / array: bold label, then a 2px gray left rail with the
 *     children stacked inside, indented ~16-20px. Children recurse into
 *     this same component.
 *
 * The form fills its parent container exactly — no extra side padding —
 * so the section body in `ProposalV2DrillIn` drops its horizontal padding
...
import {
    ...
    getViewOptions,
    type ProposalV2NestedKind,
    type ProposalV2ViewType,
} from "./proposalV2Views"

const NESTED_KIND_LABEL: Record<ProposalV2NestedKind, string> = {
    string: "string",
    number: "number",
    boolean: "boolean",
    null: "null",
...
const NESTED_KIND_TONE: Record<ProposalV2NestedKind, string> = {
    string: "geekblue",
    number: "blue",
    boolean: "purple",
    null: "default",
...
interface ProposalV2FormViewProps {
    value: Record<string, unknown>
    onChange: (next: unknown) => void
    editable?: boolean
}

export function ProposalV2FormView({value, onChange, editable}: ProposalV2FormViewProps) {
...
function FormField({label, value, depth, editable, onChange}: FormFieldProps) {
    ...
    // variable, so the pattern is consistent across the drawer.
    const isString = kind === "string"
    const stringOptions = useMemo(() => (isString ? getViewOptions(value) : []), [isString, value])
    const [stringMode, setStringMode] = useState<ProposalV2ViewType>(() =>
...
const styles = {
    ...
    rootStack: {
        ...
        // Per the competitor reference: ~24px between top-level fields.
        gap: 24,
    },
    nestedStack: {
        display: "flex",
    ...
    field: {
        display: "flex",
        flexDirection: "column" as const,
        // Generous space between the label/kind chip row and the field
        // body — the previous 6px gap was too tight for nested rows where
        // the body sits behind a left rail.
        gap: 10,
    },
    fieldBody: {
        display: "block",
    ...
    /* String leaf card — no toolbar; the dropdown moved up to the field's
...
export default ProposalV2FormView
