...
/**
 * NextStepEngine - the actual nextstepjs-backed tour engine.
 *
 * Lazy-loaded via next/dynamic from OnboardingProvider only when
 * ONBOARDING_TOURS_ENABLED is true, so the nextstepjs bundle and its
 * initialization never run on the disabled path.
 */
const NextStepEngine = ({children}: {children: React.ReactNode}) => {
    return (
        <NextStepProvider>
