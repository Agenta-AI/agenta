...
const ReferenceTag = ({
    ...
    const slug = identifiers?.slug ?? null
    const slugSwap = hovercardActive && Boolean(slug) && slug !== label
    ...
    const nameNode = slugSwap ? (
        <span className="inline-grid min-w-0">
            <span className="truncate font-medium transition-opacity duration-150 ease-in-out group-hover/refchip:opacity-0 [grid-area:1/1]">
                {label}
            </span>
            <span
                aria-hidden
                // w-0 + min-w-full: render inside the cell the NAME sizes without
                // contributing intrinsic width — a slug longer than the name must
    ...
    const tagNode = (
        <Tag
            ...
            className={clsx(
                ...
                // triggers the slug crossfade — only hovering the chip itself does.
