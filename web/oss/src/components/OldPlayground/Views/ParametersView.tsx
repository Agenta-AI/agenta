...
import {Environment, IPromptRevisions, Parameter, Variant} from "@/oss/lib/Types"
...
const PromptVersioningDrawer: any = dynamic(
    () => import("@/oss/components/PromptVersioningDrawer/PromptVersioningDrawer"),
)
...
const ParametersView: React.FC<Props> = ({
    ...
    const handleHistoryBtn = async () => {
        ...
        try {
            const mod = await import("@/oss/services/promptVersioning/api")
            const fetchAllPromptVersioning = mod?.fetchAllPromptVersioning
            if (!mod || !fetchAllPromptVersioning) return

            if (!fetchAllPromptVersioning) return

            try {
                if (variant.variantId && isDemo()) {
                    const revisions = await fetchAllPromptVersioning(variant.variantId)
