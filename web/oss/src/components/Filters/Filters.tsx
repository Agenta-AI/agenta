...
const Filters: React.FC<Props> = ({
    ...
    const filterBody = (
        <section>
            ...
            <div className={inline ? "flex flex-col gap-2" : filterContainerClass}>
                {displayedFilter.map((item, idx) => {
                    ...
                    const operatorValue =
                        item.operator || (singleOperator ? operatorOptions[0]?.value : undefined)

                    const plan =
                        field && operatorValue ? planInputs(field, operatorValue as any) : undefined
                    ...
                    const availableFeedbackOptions = (() => {
                        if (annotationValue?.evaluator) {
                            const filtered = annotationFeedbackOptions.filter(
                                (option) => option.evaluatorSlug === annotationValue.evaluator,
                            )
                    ...
                    const feedbackValueType =
                        currentFeedback?.valueType ?? selectedFeedbackOption?.type ?? "string"

                    const isEvaluatorActive = annotationValue
                        ? "evaluator" in annotationValue
                        : false
                    const isFeedbackActive = annotationValue ? "feedback" in annotationValue : false
                                        ...
                                        <div className="w-full flex items-center gap-2">
                                            ...
                                            {feedbackValueType === "boolean" ? (
                                                ...
                                                <Input
                                                    ...
                                                    onChange={(e) =>
                                                        handleFeedbackValueChange(e.target.value)
                                                    }
                                                />
                                            )}
                                            <Select
                ...
                <Button
                    ...
                    className={inline ? undefined : "mt-2"}
                >
                    Add
                </Button>
            </div>
    ...
    // Inline mode renders the same editor body directly (no popover/button) — used
    // by the Query Registry manage drawer where the filter is the primary content.
    if (inline) {
        return <div className="w-full">{filterBody}</div>
    }
