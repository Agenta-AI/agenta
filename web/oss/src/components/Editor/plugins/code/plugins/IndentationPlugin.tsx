...
import {$moveCaretToStart} from "../utils/moveCaretToStart"
                    ...
                    () => {
                        ...
                        // This ensures proper indentation for JSON objects, arrays, and code blocks
                            ...
                            const emptyHighlightNode = trailing
                                ...
                                .find(
                                    (node) =>
                                        $isCodeHighlightNode(node) && node.getTextContent() === "",
                                )
                        ...
                        // IMPORTANT: Following the key pattern for selection management
                        // We need to explicitly create and set the selection to ensure it's properly tracked
                        editor.update(() => {
                            // First find the first content node or tab in the target line
                            const targetChildren = selectionTarget.getChildren()
                            const firstContentNode = targetChildren.find(
                                (node) => !$isTabNode(node),
                            )
                            ...
                            if (firstContentNode) {
                                ...
                                console.log("SELECTION CASE 2")
                                // sel.anchor.set(lastTabNode.getKey(), lastTabNode.getTextContentSize(), "text")
                                // sel.focus.set(lastTabNode.getKey(), lastTabNode.getTextContentSize(), "text")

                                $setSelection(lastTabNode.selectEnd())
                            } else {
                                // Position at start of line if no content or tabs
                                console.log("SELECTION CASE 3", selectionTarget)
                                $setSelection(selectionTarget.selectStart())
                                // sel.anchor.set(selectionTarget.getKey(), 0, "element")
                                // sel.focus.set(selectionTarget.getKey(), 0, "element")
                                // $setSelection(sel)
                            }
