...
const WebhookDrawer = ({onSuccess}: {onSuccess: () => void}) => {
    ...
    useEffect(() => {
        ...
        if (initialValues) {
            ...
            const provider: WebhookProvider = isGitHub ? "github" : "webhook"
    ...
    const handleTestConnection = useCallback(async () => {
        ...
        try {
            ...
            const response = await testWebhook(payload)
    ...
    }, [buildPayloadFromForm, open, testWebhook])
    ...
    const handleOk = useCallback(async () => {
        try {
            ...
            if (isEdit && initialValues?.id) {
                ...
                const response = await createWebhook(payload as WebhookSubscriptionCreateRequest)
            ...
            if (subscriptionId && testPayload) {
                try {
                    const response = await testWebhook(testPayload)
                    handleTestResult(response)
                } catch (error) {
                    console.error(error)
                    message.warning(
                        "Webhook saved, but the connection test could not complete. You can retry it from the drawer or table.",
                        10,
                    )
                }
            }
    ...
    const selectedProviderConfig = useMemo(
        () => WEBHOOK_SCHEMA.find((s) => s.provider === selectedProvider),
        [selectedProvider],
    )
                ...
                children: (
                    <div className="flex flex-col gap-3">
                        <div className="mb-4 text-gray-500">
                            Set up a webhook to trigger external services when specific events occur
                            within Agenta.
                        </div>
                ...
                ? [
                      {
                          ...
                          children:
                              activeTab === "logs" ? (
                                  <WebhookLogsTab subscriptionId={initialValues.id} />
                              ) : null,
                      },
                  ]
                : []),
