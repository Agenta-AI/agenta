...
export const RequestPreview: FC<Props> = ({form}) => {
    ...
    const editingWebhook = useAtomValue(editingWebhookAtom)

    const formValues: WebhookFormValues = Form.useWatch((values) => values, form) || {
        provider: "webhook",
    }
    ...
    const preview = useMemo(() => {
        try {
            return buildPreviewRequest(formValues, {
                ...
                subscriptionId: editingWebhook?.id,
                userId: user?.id,
            })
        } catch {
            return null
        }
    }, [formValues, projectId, user?.id, editingWebhook?.id])
