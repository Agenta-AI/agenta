...
const SingleMessage = memo(
    ({message, keyPrefix, index, truncate, maxLines, showDivider}: SingleMessageProps) => {
        ...
        const roleColor = ROLE_COLORS[message.role.toLowerCase()] ?? "#6b7280"

        return (
            <section
                key={`${keyPrefix}-${index}`}
                className="w-full flex flex-col gap-1 text-xs text-gray-700"
            >
                <span className="capitalize text-[11px] font-medium" style={{color: roleColor}}>
                    {message.role}
                </span>
                {displayContent && (
                ...
                {message.tool_calls && message.tool_calls.length > 0 && (
                    <div className="flex flex-col gap-1">
                        <span className="text-[10px] text-gray-400 font-medium">Tool Calls:</span>
                ...
                {showDivider && (
                    <div className="h-px w-full bg-slate-200/90 dark:bg-slate-700/60 rounded-full mt-1" />
                )}
            </section>
        )
    },
