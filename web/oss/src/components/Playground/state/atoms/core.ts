...
// Core playground state container (factory for fresh defaults)
const createDefaultPlaygroundState = (): PlaygroundState => ({
    generationData: {
...
// App-scoped storage for playground state
const playgroundStateByAppAtom = atom<Record<string, PlaygroundState>>({})

// App-aware playground state selector
export const playgroundStateAtom = atom(
    (get) => {
        const appId = get(routerAppIdAtom) || get(recentAppIdAtom) || "__global__"
        const all = get(playgroundStateByAppAtom)
        return all[appId] ?? createDefaultPlaygroundState()
    },
    (get, set, next: PlaygroundState | ((prev: PlaygroundState) => PlaygroundState)) => {
        const appId = get(routerAppIdAtom) || get(recentAppIdAtom) || "__global__"
        const all = get(playgroundStateByAppAtom)
        const prev = all[appId] ?? createDefaultPlaygroundState()
        const value = typeof next === "function" ? (next as any)(prev) : next
        set(playgroundStateByAppAtom, {...all, [appId]: value})
    },
)
