...
export const allInputRowIdsAtom = inputRowIdsAtom
export const allChatSessionIdsAtom = chatSessionIdsAtom

// Entity readers (stable projections)
export const inputRowAtomFamily = atomFamily((rowId: string) =>
    selectAtom(inputRowsByIdAtom, (m) => m[rowId] || null, deepEqual),
)
...
export const rowVariablesAtomFamily = atomFamily((p: {rowId: string; revisionId: string}) =>
    atom((get) => {
        ...
        return row?.variablesByRevision?.[p.revisionId] || null
    }),
)
