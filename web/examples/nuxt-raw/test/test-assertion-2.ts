 ...
 * (Nuxt + raw OTel + SimpleSpanProcessor).
 *
 * Default flush window is 30s here (not 5s like the other phases) because
 * of P-NUXT-01: Nuxt 4 / Nitro / H3 v2 RC has no working abort-signal
 * propagation path, so streamText can't be told to stop server-side. The
 * model keeps generating until natural completion (~7-15s), and the
 * parent span ends only then. The trace DOES arrive — just outside a
 * "client aborts, span flushes within 5s" interactive window.
 */
...
const FLUSH_WINDOW_S = Number(process.env.ASSERTION_FLUSH_WINDOW_S ?? "30")
