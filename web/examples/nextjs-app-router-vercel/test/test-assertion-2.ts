 ...
 *
 * The most consequential streaming assertion: when a user closes the
 * tab, hits browser-back, or otherwise aborts mid-response, does the
 * trace still arrive in Agenta?
 *
 * Per P-NODE-02 (Phase 1): in Node + raw OTel + AI SDK v6, this only
 * works with SimpleSpanProcessor. The App Router instrumentation uses
 * SimpleSpanProcessor for the same reason. This assertion verifies the
 * Node-side trace pipeline survives an HTTP-layer abort triggered from
 * the test client.
 */
...
async function main(): Promise<void> {
    ...
    try {
        ...
        // Read first chunk, then schedule an abort 500ms later.
        ...
        // Continue draining until the abort kicks in.
        try {
            while (true) {
