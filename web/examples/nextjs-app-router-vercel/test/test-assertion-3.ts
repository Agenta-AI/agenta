/**
 * Assertion 3 — Per-call metadata round-trips through the App Router.
 *
 * Same shape as the Node assertion but exercised via the HTTP route's
 * `x-agenta-run-id` header → `runStreamChat({metadata: {userId, sessionId}})`.
 * Verifies the metadata flows from header → request body wiring →
 * generateText/streamText → OTel span → Agenta storage.
 */
