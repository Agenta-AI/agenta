/**
 * Assertion 1 — Cold-start trace completeness via /api/chat (TanStack Start).
 *
 * Same shape as the App Router/Pages Router versions: hit /api/chat with
 * a chat-style payload, drain the stream, then poll Agenta for an
 * ai.streamText span tagged with this run's userId.
 */
...
async function main(): Promise<void> {
    ...
    await verifyTrace({
        ...
        expectAttributes: {
            ...
            // (the generateText shape). Matches Phase 2a App Router raw OTel.
