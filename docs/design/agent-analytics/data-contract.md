...
| Failures | `attributes.ag.metrics.errors.cumulative` | numeric/continuous | `sum` |
...
- `success` = `type.trace` count minus `errors` sum, floored at zero.
- `failed` = `errors` sum. Note this is an **error count**, not strictly a failed-run count:
  `errors.cumulative` rolls up every failing span in a run, so a single run with two failing
  child spans contributes `2`. The floor-at-zero on `success` hides the resulting negative.
  This matches the existing observability mapper's convention (`analyticsToGeneration`), so
  it stays internally consistent; just read the label as "errors," not "failed runs."
...
- `latencyScore` = clamp01((3600 - avgLatencyMs) / 2800). Confirm these two constants before
  implementing; they set where "fast" and "slow" fall.
- `health` = round(100 x (0.72 x successRate + 0.28 x latencyScore)).
- Band: Healthy at 85 and above, Watch from 65 to 84, At risk below 65.
