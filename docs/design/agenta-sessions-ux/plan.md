...
Status: **plan, not started.** Nothing in this document is built.
...
| Triggered / manual | — | No origin marker exists — checkpoint 2 |
...
2. **Pinned sessions**.
3. **Recent sessions**.
4. **Recent automation runs** — trigger deliveries, newest first, with status and error. Not
   linked to their sessions yet (no delivery→session id).
