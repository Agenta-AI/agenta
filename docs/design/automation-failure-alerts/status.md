...
| 2026-09-24 | v0 channel is email only. No in-app notifications, Slack or webhooks in v0 | Email reaches people outside the app; the rest can reuse the same event later |
...
| 2026-09-24 | No new table; incidents are derived from `trigger_deliveries` | Deliveries already hold the history; a table can come later if incidents need their own UI |
| 2026-09-24 | Do not store alert state on the schedule or subscription row | An edit replaces `meta`, `data` and `flags` whole |
| 2026-09-25 | Link a delivery to its turn through the existing `run_id` | It is already the runner `turn_id`; verified on 20 of 20 local runs |
| 2026-09-25 | A failure is a non-quarantined `error` record on the turn | `done.stopReason` is empty on failed turns; one path writes no `done` |
| 2026-09-25 | Settle jobs go on a new taskiq queue, not a second group on `streams:sessions` | The outbox deletes entries after it reads them |
...
| Is "needs approval" (a paused run) in v0? | Proposal: yes, as its own email, because nobody watches automation runs |
| Does an empty answer count as a failure? | Proposal: no in v0; record it as success with a flag for later review |
| Reminder interval | 24 hours |
| Send a "back to normal" email? | Proposal: yes |
...
| Real volume and failure rate | Run [sizing.sql](./sizing.sql) (core, then tracing) | Storm cap and catalog priorities |
