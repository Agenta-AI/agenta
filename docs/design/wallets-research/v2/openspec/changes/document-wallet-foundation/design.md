...
Source: [Wave 1](../../../../v1/wave-1.md), [entities](../../../../v1/entities.md), and `api/ee/src/core/wallets/`, `measurements/`, and their PostgreSQL and worker implementations at the pinned PR head. The foundation code is implemented, but this documentation change is not an archived main-branch specification. Current settlement serialization prevents duplicate settlement; it does not prevent two provider requests from passing an earlier non-reserving check. Restricted selection exists at settlement, not action-aware admission. The expiry and repeated-plan-transition gaps were fixed on `wallets/takeover` (open-designs items 21 and 22); failed-adjustment recovery (item 22, option 3) stays with the hardening change. The spec describes `wallets/takeover` at `0693b307c1`. See ../../../spec-divergences.md for every place the code and the original design disagree, and ../../../review-findings.md for the test evidence.
