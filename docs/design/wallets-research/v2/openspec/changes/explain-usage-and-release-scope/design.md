...
Use existing measurements and wallet records before proposing analytics materialization. Core financial state and tracing usage may require permission-checked cross-store retrieval and a retention policy. Do not infer final reporting joins or permanent trace retention. Release scope is selected by Mahmoud. Sandbox-only lifetime usage must not be displayed as an empty, unusable account simply because wallet credit is zero.
