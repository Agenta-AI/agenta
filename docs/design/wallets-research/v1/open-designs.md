...
| Two stream contracts | Define versioned `streams:measurements` and `streams:debits` envelopes: required IDs, scope/actor dimensions, resource key/locator, optional metrics/costs, `amount_musd`, idempotency key, and retry/ACK ownership. | WPs cannot independently build the API producer, gateway worker, and wallet worker without one stable contract. |
