...
| Stop, then continue INSIDE the park window | local | codex | integration `9110c08000` | child alive through the warm turn 2 | alive at 20.5 s and after turn 2 | `run-codex-3-warm-turn2.json`, `runner-recovered-codex-3-warm-turn2.log` |
| Same, with a `python3` blocking command | local | codex | integration `9110c08000` | child ALIVE, so not a `sleep` artefact | alive at 35.8 s and after turn 2 | `run-codex-4-python.json`, `runner-recovered-codex-4-python.log` |
| Stop a running `sleep` | local | pi_core | integration `9110c08000` | child GONE | gone by 0.2 s | `run-pi_core-1.json`, `runner-recovered-pi_core-1.log` |
| Stop a running `python3` blocker | local | claude | integration `9110c08000` | child GONE | gone by 0.2 s | `run-claude-3.json`, `runner-recovered-claude-3.log` |
...
**Every run has a runner log, but four of them were recovered rather than captured.** The
`daytona-matrix` lane recreated the shared stack's runner container at 12:18 UTC to load a Daytona
key that can manage Secrets, which discarded the container's own log history before four of the
five integration runs had been read out. That lane preserved the complete previous log first, at
`~/agenta-qa-evidence/2026-09-03-session-round2/daytona/preserved-runner-log/runner-before-recreate.log`,
spanning 2026-09-02T22:31:34Z to 2026-09-03T12:18:02Z, which covers all five runs. The four missing
logs were extracted from it into `runner-recovered-*.log`.

One caveat for anyone reading those four files. The `stage=harness_cancel` and
`prompt stopReason=` lines carry no session id, so they are attributed by timestamp and each file
holds the neighbouring run's lines as well. The attribution is still unambiguous, because the
`park-cancelled` line one second later names the session. Every recovered run shows
`harness_cancel sent=true settled=true`, `prompt stopReason=cancelled`, `park-cancelled
ttl=60000ms`, and `hit-continue` on the next turn.
