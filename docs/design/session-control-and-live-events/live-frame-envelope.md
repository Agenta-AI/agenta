...
Set both limits per session:

- Maximum age: **15 minutes**.
- Maximum length: **100,000 frames**, trimmed approximately.

The highest observed run-average rate was 75.632 frames/s. Fifteen minutes at that rate is
`75.632 * 900 = 68,069` frames. A 100,000-frame cap leaves 47 percent headroom and represents
22.0 minutes at that rate. It also holds 31.6 times the largest measured run of 3,161 frames.

The largest measured run used 201,056 frame bytes. Scaling its 63.6-byte average to 100,000 frames
gives about 6.36 MB of SSE frame bytes. This excludes the envelope and Redis overhead. Enforce age
and length independently. A reader outside either bound discards temporary previews, reloads the
durable snapshot, and follows current frames.
