...
| Browser acceptance suites | NOT READY, blocking the gate | The suite that drove the old flow is replaced by seven cases in `web/oss/tests/playwright/acceptance/settings/mcp-connect.ts`, driven by the OSS and EE `mcp-connect.spec.ts` files (`548d1e4aef`): connect without authentication, the name suggested from server metadata, a duplicate display name refused in the field, two connections to one URL kept apart, remove, the tool list on a connection, and the OAuth path through the mock issuer ending in a disconnect that keeps the connection. Round-2 finding D25 is that this suite skips on every run and a skip reports as a pass, so the cases have never actually protected anything. It is unfixed and blocks the gate on its own. The OAuth case also needs `PLAYWRIGHT_HOST_RESOLVER_RULES` so the browser resolves the address the issuer publishes. Exercising the flow by hand against a running stack did find and fix one defect, a Disconnect offered on a connection holding no grant, and round 2 found two more the suite was supposed to catch. |
