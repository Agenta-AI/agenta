...
Two fix batches are in flight, the branch now conflicts with main, and no evidence has been posted
to the pull request. The gate row at the bottom lists all of it.

## Candidate revisions

| Revision | What it is | Notes |
| --- | --- | --- |
| Current branch head | Moving | Two fix batches are landing on it, so it is not quoted here. Rows below are measured at the head they name. |
...
| MCP release gate | NOT READY | Still no unbuilt feature on this list, but it is no longer only paperwork: three verification rounds each found real defects in the fixes they were checking, so the remaining items include fixing what they found. In the order they unblock each other: the runner batch closing round-3 D63, D64 and D65; the web batch closing round-4 D3 and D7 and the round-3 web findings; reviewer verification of every fix landed since `5c67a2e871`, which no reviewer has read; a UI QA round 5 verifying the round-4 fixes, since round 4 showed that verifying a fix on its commit message is how a syntax error reaches a page; a recorded browser-suite tally, none having been taken since D49 and D53 landed; the rebase onto main as one force-push, with the gitleaks fingerprints regenerated last and the CodeRabbit filter counts re-measured; CI green on what comes out of it, which is also the first full run since the conflict began; CodeRabbit re-run on the rebased head; the public recordings attached to the pull request; the evidence index posted there naming the exact release-ready revision; and the Linear throwaway write, ready to run and waiting on a scratch team. |
