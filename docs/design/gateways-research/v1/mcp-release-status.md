...
**Round 4 closed at ninety-five findings, D95 to D189, checked against the pushed head
`890953d2b4`. No P0, and no P1 open.** **One thing blocks the release and it is
not a defect**: the review gate has seen none of this work, because the branch that configures its
passes was unpushed. It is pushed now, so the gate closes when the two passes complete. The only
thing to hold is that completion is confirmed rather than assumed, since the failure mode is a
refusal that reports as a pass, which is how that finding arose. **No product defect is open.** Every finding raised against the product either landed
...
**One pattern recurred five times.** A guard gets written, the defect is genuinely closed, and nothing
exercises the guard. Four of the five closed once someone looked; one is still open as a P3.

**A second pattern is recorded because it cost more than any single finding.** A reported chat-notice
regression turned out to be neither a product defect nor either mechanism proposed for it: the model
declined to attempt the tool call and answered conversationally, and the assertion fired while the
turn was still running. QA retracted the report, and the explanation offered for it was wrong and is
recorded as wrong, because it fitted both runs while having no evidence on the path it explained, and
it travelled as far as the fix author and the coordinator before a transcript settled it. The commit
that came out of it is hardening rather than a regression fix, and the record labels it so. **An
explanation that fits the observations is not the same as one with evidence on the path it names.**

**The same shape recurred once more in the handling.** Two commits were rewritten to take the
withdrawn premise out of their bodies rather than annotate it, on the ground that the branch was
unpushed. It was not: the fix commit had already been merged and pushed, so the rewrite could not
take, and the correction shipped as a comment-only commit instead. The record carries this as the
reviewer's error rather than the author's, because the instinct to rewrite rather than annotate was
right and only its availability was in question, and that was one command away with the repository
open.

**What the final check established, in one line.** At the pushed head, removing the notice card kills
five cases, removing the hide's scoping kills two including the one guaranteeing an unexplained
failure stays visible, breaking the card's action kills six, and the ported configuration-pane case
kills one. Production attribution is exact: 364 files against 368 before the merge, with all four
