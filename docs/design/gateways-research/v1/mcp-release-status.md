...
is left is process rather than product. Two independent review rounds and one CodeRabbit pass are
on record and three rounds of UI QA have run; the last six fixes are unverified, a fourth QA round
is in flight, a second CodeRabbit pass and a third review round are prepared but unrun, and the
evidence is not yet posted to the pull request. The gate row at the bottom lists all of it.

## Candidate revisions

| Revision | What it is | Notes |
| --- | --- | --- |
| `d50739d927` | Current pushed head | Rows below are measured here unless they name another revision. It carries the last six fixes, all awaiting reviewer verification: D50 (`4ee17eab78`), the blocked-popup return route r3-D1 (`c82e131791`), D51 and D52 (`1dfece1e66`), D53 (`d50739d927`), a tool filter for a server advertising seventy-nine tools (`a3ac5f33b0`), and a quadratic refusal-marker read (`d4a6eab93a`, which also clears the CodeQL alert). |
| `4c698fb068` | The revision CI was last read at | The CI row below is measured here, before the CodeQL fix landed. |
...
| CI | One red left, and it is fixed on the branch | Read at `4c698fb068`: 73 checks pass. The API acceptance failure recorded at the previous refresh has cleared. What was still red is the CodeQL alert for a regular expression, and the fix landed afterwards at `d4a6eab93a`, which reads a refusal's harness marker in one pass rather than in quadratic time. That has not been confirmed green yet, because the run needs the next push. The web acceptance job is failing and is ignored by decision, which is a decision to record rather than a result to cite. |
...
| MCP release gate | NOT READY | Nothing on this list is an unbuilt feature. Every item is verification, review, or publication of work already done, and that is the honest shape of the remaining risk. In the order they unblock each other: UI QA round 4, running now, to verify the r3-D1, r3-D2 and D50 fixes; reviewer verification of the last six fixes, none of which a reviewer has read; a recorded browser-suite tally, since none has been taken since D49, D51, D52 and D53 landed; the final push, which is also what runs CodeQL against `d4a6eab93a`; CodeRabbit pass 2, whose configuration is prepared; independent review round 3 on the final head, whose briefing is prepared; the public recordings attached to the pull request; the evidence index posted to the pull request naming the exact release-ready revision; and the Linear throwaway write, which is ready to run and waits on a scratch team. |
