...
This is the last refresh before the candidate revision changes. Every round-3 blocker is fixed and
verified, the per-tool permission divergence is closed at the wire with all five readers agreeing,
and the round-4 dialog defect is fixed on both surfaces and awaiting live confirmation. One
blocking item is left in code, a runner test fixture. Then the branch rebases onto a main that has
moved far enough to conflict, and the remaining work is a review round and a CI run on what comes
out. The gate row at the bottom gives the order.

## Candidate revisions

| Revision | What it is | Notes |
| --- | --- | --- |
| `60d71db7d6` | The head UI QA round 4B ran against | Verified before and after each rerun. Seventeen commits have landed since: eleven on the API side from CodeRabbit pass 2 and from D54 and D76, and six on the web side, including D88's editor label and the round-4 fixes for D3, D4, D5 and D6. The branch moves under fix batches, so a row measured elsewhere names its own revision. |
...
| Deferred, with issues filed | Documented | Eight things are deliberately not fixed for this release and each is filed rather than remembered. #6892 and #6893 are the two known limitations described below. #6894 is D71's deferred half, the tab-return fallback that lands on a bare settings page when the app and the API are on different origins. #6896 is round-4 D2's residual: the plain reconnect notice now appears in the thread, and the harness's raw failure card still sits collapsed beneath it describing the same event in its own words, so whether the sentence should replace the card rather than sit above it is left as a product decision. #6897 is the mobile agent-configuration acceptance path, the coverage that would have caught D94. The other three: #6879, a reconnected connection that can still read as needing a reconnect, because true atomicity needs a credential-version column and that is a schema change out of scope here. #6880, host derivation stripping `/api` from the wrong string, which breaks runs behind a strict proxy. #6882, test-account sandbox mount trees that are never removed and fill the store. Round-2 D11 and D59 stay deferred with their closures stated in the code. |
| MCP release gate | NOT READY | Seven steps, in order, and each unblocks the next. 1. QA confirms D94 on a served build. 2. The runner SSRF fixture after pass-2 finding P10. 3. The rebase onto main, from the rehearsal. 4. One push, with the gitleaks fingerprints regenerated last and the CodeRabbit file counts re-measured, since a pass that crosses the 300-file cap becomes a silent skip. 5. CI and CodeRabbit on the rebased head, the first full run since the conflict began. 6. Independent review round 4, Codex gpt-6-astra at medium reasoning plus Opus, on that head. 7. The evidence post, with the recordings attached and the release-ready revision named. Mahmoud's three items are in the section below; the scratch team blocks the last piece of real-provider evidence. |
