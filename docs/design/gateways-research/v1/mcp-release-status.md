...
| MCP release gate | NOT READY | Remaining mandatory items: the doubled tool-prefix defect on the agent path, which blocks real-provider execution from an agent run and is not yet written up in `qa.md`; r3-D1, the stranded tab when popups are blocked; r3-D2, the raw harness error and absent reconnect after a disconnect; D50, the only P1 left from review round 2; the open CodeQL regular-expression alert; the newly failing API acceptance job classified and green; the playground spec; CodeRabbit pass 2; independent review round 3 on the final head; the public recordings attached to the pull request; and the Linear throwaway write, which is ready to run and waits on a scratch team. |
