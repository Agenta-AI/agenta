...
def standard_llm_endpoint(*, provider_key: str) -> Optional[LlmEndpoint]:
    ...
    deployment=DIRECT, model_slugs from the map, config at code defaults, no id and
...
stores these ids verbatim in `LlmEndpointData.model_slugs`; nothing here re-derives or strips a
prefix.
...
- **The empty allowlist refuses** (§4.4's LLM-plane echo, stated at §7.1's end): `model_slugs:
  []` on a custom endpoint means no model may be called, not "everything." Standard endpoints
...
- **`CeilingExceededError` names all three numbers** (D25): `ceiling`, `requested`, `allowed`,
  `target`. Guards **our** ceilings (`LlmEndpointConfig.max_output_tokens`) only — never
...
- `standard_llm_endpoint(provider_key="openai")` returns an `LlmEndpoint` with
  `namespace=BUILTIN`, `slug="openai"`, `deployment=DIRECT`, `model_slugs` matching
