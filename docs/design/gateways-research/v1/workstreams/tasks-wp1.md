...
- [ ] `dbs/postgres/gateways/llms/dbas.py`: add `LlmEndpointDBA` —
      ...
      nullable=True`.
- [ ] `dbs/postgres/gateways/mcps/dbas.py`: add `McpEndpointDBA` — same base mixins as
      `LlmEndpointDBA`, plus `auth_mode: SQLEnum(GatewayAuthScheme,
      name="gatewayauthscheme_enum"), nullable=False`, `secret_id: UUID,
      nullable=True`.
- [ ] Add `McpGrantDBA` — `ProjectScopeDBA, IdentifierDBA, LifecycleDBA, StatusDBA,
...
- [ ] Ruff format + check both files; commit.

## dbes

- [ ] `dbs/postgres/gateways/llms/dbes.py`: `LlmEndpointDBE` — `__tablename__ =
...
- [ ] `dbs/postgres/gateways/mcps/dbes.py`: `McpEndpointDBE` — `__tablename__ =
...
- [ ] `McpGrantDBE` — `__tablename__ = "mcp_gateway_grants"`; PK `(project_id, id)`; FK
...
- [ ] Confirm by grep over both `dbes.py` files that no unique constraint or index
      mentions `url` or `secret_id` alone — deliberate absence (§3).
- [ ] Ruff format + check; commit.

## Verification: models import

- [ ] Import both `dbes.py` modules in a throwaway shell / smoke test — confirm the
...
- [ ] `dbs/postgres/gateways/llms/mappings.py`: `map_llm_endpoint_create_to_dbe`,
      ...
      unconditionally.
- [ ] `dbs/postgres/gateways/mcps/mappings.py`: the same three-function set for
...
- [ ] Unit test: instantiate one representative DTO per entity
...
- [ ] `dbs/postgres/gateways/llms/dao.py`: `LlmEndpointsDAO.__init__(self, *,
...
- [ ] Implement `create_endpoint`: `@suppress_exceptions(exclude=[EntityCreationConflict])`;
...
- [ ] Implement `fetch_endpoint`, `fetch_endpoint_by_slug` — `@suppress_exceptions(default=None)`,
      `select(...).filter(project_id==..., id/slug==...).limit(1)`.
- [ ] Implement `edit_endpoint` — `@suppress_exceptions(default=None)`; fetch the row by
      ...
      `updated_by_id`.
- [ ] Implement `delete_endpoint` — `@suppress_exceptions(default=False)`; `delete(...)`
      by `(project_id, endpoint_id)`; return `result.rowcount > 0`.
- [ ] Implement `query_endpoints` — `@suppress_exceptions(default=[])`; filter by
...
- [ ] `dbs/postgres/gateways/mcps/dao.py`: `McpEndpointsDAO` — same six methods, same
      shape, over `mcp_gateway_endpoints` / `uq_mcp_gateway_endpoints_project_slug`.
- [ ] Ruff format + check; commit.

## dao — mcps (grants)

- [ ] Same file: `McpGrantsDAO.__init__` — same shape.
- [ ] Implement `create_grant`: `@suppress_exceptions(exclude=[EntityCreationConflict])`
...
- [ ] Implement `fetch_grant` — `@suppress_exceptions(default=None)`; filter
...
- [ ] Implement `fetch_grant_by_id` — `@suppress_exceptions(default=None)`.
- [ ] Implement `update_grant` — `@suppress_exceptions(default=None)`; `UPDATE ... WHERE
...
- [ ] Implement `delete_grant` — `@suppress_exceptions(default=False)`; row delete by
      `(project_id, grant_id)` only — does not touch the vault secret (the service does
      that first, per §2.1).
- [ ] Implement `query_grants` — `@suppress_exceptions(default=[])`; filter by
...
- [ ] `upgrade()`: `op.create_table("llm_gateway_endpoints", ...)` — every column from
      `LlmEndpointDBA`/`DBE`, the PK, both FKs, the slug unique constraint.
- [ ] `op.create_index("ix_llm_gateway_endpoints_project_provider", ...)`,
      `op.create_index("ix_llm_gateway_endpoints_flags", ..., postgresql_using="gin")`.
- [ ] `op.create_table("mcp_gateway_endpoints", ...)`, its PK, both FKs, the slug unique
      constraint, `op.create_index("ix_mcp_gateway_endpoints_flags", ...)`.
- [ ] `op.create_table("mcp_gateway_grants", ...)`, PK, both FKs (`secret_id` →
      `ondelete="CASCADE"` — double check this is not `SET NULL`).
- [ ] `op.create_index("uq_mcp_gateway_grants_user", ..., unique=True,
      postgresql_where=sa.text("user_id IS NOT NULL"))`.
- [ ] `op.create_index("uq_mcp_gateway_grants_project", ..., unique=True,
      postgresql_where=sa.text("user_id IS NULL"))`.
- [ ] `op.create_index("ix_mcp_gateway_grants_endpoint", ...)`.
- [ ] `downgrade()`: drop every index created above, then `op.drop_table("mcp_gateway_grants")`,
...
- [ ] `api/oss/tests/pytest/unit/gateways/test_gateways_mappings.py` — the DTO↔DBE
...
- [ ] `api/oss/tests/pytest/integration/gateways/test_gateways_llm_endpoints_dao.py` —
...
- [ ] `api/oss/tests/pytest/integration/gateways/test_gateways_mcp_endpoints_dao.py` —
      same six assertions against `mcp_gateway_endpoints`.
- [ ] `api/oss/tests/pytest/integration/gateways/test_gateways_mcp_grants_dao.py` —
...
- [ ] `api/oss/tests/pytest/integration/gateways/test_gateways_migration.py` — `alembic
      upgrade head` then `alembic downgrade -1` round-trips against a throwaway
      database; every constraint/index above exists post-upgrade and is gone
      post-downgrade; a second `upgrade` after `downgrade` also completes.
- [ ] Same file or a sibling: deleting a `secrets` row referenced by an endpoint's
...
- [ ] Write the three-line DAO-construction diff from `specs-wp1.md` into this
