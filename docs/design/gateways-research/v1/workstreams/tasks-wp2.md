...
- [ ] Read `sdks/python/agenta/sdk/agents/platform/connections.py` in full, focusing on
...
- [ ] Read `core/secrets/services.py` (`VaultService`) and `core/secrets/context.py`
...
- [ ] `core/gateways/policy/resolution.py`: `CredentialResolver.__init__(self, *,
      vault_service: VaultService, mcp_grants_dao: McpGrantsDAOInterface) -> None`,
      implementing `CredentialResolverInterface`.
- [ ] `async def resolve(self, *, scope: AuthScope, ref: CredentialRef, mode:
...
- [ ] Implement `_resolve_bound_secret`: call `vault_service.get_secret_by_id(ref.secret_id,
...
- [ ] Write the mode dispatch explicitly for this arm even though all three modes
...
- [ ] Implement `_resolve_provider_key`: `secrets = await
...
- [ ] Once a grant row is found (any mode): if `not grant.flags.is_valid`, raise
...
- [ ] Otherwise call `vault_service.get_secret_by_id(grant.secret_id,
...
- [ ] Build `owner=CredentialOwner(kind=USER, user_id=grant.user_id)` when the resolved
...
- [ ] Ruff format then ruff check `resolution.py`; fix all errors.
- [ ] Commit: "core/gateways: implement CredentialResolver".

## tests — unit (run now)

- [ ] `api/oss/tests/pytest/unit/gateways/test_gateways_resolution.py`: build a minimal
...
- [ ] `BoundSecretRef`: secret exists → resolves with `owner.kind == PROJECT`.
- [ ] `BoundSecretRef`: secret missing → `CredentialNotFoundError(missing=PROJECT)`.
- [ ] `ProviderKeyRef`: `provider_key`-kind match → resolves it.
- [ ] `ProviderKeyRef`: no `provider_key`-kind match, `custom_provider`-kind match
      exists → resolves the `custom_provider` one.
- [ ] `ProviderKeyRef`: both kinds match → resolves the `provider_key`-kind one.
- [ ] `ProviderKeyRef`: no match → `CredentialNotFoundError(missing=PROJECT)`.
- [ ] `GrantRef`, `PROJECT_ONLY`, only a user-owned grant exists → `CredentialNotFoundError`
...
- [ ] `GrantRef`, `USER_REQUIRED`, only a project-owned grant exists → `CredentialNotFoundError(missing=USER)`
      (must NOT fall back to the project's — the single most important assertion in this
      package).
- [ ] `GrantRef`, `USER_REQUIRED`, a user-owned grant exists → resolves it,
      `owner.kind == USER`, `owner.user_id == scope.user_id`.
- [ ] `GrantRef`, `USER_OPTIONAL`, both a user grant and a project grant exist →
      resolves the user's.
- [ ] `GrantRef`, `USER_OPTIONAL`, only a project grant exists → resolves it,
      `owner.kind == PROJECT`.
- [ ] `GrantRef`, `USER_OPTIONAL`, neither exists → `CredentialNotFoundError(missing=USER)`.
- [ ] `GrantRef`, grant exists with `is_valid=False` → `CredentialInvalidError`,
      regardless of mode; assert the fake vault's `get_secret_by_id` was never called.
- [ ] `GrantRef`, grant exists and valid, `secret_id` resolves to nothing in the fake
      vault → `CredentialInvalidError` (not `CredentialNotFoundError`).
- [ ] Every raised `CredentialNotFoundError`/`CredentialInvalidError` across the above:
...
- [ ] Write the `CredentialResolver(vault_service=vault_service,
