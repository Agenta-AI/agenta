...
A fake LLM endpoint and a fake MCP server, first-class deliverables (D23) rather than test
...
"our own servers and the fakes" (`open-designs.md` OD10), so nothing downstream can be
...
- **The adapter-level fake** — `FakeLlmAdapter` / `FakeMcpAdapter`, in-process classes
  implementing the south port (`LlmUpstreamInterface` / `McpUpstreamInterface`), registered under
  the `"fake"` adapter key alongside `passthrough`/`translated` and `http`/`composio`
  (`entities.md` §9, the wiring block). No network call, no process. This is what `entities.md`
  draws in the file tree (§0, `providers/fake/adapter.py`) and what satisfies **unit and contract
  tests**: a test constructs a service with `upstream_registry=...Registry(adapters={"fake":
  FakeLlmAdapter()})` and calls it directly — nothing running.
- **The deployable fake** — a standalone process speaking the real upstream protocol (OpenAI-
  compatible HTTP for the model plane, MCP Streamable HTTP for the tool plane), run as its own
  docker-compose service. `entities.md` §0 says outright: *"Not shown: the deployable fakes...
  Checkpoint A's acceptance tests additionally need the fakes running as compose services in the
...
**Explicitly not built here:** the registry that picks `"fake"` for a given `(provider_key,
...
the fake MCP server's catalog entry (`core/gateways/mcps/service.py` — WP9); the custom LLM
endpoint row that points a slug at the fake LLM server's URL (WP1's DAO / WP10's CRUD, seeded by
...
- `core/gateways/llms/providers/fake/app.py` — the deployable OpenAI-compatible fake server
  (not in `entities.md`'s tree — it is the "deployable fake" the document explicitly disclaims;
...
- `core/gateways/mcps/providers/fake/app.py` — the deployable MCP Streamable HTTP fake server
- `core/gateways/mcps/providers/fake/__init__.py`

Compose wiring (owned by no other package):
- `hosting/docker-compose/oss/docker-compose.dev.yml` — two new services, `fake-llm-gateway` and
  `fake-mcp-gateway`
- `hosting/docker-compose/ee/docker-compose.dev.yml` — same two services (EE dev stack mirrors
  OSS dev for anything not license-gated, per the compose file list; the fakes are not EE work)
- `api/oss/src/utils/env.py` — one `FakeGatewaysConfig` block, following the `ComposioConfig`
  shape (`env.py` lines 685–704): two URLs the fakes' own consumers (WP1/WP9/WP10) read to seed
...
`GatewayUsage` (§4.2, `core/gateways/policy/dtos.py`), populated by `FakeLlmAdapter` once `body`
...
```python
# core/gateways/llms/providers/fake/adapter.py
...
`api/entrypoints/routers.py` per the wiring snippet (§9): `"fake": FakeLlmAdapter()`. It never
opens a socket; `credential` may be `None` (targets with `GatewayAuthScheme.NONE` are the
intended callers, per §2's "an endpoint with no credential is legitimate — the fake (D23)") and
...
|---|---|
| `fake/echo` (default; any name not matching a suffix below) | Returns a well-formed chat-completion response echoing the request's last message content; if `context.stream` is set, streams it as 2–3 SSE chunks ending `data: [DONE]\n\n`, matching the OpenAI streaming shape |
| `fake/error` | Raises `LlmUpstreamError(provider_key="fake", status_code=500, detail="forced by fake/error")` |
| `fake/slow-{seconds}` | `await asyncio.sleep(seconds)` before returning the `fake/echo` response — this is what WP6's relay-side timeout must fire against; `{seconds}` is a plain integer, e.g. `fake/slow-30` |

`GatewayUsage` on a successful call: `calls=1`, `input_tokens`/`output_tokens` counted off the
request/response body length (word count is enough — this is a fake, not a tokenizer), `cost=0.0`
(the fake spends nothing).

## `FakeMcpAdapter`

```python
# core/gateways/mcps/providers/fake/adapter.py
...
docstring — "same method, same body, same response"); a fake *server* interpreting its own
...
`McpUpstreamError(target="agenta/<slug>", status_code=501)` — there is no fourth method to fake.
...
## The deployable fakes

Two standalone ASGI apps, each importable and runnable independently of the main API process
(`uvicorn core.gateways.llms.providers.fake.app:app`). They implement the **same control
convention** as the adapters above, because the whole point is that a test written against the
in-process `FakeLlmAdapter` and a test written against the compose service see identical
behavior for the same input:

- `core/gateways/llms/providers/fake/app.py` — a FastAPI (matching the API's own framework)
  ...
  field with the identical `fake/echo` / `fake/error` / `fake/slow-{n}` convention. No
...
- `core/gateways/mcps/providers/fake/app.py` — a stateless-JSON-mode MCP Streamable HTTP server:
...
```yaml
    fake-llm-gateway:
        ...
        command: ["uvicorn", "oss.src.core.gateways.llms.providers.fake.app:app",
    ...
    fake-mcp-gateway:
        ...
        command: ["uvicorn", "oss.src.core.gateways.mcps.providers.fake.app:app",
...
compose.dev.yml` lines 6–7) rather than a new Dockerfile: the fake apps are pure Python modules
...
class FakeGatewaysConfig(BaseModel):
    ...
    llm_url: str = os.getenv("AGENTA_FAKE_LLM_GATEWAY_URL", "http://fake-llm-gateway:9091")
    mcp_url: str = os.getenv("AGENTA_FAKE_MCP_GATEWAY_URL", "http://fake-mcp-gateway:9092")
```

registered on `EnvironSettings` next to `composio` (`env.py` line 1609). WP1/WP10 (the custom LLM
endpoint row that seeds the fake) and WP9 (the `agenta`-namespace MCP entry) read these; WP5 does
...
WP7 and WP9 add the corresponding `"fake": FakeLlmAdapter()` / `"fake": FakeMcpAdapter()` entries
...
- **Registered always, reachable conditionally.** The wiring snippet's own comment: `"fake":
  FakeLlmAdapter(), # registered always; reachable only via the fake endpoints the local stack
...
- **Transparent-relay discipline still applies to the adapter's exceptions.** `LlmUpstreamError`
  / `McpUpstreamError` are for transport-level failures the fake is asked to simulate
  (`fake/error`); a tool's own business failure (`fail`) is a JSON-RPC result, never an exception
  — this is D16 and `api/AGENTS.md`'s pass-through rule, and the fake exists partly to prove the
  distinction is testable.
- **No secret material anywhere in this package.** The fakes' whole point is an unauthenticated
  target (`GatewayAuthScheme.NONE` on the MCP side, `secret_id=None` on the LLM side per §2); if a
  test ever needs the fakes to *require* a credential, that is a different fixture, not this one.

## Tests

Unit — nothing running, both adapters exercised as plain Python objects:
- `FakeLlmAdapter().relay_chat_completion(..., context=LlmCallContext(model="fake/echo", ...))`
  returns a well-formed `LlmRelayResult`, `status_code=200`, non-empty `body`.
- `context.model="fake/error"` raises `LlmUpstreamError` with `provider_key="fake"`.
- `context.model="fake/slow-1"` takes ≥1s wall-clock (use a short value, not the eventual
...
- Streaming: `context.stream=True` yields more than one chunk over `body`, terminated by `data:
  [DONE]`.
- `FakeMcpAdapter().relay(...)` with a `tools/list` body returns all three tools;
...
Contract — the same fixture both a fake and (once it exists) a real adapter must pass, run against
`FakeLlmAdapter`/`FakeMcpAdapter` now and reused by WP6/WP7/WP8/WP9's own adapters later. Still
nothing running:
...
- `curl -sf http://localhost:9091/health` and `.../9092/health` both return 200 once
  `fake-llm-gateway` / `fake-mcp-gateway` are healthy.
- `POST http://fake-llm-gateway:9091/v1/chat/completions` with `"model": "fake/echo"` returns a
...
- The same request with `"model": "fake/slow-30"` and a client-side timeout shorter than 30s
  observes the connection cut, proving the hang is real (a genuine open socket, not a Python
  `await` a test harness can preempt).
- `POST http://fake-mcp-gateway:9092/` with a `tools/list` JSON-RPC body returns the three tools
...
docker compose exec api curl -sf http://fake-llm-gateway:9091/health
docker compose exec api curl -sf http://fake-mcp-gateway:9092/health
```
...
Matches `plan.md` WP5's own done condition verbatim: *"both fakes run in the local stack and can
be driven to fail on demand."*

## Out of scope

- `select_upstream` and the `LlmUpstreamRegistry`/`McpUpstreamRegistry` classes — WP7 / WP9.
- The `agenta`-namespace code that turns the fake MCP server's URL into a listed endpoint — WP9's
  `service.py`.
- The custom LLM endpoint row that turns the fake LLM server's URL into a reachable
...
- `McpScopeInsufficientError`-driven behavior (forced scope challenges) — deferred to wave 3
  alongside the OAuth checkpoint; the type exists, the fake does not yet exercise it.
- Any change to `passthrough`/`translated`/`http`/`composio` adapters — WP6, WP7, WP8.

## Missing from the design, needs a ruling

- **The deployable fakes' implementation files.** `entities.md` explicitly places the deployable
  fakes "out of this document's scope," so `app.py` under each `providers/fake/` directory is not
  ...
  spec.
- **The env var names** (`AGENTA_FAKE_LLM_GATEWAY_URL`, `AGENTA_FAKE_MCP_GATEWAY_URL`) and the
  compose service names (`fake-llm-gateway`, `fake-mcp-gateway`) are this package's own choice for
...
- **How the fake LLM server gets a seeded `custom` endpoint row in the local stack** (a migration
  ...
  acceptance test that reaches the fake *through the gateway* rather than directly — raise it at
  the M1→Checkpoint A merge point.

## Checkpoint

Feeds **Checkpoint A** (`plan.md`): *"The LLM gateway and the MCP gateway both accept a call,
authorise it, resolve and inject a secret, reach a fake upstream, and return... Everything it
proves is proved against our own fakes."* WP5's direct contribution: without it there is no fake
upstream for WP6/WP7/WP8/WP9's relay paths to reach, and Checkpoint A's acceptance suite (a
permitted request reaches the fake with the caller's token replaced by the upstream secret; a
