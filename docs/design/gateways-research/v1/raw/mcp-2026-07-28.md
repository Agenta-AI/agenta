...
1. **Stateless routing.** A gateway can derive policy-relevant method and target fields from a
   parsed JSON-RPC request while forwarding the original body unchanged. The protocol does not
   require private method or target routing headers.
