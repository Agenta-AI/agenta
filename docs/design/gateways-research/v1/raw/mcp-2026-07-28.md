...
session** state. OAuth is **credential lifecycle** state — expiry, refresh, step-up scopes,
...
2. Auth support splits cleanly by server, not by protocol: static-credential servers are
