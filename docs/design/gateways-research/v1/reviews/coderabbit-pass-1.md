...
| M11 | `services/runner/src/tools/relay-watch.ts:130-142` | Real | `9e848417de`, `ad52aa0777`, `ceccffbf38` | The local relay source declares an abort signal and ignores it, so an abort is honoured only if the source is also closed. Bounded delay. **Not closed in effect.** The source now honours a signal, but the only caller passes none (`services/runner/src/tools/relay.ts:976`), so an abort still reaches neither source. The test supplies the signal the product never supplies. **Now closed in code** by `ad52aa0777`: the signal is wired through `run-turn.ts:1238`, the only production caller, and reaches the source at `relay.ts:987`. **Now guarded too** by `ceccffbf38`, which drives the real engine and asserts the signal's identity all the way to the wait. Checked by mutation here rather than taken on trust: reverting the argument at the call site fails it, and so does reverting the forwarding in the relay, which is what the residual asked for. |
...
**The coverage gap is closed too**, at `915f3be06a`, and the case is built carefully. The earlier
ones could not have covered this path: they build a storage with no callback address, so the address
slug is never a candidate at all. The new case gives the storage its callback, puts a crafted row at
the address slug and a marked registration at the issuer slug, and asserts both that the genuine one
wins and that the address slug was actually read — so it cannot pass by the path never being taken.
Pinned before the preference, it fails. Thirty-one cases pass at the fix.

The reverse guard beside it, that a marked row at the address slug still beats an older issuer row,
passes either way, because without the preference the address slug short-circuits and returns that
row regardless. It is a non-regression guard rather than a discriminator, which is the right thing
