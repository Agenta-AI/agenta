...
5. Let users save active models. Support manual model IDs when discovery is unavailable.
6. Let users save an allowed harness subset. Disable technically incompatible harnesses using the
   global harness catalog.
7. Reopen saved connections with their model and harness choices intact.
...
2. Missing and empty model lists need distinct, explicit behavior.
