...
| How long does a rotated-away refresh token stay valid? | Between 30 and 102 minutes. The old token still refreshed at 15 and 30 minutes of age. At 102 minutes the server answered HTTP 401 with `code: refresh_token_reused` and the message "Your refresh token has already been used to generate a new access token. Please try signing in again." Log: `~/agenta-qa-evidence/2026-09-08-hosted-subscriptions/reuse-window.log`. | real provider |
