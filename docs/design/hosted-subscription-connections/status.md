...
| Real Pi refresh: stored login expired in the database, then one turn | half passed: Pi refreshed with the real provider under its lock (local file holds a fresh 10-day token, new refresh fingerprint). FAILED: nothing published, the store still holds the expired copy. Defect in the publish path, fix in progress. | `cell-refresh-2.log`; runner log for session `d8363389` |
