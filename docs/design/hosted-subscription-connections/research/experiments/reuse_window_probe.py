...
a = ap.parse_args()
stash = os.path.expanduser(a.stash)
if a.arm:
    doc = json.load(open(os.path.expanduser(a.codex_auth)))
    old = doc["tokens"]["refresh_token"]
    acc = doc["tokens"]["account_id"]
    s, b = refresh(old)
    print(
        "arm refresh:",
        s,
        ...
        fp(old),
        "new=",
        fp(b.get("refresh_token", "")) if s == 200 else b.get("error"),
    )
    if s != 200:
        raise SystemExit(1)
    json.dump(
    ...
    for p in a.write_back:
        ...
        d["tokens"].update(
            ...
            id_token=b.get("id_token", d["tokens"].get("id_token")),
        )
    ...
    for p in a.pi_write_back:
        ...
        d["openai-codex"] = {
            ...
            "expires": int(time.time() * 1000) + int(b.get("expires_in", 0)) * 1000,
            "accountId": acc,
        }
        json.dump(d, open(p, "w"), indent=2)
        os.chmod(p, 0o600)
        print("wrote", p)
if a.check:
    st = json.load(open(stash))
    age = time.time() - st["rotated_at"]
    s, b = refresh(st["old_refresh"])
    print(
        f"check: old={st['old_fp']} age={age / 60:.1f}min status={s} "
        + (
            f"new={fp(b['refresh_token'])} (discarded)"
            if s == 200
            else f"error={ {k: b.get(k) for k in ('error', 'code', 'error_description')} }"
        )
    )
