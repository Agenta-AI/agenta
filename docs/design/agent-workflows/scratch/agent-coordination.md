...
| mcp-mvp-claude (BUT-LOCK) | active | Address Mahmoud's review on #5047 on jp's lane `chore/add-remote-tools-gate` (apply, edit gate predicate + log, test, commit, push, merge into big-agents, but pull). JP OK'd the merge per Mahmoud. Next after: #4985 rebase (own lane), #4912. | `chore/add-remote-tools-gate`; `services/runner/src/engines/sandbox_agent/{run-plan,mcp}.ts` + runner unit tests | 2026-07-04 +15min rolling | Holds BUT-LOCK for the but writes of this sequence. |
