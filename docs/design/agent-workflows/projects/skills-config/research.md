...
2. We need two delivery models, not three, and only one is a new type. An **author-provided
   inline skill** is the `SkillConfig` shape itself (content that rides the wire and is
   materialized into the agent dir). A **referenced skill** reuses the existing `@ag.embed`
   mechanism: an embed object in the `skills` list resolves, server-side, into a `SkillConfig`.
   There is no `source`/`type` discriminator and no separate curated-by-name variant. Platform
   default skills are seeded as locked workflow revisions at project creation and embedded like
   anything else. See the proposal for why the earlier three-variant `source` union collapsed.
