...
First-message delivery derives deterministic session and execution ids from the project and request key. The session input service stores the full invocation payload and fingerprint in the existing inputs table, then binds it to that execution id. The session start service starts the exact created revision only when the durable execution row is absent, and confirms the row before the loader succeeds. Timeout, reconnect, refresh, and two-tab replays return the same input and execution identities. They do not append another greeting or rebuild the message from a changed account list.
