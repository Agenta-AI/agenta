...
4. Preserve the current connection step and its timing. `/m` is the default app on desktop and mobile; its existing connection card collects choices before the server-started first turn. The older web host retains its current pre-create behavior. Valid choices become ordinary configuration or remaining setup context.
...
First-message delivery derives deterministic session and execution ids from the project and request key. The session input service stores the full invocation payload and fingerprint in the existing inputs table, then binds it to that execution id. The session start service commits a one-shot dispatch claim in the input row before invoking the exact created revision. It confirms an execution row or matching stream heartbeat before the loader succeeds. A retry after an ambiguous dispatch only reads back acceptance; it never dispatches again. A crash between claim and dispatch requires operator recovery after verifying the remote run is absent. Timeout, reconnect, refresh, and two-tab replays return the same input and execution identities. They do not append another greeting or rebuild the message from a changed account list.

This is ordinary request deduplication. It does not track whether the agent completed setup. v0.119's queued-input path has idempotency checks, but its idle path returns before a durable input claim. The concrete implementation adds a general claim method to the same owning service and tests both the input fingerprint and execution row. See [the design](../../../../../openspec/changes/load-single-agent-templates/design.md) and [implementation plan](../../../../../docs/superpowers/plans/2026-09-20-load-single-agent-templates.md).

## Resource ownership

The loader resolves source data and calls existing services. The compiler transforms validated data without side effects. Workflow, skill, mount, gateway, and session start services retain authorization and storage ownership. Do not write directly into another domain's tables.
