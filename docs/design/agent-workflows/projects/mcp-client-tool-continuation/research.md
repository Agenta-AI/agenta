...
([../in-sandbox-tool-mcp/](../in-sandbox-tool-mcp/README.md)) narrows that refusal: once its
shim ships, gateway tools reach MCP-client harnesses on Daytona through an in-sandbox stdio
shim and the file relay, and the up-front refusal remains only for client tools and for
non-Daytona remote providers.
