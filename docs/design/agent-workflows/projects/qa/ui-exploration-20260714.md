...
| e. Approvals | PASS w/ BUG | Dock appeared (default policy "Allow reads" gates non-reads). Approve ran the tool; Deny (via a Write) blocked it and the agent acknowledged. BUG: after one Approve, Terminal never asked again — not in new sessions, not after committing Policy=Ask (see bug 1). |
...
1. **HIGH — One Terminal approval becomes a permanent grant; Policy=Ask not enforced for Terminal (Claude Code harness).**
   Repro: new Claude Code agent (local), default policy. Ask for a bash command → dock appears → Approve. Set Advanced > Permissions > Policy = "Ask" ("A human approves every tool call"), save, commit, open a NEW session, ask for another bash command. Expected: approval dock. Actual: Terminal runs immediately (repro'd 3x: same session, new session pre-commit, new session post-commit). Write still asks, and Pi's bash re-asks every time, so this is specific to the Claude harness's Terminal grant persisting at sandbox level. The Approve button says nothing about granting always-allow. Policy label promises "every tool call" and is silently wrong; also Read File never asks under Ask.
