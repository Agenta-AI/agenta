...
| `hosting/**` (compose `.gh.yml` + env examples, all k8s helm/values incl. new `sandbox-agent-{deployment,service}.yaml`, all railway scripts + `sandbox-agent/Dockerfile`) | other | leave (rename lane) | adds the `sandbox-agent` service + `AGENTA_AGENT_RUNNER_URL` contract |
