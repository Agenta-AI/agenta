...
| [Runner to MCP server](cross-service/runner-to-mcp-server.md) | cross-service | `agents/mcp/`, `engines/sandbox_agent/mcp.ts`, `tools/{mcp-bridge,tool-mcp-http,mcp-server,relay}.ts` | evolving (internal gateway-tool channel over loopback HTTP; user http delivered, user stdio disabled) | `services/agent/tests/unit/{mcp-servers,tool-bridge,session-mcp-layering}.test.ts` |
