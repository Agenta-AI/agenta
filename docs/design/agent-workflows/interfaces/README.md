...
| [Runner to tool callback](cross-service/runner-to-tool-callback.md) | cross-service | `tools/{callback,dispatch}.ts`, `apis/fastapi/tools/router.py` (`/tools/call`, `/tools/discover`, `_call_agenta_tool`), `core/tools/{discovery,service}.py`, `agent/tools/resolver.py` | stable (the reserved `tools.agenta.find_capabilities` op is server-wired; SDK declaration pending) | `services/agent/tests/unit/{code-tool,extension-tools}.test.ts`, `api unit/tools/{test_workflow_tool_call,test_discovery}.py` |
