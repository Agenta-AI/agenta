...
| `commit_revision` | `POST /api/workflows/revisions/commit/agent` | mutating (approval) | "Update yourself": binds `workflow_revision.workflow_variant_id` ← `$ctx.workflow.variant.id`, so the agent can only ever commit a revision to its own variant. The `/agent` route is the scoped sibling of the human commit endpoint: it hard-applies the agent scope policy (no writes to `harness.kind`, `harness.permissions`, `runner.permissions`, `sandbox.kind`, `sandbox.permissions`) and refuses full-data commits. Enforcement design: `docs/design/agent-config-editing/contracts/read-config.md` §11.2. |
| `read_config` | `POST /api/workflows/revisions/read-config` | read (auto-allow) | Read the agent's own stored configuration, whole or a named part, so an edit can anchor on real current text. Binds the variant id and draft state from run context. Present in the catalog only when `AGENTA_WORKFLOWS_ORDERED_OPERATIONS_ENABLED` is on. Contract: `docs/design/agent-config-editing/contracts/read-config.md`. |
