...
| `rename_session` | `POST /api/sessions/streams/header?session_id={session_id}&author=auto` | mutating (auto-allow via overlay) | Name and describe the session the agent is running in. The session id is bound from `$ctx.session.id` (runner-filled) and stripped from the model schema, so the agent can only rename its own session. The `name` schema requires a non-whitespace character, so the tool cannot blank a title. `author=auto` is fixed in the catalog path, never in the body, so a model cannot claim a person chose the name: the endpoint answers 409 when this call would replace a name a person typed, and the message carries the current name for the agent to adopt. `override_user_name` in the body is the way past it, for a rename the person asked for. In the default build kit. Design: `docs/design/agent-self-naming-tools/`. |
