...
**Interim user-side workaround (discovered Phase 7, 2026-05-12):** `@vercel/otel`'s `registerOTel()` accepts a `spanProcessors` array that overrides the default `BatchSpanProcessor`. Passing `spanProcessors: [new SimpleSpanProcessor(exporter)]` explicitly switches the flush behaviour and **eliminates the P-APP-VERCEL-01 silent failure** on assertion-2. Empirically verified in Phase 7's Braintrust dual-export wiring (first iteration accidentally used `SimpleSpanProcessor`, assertion-2 went from FAIL to PASS). Trade-off: each ended span triggers a synchronous HTTP round-trip at the exporter — ~50-200ms added per LLM call. Acceptable for low-volume apps; expensive for production chat. This workaround does NOT replace the SDK-side wedge (users still have to know to override the processor, and the latency cost makes it inappropriate at scale), but it gives Vercel-AI-SDK-on-`@vercel/otel` users an immediate mitigation while the SDK ships.
