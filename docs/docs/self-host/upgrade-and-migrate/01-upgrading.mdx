...
For Helm, set `gatewayEgress.insecureAllowed` and `gatewayEgress.insecureHttpAllowed` in your values file. The chart renders each one into the containers that read it. See [Agenta gateways](/self-host/configuration#agenta-gateways).
