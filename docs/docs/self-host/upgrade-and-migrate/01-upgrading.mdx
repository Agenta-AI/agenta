...
If an MCP server in one of your projects is on `http://` or on a private address, add these to your env file:

```bash
# Let the relay call MCP servers on plain HTTP and on private addresses.
# Code default: false. The 0.119.0 env templates ship true.
...
# at a non-loopback address. Code default: false. The 0.119.0 env templates ship true.
...
For Helm, the chart has no dedicated values for these settings. Set them through `api.extraEnv`, `services.extraEnv` and `agentRunner.extraEnv`. See [Agenta gateways](/self-host/configuration#agenta-gateways).

### Saving an MCP server URL follows a different setting in this version

In 0.119.0, the check that runs when someone saves a custom MCP server URL reads the older `AGENTA_INSECURE_EGRESS_ALLOWED` rather than the gateway setting above. The two production templates, `env.oss.gh.example` and `env.ee.gh.example`, set it to `false`, so on such a deployment the save is refused with a `400`:
...
To save a plain-HTTP or private-address MCP server URL on that deployment, set `AGENTA_INSECURE_EGRESS_ALLOWED=true` as well. Doing so also widens what webhooks, workflow hooks, custom model provider endpoints and OIDC issuer probes may reach. A later release makes the save-time check follow the gateway setting, and this step is then no longer needed.
