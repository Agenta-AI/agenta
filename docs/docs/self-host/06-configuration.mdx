...
Agent workflows run through a separate `runner` service. Its full configuration (routing, sandbox
providers, Daytona, warm sessions, and local subscription inputs) lives in the
[runner and sandbox configuration reference](/self-host/agent-execution/runner-configuration).
...
own `AGENTA_RUNNER_DAYTONA_SNAPSHOT`; see the
[runner reference](/self-host/agent-execution/runner-configuration#daytona).
