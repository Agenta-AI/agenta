...
slug: /self-host/infrastructure/architecture
...
Background work runs as two list-parameterized container kinds. Each kind hosts a
family of loops in a single process, so the default deployment is one container of
each rather than seven separate ones. Which loops a container runs is chosen by an
environment selector; an empty selector runs the whole family.

- **`worker-streams`** — Redis Streams consumers, selected by `AGENTA_WORKER_STREAMS`
  (subset of `records`, `events`, `spans`, `sessions`; empty ⇒ all four):
  ...
  - **Channel Replies**: sends agent replies to Slack and Telegram (`streams:sessions`). Run this loop in exactly one replica.
- **`worker-queues`** — TaskIQ queue consumers, selected by `AGENTA_WORKER_QUEUES`
  (subset of `webhooks`, `triggers`, `interactions`, `evaluations`, `channels-inbox`; empty ⇒ all five):
  ...
  - **Channel Messages**: processes incoming Slack and Telegram messages (`queues:channels-inbox`).

Stream and queue names, consumer groups, and message shapes are unchanged by this
grouping, so scaling a container out shares work across its consumer groups.
To scale a single hot loop, run a second instance of the same kind with a disjoint
selector (for example, `AGENTA_WORKER_STREAMS=spans` on its own).
...
See [How agents run](/self-host/agent-execution/how-agents-run) for the runner topology and
[Runner and sandbox configuration](/self-host/configuration#agent-runner) for the
variables.
...
- **Error Handling**: Handles LLM API failures
