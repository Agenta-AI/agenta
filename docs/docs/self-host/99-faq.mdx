---
title: Frequently Asked Questions
sidebar_label: FAQ
description: Self-hosting Agenta FAQ. Learn how to lock Agenta to a specific version, configure Docker images, and troubleshoot common deployment issues.
sidebar_position: 8
---

## How do I lock Agenta to a specific version?

Set the image tag for each service in your Compose env file, for example `hosting/docker-compose/oss/.env.oss.gh`. All five default to `latest`.

```bash
AGENTA_WEB_IMAGE_TAG=v0.105.6
AGENTA_WEB_MOBILE_IMAGE_TAG=v0.105.6
AGENTA_API_IMAGE_TAG=v0.105.6
AGENTA_SERVICES_IMAGE_TAG=v0.105.6
AGENTA_RUNNER_IMAGE_TAG=v0.105.6
```

Use the same version for all five. To pull an image from a different name, set the matching
`AGENTA_<service>_IMAGE_NAME` (for example `AGENTA_RUNNER_IMAGE_NAME`).
