...
:::warning One-time migration from pre-v0.100.2 to v0.100.2
v0.100.2 relocated the Helm chart (from `hosting/helm/agenta-oss/` to `hosting/kubernetes/helm/`), reshaped the `values.yaml` keys, and renamed many env vars. If you are upgrading **across** that boundary, follow [Migrate to v0.100.2 (Helm chart relocation + config reshape)](/self-host/upgrades/v0.100.2-migration) once, then resume using this section for subsequent upgrades.
:::
...
kubectl -n agenta logs job/agenta-agenta-oss-alembic -n agenta
```
