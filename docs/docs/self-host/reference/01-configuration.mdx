...
:::note This variable also gates saving an MCP server URL
In 0.119.0, the check that runs when someone saves a custom MCP server URL reads
`AGENTA_INSECURE_EGRESS_ALLOWED`, not the gateway's own
`AGENTA_GATEWAYS_INSECURE_EGRESS_ALLOWED` below. On a deployment built from
`env.oss.gh.example` or `env.ee.gh.example`, which set it to `false`, a plain-HTTP or
literal private-IP MCP server URL is refused at save time with a `400`, even though the
relay is configured to call it. See
...
| Env var | env.py path | values.yaml path |
|---|---|---|
| `AGENTA_LLM_GATEWAY_ENABLED` | `llm_gateway.enabled` (api) | n/a (`api.extraEnv`) |
| `AGENTA_MCP_GATEWAY_ENABLED` | `mcp_gateway.enabled` (api; the web container reads the same name) | n/a (`api.extraEnv`, `web.extraEnv`) |
| `AGENTA_MCP_GATEWAY_HOST_ALLOWLIST` | `mcp_gateway.host_allowlist` (api) | n/a (`api.extraEnv`) |
| `AGENTA_GATEWAYS_INSECURE_EGRESS_ALLOWED` | `gateway_egress.insecure_allowed` (api) | n/a (`api.extraEnv`) |
| `AGENTA_GATEWAYS_INSECURE_HTTP_ALLOWED` | n/a (read by the services and runner containers, not by the api) | n/a (`services.extraEnv`, `agentRunner.extraEnv`) |
| `AGENTA_GATEWAYS_CREDENTIALS_TTL_SECONDS` | `gateway_credentials.ttl_seconds` (api) | n/a (`api.extraEnv`) |
...
| Env var | Code default | `env.oss.dev.example` / `env.ee.dev.example` | `env.oss.gh.example` / `env.ee.gh.example` |
|---|---|---|---|
| `AGENTA_MCP_GATEWAY_ENABLED` | `true` | `true` | `true` |
| `AGENTA_LLM_GATEWAY_ENABLED` | `false` | `false` | `false` |
| `AGENTA_MCP_GATEWAY_HOST_ALLOWLIST` | empty | commented out | commented out |
| `AGENTA_GATEWAYS_INSECURE_EGRESS_ALLOWED` | `false` | `true` | `true` |
| `AGENTA_GATEWAYS_INSECURE_HTTP_ALLOWED` | `false` | `true` | `true` |
| `AGENTA_GATEWAYS_CREDENTIALS_TTL_SECONDS` | `43200` | unset | unset |

:::note Kubernetes / Helm
The chart has no dedicated values for the gateway settings. `llmGateway.enabled`,
`mcpGateway.enabled`, `mcpGateway.hostAllowlist`, `gatewayEgress.insecureAllowed` and
`gatewayCredentials.ttlSeconds` are not keys the chart reads. The values schema accepts
unknown keys, so setting one of those produces no error and no effect.

Set the env var on the container that reads it instead, through the `extraEnv` list that
every Agenta workload in the chart accepts. Each entry is a Kubernetes env entry, so the
...
`agentRunner` is the values key for the runner workload.
:::
...
This setting governs what the relay may call. It does not govern what the API accepts when
someone saves an MCP server URL. See
[Use an MCP server on a private network or over plain HTTP](/self-host/app-tools-and-triggers#use-an-mcp-server-on-a-private-network-or-over-plain-http).
:::
