...
Services API; in Helm, point `agentRunner.auth.tokenSecretRef` at a Secret holding it — the chart
injects the same value into both the runner and the Services deployment.
