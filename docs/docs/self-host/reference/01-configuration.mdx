...
- Your Daytona API key needs permission to manage Secrets. Without it, every run fails at
  sandbox creation rather than falling back to plaintext keys.
- Keys that a provider SDK signs with locally instead of sending, which today means the AWS access
  keys used for Bedrock, cannot be hidden this way and still reach the sandbox in full. Nothing
  Daytona substitutes on the way out can help there, because the secret never leaves the sandbox
  in the first place.
