...
| `AGENTA_RUNNER_TIMEOUT_SECONDS` | Idle timeout the caller allows on the run request | Services API and SDK (caller-side) | `1920` | no | n/a |
...
| Env var | env.py path | values.yaml path |
|---|---|---|
| `AGENTA_LLM_GATEWAY_ENABLED` | `llm_gateway.enabled` | `llmGateway.enabled` |
| `AGENTA_MCP_GATEWAY_ENABLED` | `mcp_gateway.enabled` | `mcpGateway.enabled` |
| `AGENTA_MCP_GATEWAY_HOST_ALLOWLIST` | `mcp_gateway.host_allowlist` | `mcpGateway.hostAllowlist` |
| `AGENTA_GATEWAYS_INSECURE_EGRESS_ALLOWED` | `gateway_egress.insecure_allowed` | `gatewayEgress.insecureAllowed` |
| `AGENTA_GATEWAYS_INSECURE_HTTP_ALLOWED` | n/a (read directly by the SDK and the runner) | n/a |
| `AGENTA_GATEWAYS_CREDENTIALS_TTL_SECONDS` | `gateway_credentials.ttl_seconds` | `gatewayCredentials.ttlSeconds` |
...
:::note Serving the gateway over plain HTTP
...
to a loopback address, which never leaves the machine. Both defaults are on, and
`AGENTA_GATEWAYS_INSECURE_HTTP_ALLOWED` defaults to `false`.

This matters if you reach your deployment at a plain-HTTP address that is not loopback, such
as `http://192.0.2.10:8680` or `http://agenta.internal`. Agenta refuses to send the
credential, so every gateway-routed run fails with the error code
`gateway_insecure_endpoint` and a message naming this variable.

Two ways to fix it. Put the deployment behind HTTPS, which is the right answer for anything
other people can reach. Or, if it is a single-tenant deployment on a network you control, set
`AGENTA_GATEWAYS_INSECURE_HTTP_ALLOWED=true` and accept that the credential travels in clear
...
The variable is read by the API, the agent service, and the runner, so set it in the env file
all three load and recreate those containers.
:::
