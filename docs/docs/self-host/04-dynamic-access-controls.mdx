...
When `STRIPE_ENABLED=false` and `AGENTA_ACCESS_DEFAULT_PLAN` is unset,
signup onboards new organizations on `self_hosted_enterprise`. This is the
operative plan for almost every self-hosted deployment, and the canonical
shape to use as a starting point for any further customization via
`AGENTA_ACCESS_PLANS`:
...
- **All four flags are `true`.** RBAC, access controls, custom domains, and SSO are all on by default.
...
- `/billing/usage` shows an org rollup — it sums every matching row for today across users, so the card reflects total org-wide daily retrievals. The per-user cap is enforced; the org-wide display is informational.
- The counter is not in `REPORTS` so it is never pushed to Stripe — it is tracked and surfaced internally and to customers but not billed.
...
the wildcard `"*"`. Unknown permissions fail startup. See [Reference: Permission slugs](#reference-permission-slugs)
...
The roles overlay (and the role catalog generally) is **plan-independent**
— roles are not part of the plan map. A roles overlay applies to every
organization regardless of which plan they are on. This is the opposite
of [`AGENTA_ACCESS_DEFAULT_PLAN_OVERLAY`](#agenta_access_default_plan_overlay),
which only patches the resolved default plan.
:::

### Targeting

The overlay accepts **only the `project` scope key today**. Setting `workspace` or `organization` as a
...
```json
{"project": {"editor": {"permissions": ["edit_annotations", "view_evaluation_runs", "read_system", "view_billing"]}}}
```

Add a new `auditor` role to both workspace and project:

```json
{"project": {"auditor": {"description": "Audit-only access.", "permissions": ["read_system"]}}}
```

Rename the `annotator` description without touching its permissions:

```json
{"project": {"annotator": {"description": "Annotates traces for evaluation."}}}
```
...
the slugs in the effective plan map. When unset, falls back to
`self_hosted_enterprise` (the self-hosted default).
...
Bump trace retention to monthly (44640 minutes — one of the canonical
[`Retention`](#reference-retention) enum values):
...
These tables list the canonical slug sets for the enums referenced
throughout the schema. Operator JSON must use exactly these values — any
other slug fails startup with a clear Pydantic error pointing at the
offending field.

### Reference: `Period`

Metering bucket — controls how often the counter resets and which
year/month/day coordinates each meter row lands on. `null` means
non-periodic (used for gauges).

| Slug | Bucket |
|------|--------|
| `"daily"` | Resets per calendar day (`year` + `month` + `day` set on the meter row). |
| `"monthly"` | Resets per calendar month, honoring the subscription's `anchor` day (`year` + `month` set). |
| `"yearly"` | Resets per calendar year (`year` set). |

### Reference: `Scope`

Meter-row granularity — controls which row a `check_entitlements()` call
reads and writes. `null` is treated as `"organization"`.

| Slug | Row identity |
|------|--------------|
| `"organization"` | One row per organization (the historical default). |
| `"workspace"` | One row per `(organization_id, workspace_id)`. |
| `"project"` | One row per `(organization_id, workspace_id, project_id)`. |
| `"user"` | One row per `(organization_id, workspace_id, project_id, user_id)`. |

The DAO writes the full hierarchy needed by the declared scope and leaves
finer dimensions `NULL`. A counter declared with `scope: "user"` therefore
yields multiple rows per period (one per user); `/billing/usage` sums them
for the org-level rollup display while `check_entitlements` enforces the
per-user cap.

### Reference: `Retention`

Retention window in **minutes**. Must be one of the canonical values — an
arbitrary integer is rejected at startup. Used by `traces_ingested` and
`events_ingested`; the per-domain retention flush jobs delete data older
than the cutoff.

| Slug name | Minutes | Approx. window |
|-----------|---------|----------------|
| `EPHEMERAL` | `0` | Instant (no retention). |
| `HOURLY` | `60` | 1 hour. |
| `DAILY` | `1440` | 1 day. |
| `MONTHLY` | `44640` | 31 days. |
| `QUARTERLY` | `131040` | 91 days. |
| `YEARLY` | `525600` | 365 days. |

Operator JSON passes the integer (e.g. `"retention": 44640`), not the slug
name.

### Reference: Permission slugs

The full set of valid permission slugs is defined on the `Permission` enum
in [`api/ee/src/models/shared_models.py`](https://github.com/Agenta-AI/agenta/blob/main/api/ee/src/models/shared_models.py).
That file is the canonical source — refer to it directly when authoring a
custom role.

`"*"` is the wildcard (full access) and is reserved for the platform
`owner` minima role; custom roles should not use it.

If you reference a permission slug that does not exist in the enum,
startup fails with `Unknown permission '<slug>'` so typos surface
immediately.
