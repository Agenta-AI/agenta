...
- `AGENTA_ACCESS_ROLES` — custom roles per scope on top of the platform
  minima (`owner` / `viewer`).
- `AGENTA_ACCESS_ROLES_OVERLAY` — small role-catalog patches that apply to
  both workspace and project scopes.
...
The overlay applies to **both `workspace` and `project` scopes** because
those scopes share the same default role catalog in the code defaults.

Two payload shapes are accepted:

- shortcut form: role slugs at the root, e.g. `{ "auditor": { ... } }`
- scoped form: `{ "project": { "auditor": { ... } } }`

If you use a scope key at the root, only `project` is accepted today.
Setting `workspace` or `organization` as a top-level key fails startup with
a clear error — silent ignore would mislead operators. Do not mix scope keys
and role slugs at the root.
...
| No (new role) | Appended to both workspace and project. `permissions` must be supplied; `description` is optional. |
...
```json
{"project": {"editor": {"permissions": ["edit_annotations", "view_evaluation_runs", "read_system", "view_spans"]}}}
```

Add a new `auditor` role to both workspace and project using the shortcut
form:
...
- mixing scope keys and role slugs at the root → fail
...
yields multiple rows per period (one per user); the usage rollup sums them
