...
:::tip Kubernetes / Helm
If you deployed Agenta using the Helm chart, configuration is done through `values.yaml` rather than environment variables. See the [Deploy on Kubernetes](/self-host/guides/deploy-to-kubernetes#configuration-reference) guide for the full Helm values reference. The environment variables listed below still apply — the Helm chart maps its values to the same variables internally.
:::
