...
### Access controls [ee-only]

Operators can override the built-in plans and role catalogs via JSON
environment variables. Values are parsed once at API
...
fail startup with a clear error. All default to the in-code values when
unset.
...
| `AGENTA_ACCESS_ROLES_OVERLAY` | JSON object: per-role patch applied to both workspace + project scopes. Accepts either role slugs at the root or `{ "project": { ... } }`. Avoids restating the full scope catalog via `AGENTA_ACCESS_ROLES`. | _(no overlay)_ |
| `AGENTA_ACCESS_DEFAULT_PLAN` | Plan slug assigned to new orgs on signup. (Legacy: `AGENTA_DEFAULT_PLAN`.) | `self_hosted_enterprise` |
...
[Dynamic access controls](./04-dynamic-access-controls.mdx) — plans,
entitlements, roles.
