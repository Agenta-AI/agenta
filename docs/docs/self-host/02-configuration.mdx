...
| Env var | env.py path | values.yaml path |
|---|---|---|
| `AGENTA_STORE_ENDPOINT_URL` | `store.endpoint_url` | `mounts.endpointUrl` (auto when `mounts.seaweedfs.enabled`) |
| `AGENTA_STORE_ACCESS_KEY` | `store.access_key` | `mounts.accessKey` |
| `AGENTA_STORE_SECRET_KEY` | `store.secret_key` | `mounts.secretKey` |
| `AGENTA_STORE_REGION` | `store.region` | `mounts.region` |
| `AGENTA_STORE_BUCKET` | `store.bucket` | `mounts.bucket` |
| `AGENTA_STORE_SIGNING_KEY` | n/a (store-side) | `mounts.signingKey` |

:::note SeaweedFS STS signing key
`AGENTA_STORE_SIGNING_KEY` is read by the **store**, not the API: a base64-encoded
