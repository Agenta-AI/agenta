...
| `AGENTA_RUNNER_TOKEN` | Services API, `runner` | `agentRunner.token` |
...
| `DAYTONA_SNAPSHOT_AGENT` | `runner` | n/a (compose-only override of `DAYTONA_SNAPSHOT`; Helm scopes per pod) |
| `DAYTONA_IMAGE` | `runner` | `agentRunner.daytona.image` |
| `DAYTONA_AUTOSTOP` | `runner` | `agentRunner.daytona.autostopDelay` |
| `AGENTA_RUNNER_DAYTONA_SESSION_IDLE_TTL_MS` | `runner` | `agentRunner.daytona.sessionIdleTtlMs` |
| `AGENTA_RUNNER_DAYTONA_SESSION_MAX_WARM` | `runner` | `agentRunner.daytona.sessionMaxWarm` |
| `DAYTONA_AUTODELETE` | `runner` | `agentRunner.daytona.autodeleteDelay` |
...
:::warning Runner authentication
`AGENTA_RUNNER_TOKEN` is the runner's only authentication. It is **unset by default, which
means no authentication check at all**: the `runner` service accepts any request that can
reach it, and its `/run` body carries plaintext provider keys. Zero-config self-hosting
relies on network isolation alone — keep the runner on a private network and never publish
its port.

Set it to require a shared bearer token on the runner's `/run` and `/kill` endpoints;
requests without a matching `Authorization: Bearer <token>` (or `X-Agenta-Runner-Token`)
header are refused with an HTTP 401. Set the **same value** on the runner and on the
Services API that calls it, or the Services API locks itself out. In Helm, `agentRunner.token`
does both: it stores the value in the release Secret and injects it into both pods. A static
shared secret is defense in depth on top of network isolation, not a substitute for it.
:::
...
[snapshot recipe](/self-host/guides/agent-daytona-sandboxes#optional-snapshot) equips one
