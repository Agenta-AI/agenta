...
| `AGENTA_AGENT_MCP_SERVERS_ENABLED` | Services API | `agentRunner.enableMcp` |
| `SANDBOX_AGENT_PROVIDER` | `sandbox-agent` | `agentRunner.provider` |
| `SANDBOX_AGENT_LOG_LEVEL` | `sandbox-agent` | `agentRunner.logLevel` |
| `DAYTONA_API_KEY` | `sandbox-agent` | `agentRunner.daytona.apiKey` |
| `DAYTONA_API_URL` | `sandbox-agent` | `agentRunner.daytona.apiUrl` |
| `DAYTONA_TARGET` | `sandbox-agent` | `agentRunner.daytona.target` |
| `DAYTONA_SNAPSHOT` | `sandbox-agent` | `agentRunner.daytona.snapshot` |
| `DAYTONA_IMAGE` | `sandbox-agent` | `agentRunner.daytona.image` |
| `DAYTONA_AUTOSTOP` | `sandbox-agent` | `agentRunner.daytona.autostopDelay` |
| `AGENTA_AGENT_SANDBOX_PI_INSTALLED` | `sandbox-agent` | `agentRunner.daytona.installPi` |
...
## Mounts (durable object store)

Backs the durable working directory for sessions and agents — files written by an agent run
survive sandbox teardown and reappear in the next turn. Disabled until `ACCESS_KEY` and
`SECRET_KEY` are set. Point these at any S3-compatible store (AWS S3, Cloudflare R2, MinIO, or
the bundled SeaweedFS); an empty endpoint means real AWS S3.

The API holds the master credentials and **never** passes them to the runner: at run start it
mints short-lived, prefix-scoped credentials (S3 STS `GetFederationToken`) and only those reach
the sandbox, which geesefs-mounts the durable prefix as its cwd.

| Env var | env.py path | values.yaml path |
|---|---|---|
| `AGENTA_MOUNTS_STORAGE_ENDPOINT_URL` | `mounts.storage_endpoint_url` | `mounts.endpointUrl` (auto when `mounts.seaweedfs.enabled`) |
| `AGENTA_MOUNTS_STORAGE_ACCESS_KEY` | `mounts.storage_access_key` | `mounts.accessKey` |
| `AGENTA_MOUNTS_STORAGE_SECRET_KEY` | `mounts.storage_secret_key` | `mounts.secretKey` |
| `AGENTA_MOUNTS_STORAGE_REGION` | `mounts.storage_region` | `mounts.region` |
| `AGENTA_MOUNTS_STORAGE_BUCKET` | `mounts.storage_bucket` | `mounts.bucket` |
| `AGENTA_MOUNTS_STORAGE_SIGNING_KEY` | n/a (store-side) | `mounts.signingKey` |

:::note SeaweedFS STS signing key
`AGENTA_MOUNTS_STORAGE_SIGNING_KEY` is read by the **store**, not the API: a base64-encoded
32-byte key that the bundled SeaweedFS uses to serve S3 STS in-process (so the API can mint the
scoped runner credentials on the same S3 port — no separate IAM service). Real AWS S3 / R2 serve
STS natively and ignore it. The bundled store and the API must agree on `ACCESS_KEY` / `SECRET_KEY`;
in the Helm chart and docker-compose this is wired automatically.
:::

:::warning Remote sandboxes (Daytona/E2B) need a reachable store
A local sandbox geesefs-mounts on the runner host against the in-network store. A **remote**
sandbox runs geesefs inside the cloud sandbox, so the store endpoint must be publicly reachable —
enable the `ngrok` tunnel (the compose `remote` profile) or point the mounts endpoint at a public
S3/R2 URL. Only the scoped, short-lived credentials ever cross into a remote sandbox.
:::
