...
| `SMTP_PASSWORD` | SMTP authentication password | Uses `SENDGRID_API_KEY` if not set |
| `SMTP_USE_TLS` | Enable TLS encryption for SMTP | `true` |

:::info
Note that if you do not set the SMTP configuration, the system will show you a URL with the invitation link instead of sending an email.
:::
...
| `SENDGRID_API_KEY` | `SMTP_*` | Swapped SendGrid by direct SMTP configuration |
