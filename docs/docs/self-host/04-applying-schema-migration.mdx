...
- `-w /app/oss/databases/postgres/migrations/core`: Sets the working directory inside the container to the migrations directory.
- `<api-container-name>`: The name of the Docker container running the backend.
- `alembic -c alembic.ini upgrade head`: Applies all new migrations up to the latest available version (`head`), using Alembic configuration file `alembic.ini`.
...
In the event that you encounter issues and need to revert the schema migration, you can revert by running `alembic -c alembic.ini downgrade head`. Afterwards, create a GitHub issue describing the problem you encountered.
