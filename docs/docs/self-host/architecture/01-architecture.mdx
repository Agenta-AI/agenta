...
- **`worker-streams`** — Redis Streams consumers, selected by `AGENTA_WORKER_STREAMS`
  (subset of `records`, `events`, `spans`; empty ⇒ all three):
...
- **`worker-queues`** — TaskIQ queue consumers, selected by `AGENTA_WORKER_QUEUES`
  (subset of `webhooks`, `triggers`, `interactions`, `evaluations`; empty ⇒ all four):
...
[Runner and sandbox configuration](/self-host/agent-execution/runner-configuration) for the
variables.
