...
The `store.seaweedfs.enabled` Helm toggle controls whether the chart bundles a SeaweedFS StatefulSet or points `store.endpointUrl` at an external store. This mirrors the `postgresql.enabled` pattern. An empty endpoint URL means real AWS S3.
