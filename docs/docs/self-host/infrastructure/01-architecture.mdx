...
- **`worker-streams`** — Redis Streams consumers, selected by `AGENTA_WORKER_STREAMS`
  (subset of `records`, `events`, `tracing`; empty ⇒ all three):
  - **Tracing Ingestion**: consumes the OTLP tracing pipeline (`streams:tracing`).
...
selector (for example, `AGENTA_WORKER_STREAMS=tracing` on its own).
