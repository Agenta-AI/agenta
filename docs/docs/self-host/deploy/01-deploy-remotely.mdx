...
:::note Retry semantics behind a gateway
A gateway in front of the workflow service can change which failures a retry follows. See [Retry semantics and your gateway](/self-host/infrastructure/networking).
:::
...
**Cause:** The trace endpoint on the run does not match the runner's configured `AGENTA_API_URL`.
Each run carries the endpoint the API built from its public base, for example
`https://agenta.example.com/api/otlp/v1/traces`, while the runner may know only its internal hop,
`http://api:8000`, or a different public base. A strict credential check then withholds the run's
credential, and the API rejects every callback. Conversation history is never written or read back.

When `AGENTA_API_URL` is unset, the runner keeps the credential and logs a warning that names the
missing variable. On a runner with this fallback,
`cred=DROPPED(endpoint-not-agenta-ingest)` means `AGENTA_API_URL` is set but does not match the
run's endpoint. Set it correctly in either case so the runner can attribute the credential.
