...
slug: /self-host/guides/deploy-on-railway
...
cd agenta
git sparse-checkout set hosting/railway
```
...
export AGENTA_WEB_MOBILE_IMAGE="ghcr.io/agenta-ai/agenta-web-mobile:v0.30.0"
export AGENTA_SERVICES_IMAGE="ghcr.io/agenta-ai/agenta-services:v0.30.0"
export AGENTA_RUNNER_IMAGE="ghcr.io/agenta-ai/agenta-runner:v0.30.0"
```
...
The script re-configures environment variables, runs any new migrations, and redeploys all services. If it stops with `Service 'web-mobile' is missing`, the environment was bootstrapped before the mobile app became standard. Re-run `./hosting/railway/oss/scripts/bootstrap.sh`, then the deploy script.
...
| Variable | Default | Description |
|---|---|---|
| `AGENTA_RUNNER_IMAGE` | `ghcr.io/agenta-ai/agenta-runner:latest` | Image used by the `runner` service |
| `AGENTA_RUNNER_ENABLED_SANDBOX_PROVIDERS` | `local` | Sandbox providers this deployment can use |
| `AGENTA_RUNNER_DEFAULT_SANDBOX_PROVIDER` | `local` | Default sandbox provider for agent runs |
| `AGENTA_RUNNER_DAYTONA_API_KEY` | (none) | Daytona provisioning credential |
