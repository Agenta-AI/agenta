...
In the event that you encounter issues and need to revert the schema migration, you can do so by running the same command above, but changing `upgrade` to `downgrade`. Afterwards, create a GitHub issue describing the problem you encountered.
