...
available to Daytona runs, copy them into the snapshot with a `COPY` or a download step in the same
`dockerfile_commands` list, then rebuild.
...
4. Point the `runner` service at your image with a Compose override file
   ...
   For Helm, set the runner image under `agentRunner` in your values file.
...
when they should only read it. For Helm, add the volume to the runner pod in your values file.
...
For Helm, set the runner resource requests and limits under `agentRunner` in your values file.
