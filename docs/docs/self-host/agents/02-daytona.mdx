...
the [runner reference](/self-host/configuration#agent-runner).
...
DAYTONA_API_KEY=<daytona-api-key> DAYTONA_TARGET=eu uv run build_snapshot.py
```
...
If you already built a snapshot from an earlier release, build the new recipe under a new name
after upgrading, then point the runner at that name (next step):

```bash
uv run build_snapshot.py --name agenta-agent-sandbox-v1-20260922
```

Daytona keeps the snapshot you built, so a snapshot built from an older recipe keeps its older
toolset until the runner moves to a new one. The script never deletes a working snapshot, because
your runner may be starting sandboxes from it; `--force` only replaces a snapshot whose build
failed. Delete the old snapshot in Daytona once no runner points at it.

Point the runner at the snapshot you built. On a first install that is `agenta-agent-sandbox-v1`;
after a refresh, use the new name:
...
[warm sessions and scale](/self-host/configuration#warm-sessions-and-scale).
