...
keeps its older toolset until you rebuild it. `--force` first builds a trial copy under a
temporary name, so a recipe that fails its checks leaves your working snapshot untouched. New
sandboxes cannot start while the real snapshot is rebuilt, so run it in a quiet window. If that
last step fails, the script prints the setting that points the runner at the trial copy.
