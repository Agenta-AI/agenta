...
- **Worker (records)** for session record persistence from the `streams:records` stream
- **Worker (interactions)** for async session interaction dispatch from `queues:interactions`
- **Worker (triggers)** for automated workflow trigger processing
...
Each component (`api`, `web`, `services`, `workerEvaluations`, `workerTracing`, `workerWebhooks`, `workerEvents`, `workerRecords`, `workerTriggers`, `workerInteractions`, `cron`, `supertokens`) supports:
