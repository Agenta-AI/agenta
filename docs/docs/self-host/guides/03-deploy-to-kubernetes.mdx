...
| `global.extraEnv` | Extra non-secret env vars injected into all Agenta application pods | `{}` |
...
| `secrets.extraEnv` | Extra secret env vars injected into all Agenta application pods | `{}` |
...
### Shared self-host env buckets

Use the shared env buckets when you want a value to be available across Agenta application pods:

- `global.extraEnv` for non-secret values
- `secrets.extraEnv` for secret values

This is the recommended place for optional self-hosting configuration such as:
...
global:
  extraEnv:
    AGENTA_ALLOWED_DOMAINS: "example.com"
    AGENTA_BLOCKED_DOMAINS: "mailinator.com"
    AGENTA_BLOCKED_EMAILS: "blocked@example.com"
    AGENTA_ORG_CREATION_ALLOWLIST: "admin@example.com"
...
`<component>.env` is still available for component-specific overrides. Use the shared buckets only for values that should be present across the app.
