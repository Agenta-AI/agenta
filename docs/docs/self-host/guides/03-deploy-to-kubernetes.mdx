...
The chart supports both OSS and Enterprise Edition. Set `global.agentaLicense` to `oss` or `ee`.
...
To deploy Enterprise Edition, set `global.agentaLicense: ee`.

Enterprise images are private packages in GHCR. Create the namespace and image pull secret first:
...
Then install with the EE example file:

```bash
helm install agenta hosting/helm/agenta-oss \
  --namespace agenta \
  -f hosting/helm/agenta-oss/values-ee.example.yaml
```
