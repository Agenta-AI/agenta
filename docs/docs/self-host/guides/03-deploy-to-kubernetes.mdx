...
### Typed self-host configuration

The chart now exposes first-class values for the most common self-hosted configuration instead of relying on one shared env bucket for everything.
...
accessControl:
  ...
  blockedDomains: "mailinator.com"
  blockedEmails: "blocked@example.com"
  orgCreationAllowlist: "admin@example.com"
```

#### SendGrid (backend secrets; web receives only a derived public flag)
...
#### Composio / tools (backend secrets; web receives only a derived public flag)
...
Use these only for uncommon backend-only values that do not yet have first-class chart fields:

```yaml
backend:
  extraEnv:
    AGENTA_DEFAULT_PLAN: "pro"
  secretEnv:
    MY_PRIVATE_BACKEND_TOKEN: "token"
```

`<component>.env` is still available for component-specific overrides.

#### Notes on web pod scoping

- OAuth provider secrets are still injected into the web pod today because the web entrypoint currently checks both client IDs and client secrets to derive frontend auth provider flags. This should be cleaned up later.
- SendGrid, Composio, Stripe, and Turnstile are handled differently: the web pod receives only explicit public/derived flags, while backend secrets stay backend-only.

For self-hosted EE, keep these unset by default unless you explicitly need them:
