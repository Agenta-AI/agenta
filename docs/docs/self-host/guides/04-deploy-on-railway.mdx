...
- **Worker (webhooks)** for async webhook deliveries
...
export AGENTA_AGENT_RUNNER_IMAGE="ghcr.io/agenta-ai/agenta-agent-runner:latest"
```
...
export AGENTA_AGENT_RUNNER_IMAGE="ghcr.io/agenta-ai/agenta-agent-runner:v0.30.0"
```
...
| Variable | Default | Description |
|---|---|---|
| `AGENTA_AGENT_RUNNER_IMAGE` | `ghcr.io/agenta-ai/agenta-agent-runner:latest` | Image used by the `sandbox-agent` runner service |
| `SANDBOX_AGENT_PROVIDER` | `local` | Default sandbox provider for agent runs |
| `DAYTONA_API_KEY` | (none) | Daytona key for agent-runner sandboxes |
