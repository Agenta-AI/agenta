...
You can also describe the change in chat instead of opening the editor. While `Playground build kit` is on in the **Advanced** dialog (it is on by default), the agent can revise its own configuration as you talk to it, including its instructions.

The build kit itself does not ship with the agent. Everything the build kit adds is removed on commit.
