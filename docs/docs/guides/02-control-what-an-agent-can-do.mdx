...
`Permission` is on connected-app tools (listed under `CONNECTED APPS`) and on tools you write with `Tool definition`.

Built-in tools have no per-tool permission. Clicking one opens a read-only view with the subtitle `Provider built-in tool`, and the agent's `Policy` governs it instead.
...
## Settle the permissions before the agent runs unattended
