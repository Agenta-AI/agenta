...
**The tool list could not be read.** The permission drawer says so where the tools would be, and shows the server's own wording for the refusal when it gave one. Click `Retry tools` to read it again. Your credentials were saved, so there is nothing to authorize. If the drawer says instead that the server is not connected yet, it offers `Connect` rather than a retry, because a tool list cannot be read until the connection is authorized.
