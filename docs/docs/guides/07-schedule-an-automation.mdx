...
description: "Run an agent on a recurring schedule: set the cadence and times in UTC, pin the version that runs, and write the message the agent receives on each run."
---
...
A schedule runs an agent on a clock, with nobody in the chat. This page sets one up. Prepare the agent first with [Create an automation](/guides/create-an-automation), because a scheduled run cannot ask you anything.

{/* VIDEO: filling in the New schedule dialog for a daily digest, checking the next fire time, and switching it on. */}
...
<Image
  ...
  alt="The New schedule dialog with its Name, When should it run, Which version runs, and What should the agent do sections"
  loading="lazy"
/>
