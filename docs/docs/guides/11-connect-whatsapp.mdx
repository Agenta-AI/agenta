...
Agenta checks the token against the phone number before it saves anything. If the token is wrong or cannot read that number, the connection fails and nothing is stored.
...
If the agent's reply is ready more than 24 hours after the customer's last message, Agenta does not send it. It holds the reply and shows it as held in the session. When the customer writes again, Agenta sends the held reply.
