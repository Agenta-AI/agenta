...
Domain verification establishes your organization's authority over an email domain (e.g., `acme.com`). Once verified, you can automatically add users with matching email domains and restrict access to verified domains only.
...
✗ Does NOT grant authentication (users still prove identity)
...
Subdomain verification is not currently supported. Verify the parent domain instead (e.g., verify `acme.com` to cover all `@acme.com` emails).
