...
:::info[Availability]
RBAC is enabled in every edition of Agenta, including the open source edition, where it is always enforced. On Agenta Cloud and Enterprise, enforcement follows the plan: legacy or non-RBAC plans grant full access to all members regardless of role.
:::
