...
{/* VIDEO: a marketing coworker being built in chat, corrected once after a bad draft, then put on a weekly schedule that writes a content plan with nobody in the chat. */}
...
An agent's instructions are its identity. They live in a file called `AGENTS.md` and they are sent at the start of every session, so whatever is in them is guaranteed to be there every time the agent thinks. See [Agents](/concepts/agents) for the full picture.
