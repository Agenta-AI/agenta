...
If a prompt has no fallback models, `fallback_llm_configs` can be empty, `null`, or missing. If no fallback policy is set, fallback behavior is off. If no retry policy is set, Agenta SDK runtimes use their built-in retry defaults.
