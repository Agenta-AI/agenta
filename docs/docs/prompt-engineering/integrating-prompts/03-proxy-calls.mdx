...
response = requests.post(
    ...
    headers={
        ...
        "Authorization": f"ApiKey {os.environ['AGENTA_API_KEY']}",
        # highlight-next-line
        "Baggage": f"ag.session.id={session_id},ag.session.type=chat,ag.actor.id={user_id},ag.actor.type=user"
    },
    json={
...
All requests with the same session ID will be grouped together in the observability dashboard. You can also track actors (users) to see who is interacting with your application. 