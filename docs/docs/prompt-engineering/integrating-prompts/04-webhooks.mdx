...
Revision events carry `references` (the artifact, variant, and revision identifiers — partial when the response does not expose all fields), `count`, and `user_id`. Commit events also carry optional `message`. The `environments.revisions.committed` event additionally carries `state` and `diff` describing the committed configuration, as shown in the examples above.
