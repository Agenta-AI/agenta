...
To create a new application, use the `AppManager.create` method.
...
This command will create a new application and set its type

- **Parameters:**
  - `app_slug`: The slug of your application.
  - `template_key`: The type of the applicaton.
    - `SERVICE:chat`: for managed `chat` applications.
    - `SERVICE:completion`: for managed `completion` applications.

**Note:** If an application with the same `slug` already exists, the SDK will raise an exception.
...
To rename an application, use the `AppManager.update` method.

```python

app = ag.AppManager.rename(
    app_id="my-app-id",
    app_slug="my-new-app-slug",
)

print("Renamed application:")
print(app)
```
...
To delete an application, use the `AppManager.delete` method.

```python

ag.AppManager.delete(
    app_id="my-app-id",
)
```
