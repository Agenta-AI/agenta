...
| Pi | Models from several providers, including OpenAI, Anthropic, Google Gemini, Mistral, Groq, MiniMax, Together AI, OpenRouter, and xAI, plus any OpenAI-compatible endpoint. |
...
All three also accept the [MCP servers](/guides/add-an-mcp-server) you connect to a project, so the tools an agent gets from a server do not change when you switch harness. One exception: a server that sends its tool list in pages gives a Codex agent only the first page, because Codex does not ask for the rest. Pi and Claude Code do.
...
Agenta works with models from OpenAI, Anthropic, Google Gemini, Azure, AWS Bedrock, Mistral AI, Cohere, Groq, Together AI, DeepInfra, Perplexity AI, Anyscale, OpenRouter, MiniMax, and xAI. It also works with any OpenAI-compatible endpoint, and with models you host yourself through Ollama.
