...
2. **Understand the Project:** Familiarize yourself with our architecture and concepts by reading our [Guide](/concepts/concepts).
...
1. Start the agenta platform locally by running `docker compose -f "docker-compose.yml" up -d --build`
...
2. Start the agenta platform locally by running `docker compose -f "docker-compose.yml" up -d --build`
3. Update the `.env.local` file with your OpenAI API Key:
   To use your OpenAI API Key in the tests, you need to update the `.env.local` file with the key. Follow these steps:

- Open the `.env.local` file located at `/agenta/agenta-web/.env.local` in your code editor
- Add your OpenAI API Key using the variable `NEXT_PUBLIC_OPENAI_API_KEY`
- Save the `.env.local` file after adding the key

4. Navigate to the frontend directory of the project `cd agenta-web`
5. Run `npm run test` to run the frontend tests
