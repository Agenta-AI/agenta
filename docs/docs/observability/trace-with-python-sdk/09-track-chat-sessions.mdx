...
def process_message(message: str, session_id: str):
    # Set session ID and optionally session type
...
You can also set actor (user) information to track who is interacting:

```python
@ag.instrument()
def process_message(message: str, session_id: str, user_id: str):
    # Set session information
    ag.tracing.store_session(session_id=session_id, session_type="chat")
