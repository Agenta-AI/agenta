...
Invoking an archived application, evaluator or workflow fails with status `410`, and `status.type` ends in `#v0:references:archived`. This applies whether the invoke names the target directly or through an environment. Unarchive the target to invoke it again.
