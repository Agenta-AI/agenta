...
import FindSlugsInUI from "@site/docs/_partials/find-slugs-in-ui.mdx";
...
| prompt.llm_config.reasoning_effort | string or null | Controls reasoning effort for thinking models. Options: `none` (cost-optimized, 0 tokens), `low` (1024 tokens), `medium` (2048 tokens), `high` (4096 tokens) |
...
| prompt.fallback_llm_configs | array or null | Ordered backup model configurations |
| prompt.fallback_llm_configs[].model | string | Required model identifier for each fallback model |
| prompt.retry_policy | object or null | Retry settings applied to each attempted model |
| prompt.retry_policy.max_retries | integer | Number of extra attempts after the first failed attempt |
| prompt.retry_policy.delay_ms | integer | Wait time between retry attempts, in milliseconds |
| prompt.fallback_policy | string or null | Error policy for moving to a fallback model. Options: `off`, `availability`, `capacity`, `access`, or `any` |
