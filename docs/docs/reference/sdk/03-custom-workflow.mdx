...
PromptTemplate(
    ...
    fallback_llm_configs=[FallbackModelConfig(model="...")],
...
- `fallback_llm_configs` (List[FallbackModelConfig]): Optional ordered backup model settings
- `retry_policy` (RetryPolicy): Optional retry settings for each attempted model
