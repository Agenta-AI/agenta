...
A retry with the same key and the same answer is safe. A different answer to an interaction that was already answered, or to one that is no longer pending, returns `409`.
