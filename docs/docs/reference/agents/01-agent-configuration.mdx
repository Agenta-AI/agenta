...
Setting either `tool_permissions` or `new_tool_permission` opts this server into per-tool policy. An advertised tool then resolves to its `tool_permissions` entry, and otherwise to `new_tool_permission`, which itself falls back to `permission` and then to `ask`. `runner.permissions.default` is not consulted for a server that opted in, so a run default cannot widen the table. With both fields absent, the entry carries no per-tool policy and `permission` governs the whole server.
