...
The permission editor writes `new_tool_permission` for you where the server is set to `allow`: giving one tool a rule of its own also records `allow` as the default for the tools you have not named, so those tools keep running as they did. Set the field yourself to choose a narrower default.
