...
A successful response is `202 Accepted`. The run continues in the background, in a new execution of the same session. The response below is trimmed; `interaction` carries the full interaction record.
...
`execution.state` is `awaiting_interactions`, and `command` is `null`, when the paused execution still has other pending interactions. The run continues once all of them are answered.
...
An interaction has a `kind` of `user_approval`, `user_input`, or `client_tool`, and a `status` of `pending`, `responded`, `resolved`, or `cancelled`. Sending the same answer again returns the accepted result. Answering an interaction that was answered differently, or that is no longer `pending`, returns `409`.
