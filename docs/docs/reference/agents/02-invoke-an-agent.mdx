...
The `Idempotency-Key` header is a string of at most 255 characters. A retry with the same key and the same answer returns the original result, so a client can retry safely after a network error. Reusing a key for a different answer returns `409` with the code `idempotency_key_reused`.
...
An interaction has a `kind` of `user_approval`, `user_input`, or `client_tool`, and a `status` of `pending`, `responded`, `resolved`, or `cancelled`. To retry a request, reuse its `Idempotency-Key`. Answering an interaction that was already answered differently, or that is no longer `pending`, returns `409`.
