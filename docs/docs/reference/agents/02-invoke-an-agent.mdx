...
Reuse the same `session_id` across turns and send the conversation so far in `data.inputs.messages`. See [Sessions and versions](/concepts/sessions-and-versions).
