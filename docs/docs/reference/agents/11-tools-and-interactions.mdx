...
The endpoint answers `202 Accepted` and the run continues. A retry with the same key and the same answer is safe; it returns the original result. The full request and response shape is in [Invoke an agent](/reference/agents/invoke-an-agent#continuing-a-conversation).
