...
When called, the turn pauses with a `client_tool` interaction. Post the result to `POST /api/sessions/interactions/{interaction_id}/respond` with an `Idempotency-Key` header. A request without the header returns `422`.
...
curl -X POST \
  ...
  -H "Idempotency-Key: call_019d952f" \
  -d '{
    "answer": {
...
The endpoint answers `202 Accepted`. The run continues once every pending interaction of the paused execution has an answer. A retry with the same key and the same answer is safe: it does not answer twice, and it returns `202` with the current state. The full request and response shape is in [Invoke an agent](/reference/agents/invoke-an-agent#continuing-a-conversation).
