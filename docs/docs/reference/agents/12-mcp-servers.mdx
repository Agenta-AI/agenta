...
`policy.tools` decides which of the server's tools are advertised to the model at all. A tool the filter hides is never offered and never called, so it needs no permission. This is not the box that filters a long tool list in the connection view, which only narrows what you are looking at and stores nothing.
