...
- An authorization failure on an OAuth server means the grant was revoked or has expired. The connection then reads `Login expired` in `Settings > MCPs`, and `Reconnect` on its row menu restores it.
