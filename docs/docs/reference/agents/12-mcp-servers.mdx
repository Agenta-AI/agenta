...
:::info MCP release work
PR #6049 is preparing project connections with independent accounts at the same server and per-tool permission controls. These are target behaviors pending implementation and release verification. Existing configuration fields below remain baseline documentation; the release must update them to match tested behavior.
:::
