...
| Rename                        | `POST /api/sessions/streams/header?session_id={session_id}`          |
| Archive or unarchive          | `POST /api/sessions/archive?session_id={session_id}` or `/unarchive` |
| Cancel active work            | `POST /api/sessions/{session_id}/cancel`                             |
| Delete                        | `DELETE /api/sessions/?session_id={session_id}`                      |
| Query interactions            | `POST /api/sessions/interactions/query`                              |
| Edit or remove a queued input | `PATCH` or `DELETE /api/sessions/{session_id}/inputs/{input_id}`     |

The session snapshot from `GET /api/sessions/{session_id}` includes the inputs queued for that session.
