...
The `evaluator_slug` can be any string; the annotation call accepts unknown slugs. To make the evaluator show up in the UI with aggregated stats, create it explicitly through `POST /api/simple/evaluators/` or from the Agenta UI.
