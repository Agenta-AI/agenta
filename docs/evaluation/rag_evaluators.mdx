...
description: 'Internal variables and inline traces in RAG applications'
---

## 
On this page, you'll learn how to configure and use RAG evaluators for your custom-built application. RAG evaluators are unique because they need intermediate outputs and internal variables (such as context) to run.

By the end of this tutorial, you'll know how to write a RAG application that saves the required internal variables and how to configure the evaluator in Agenta to use these in evaluation.

<Info>Access to `internals` and `inline traces` requires version 0.20.0+ of the Python SDK.</Info>
<Info>Access to `RAG evaluators` and `view traces` in the Web UI requires the cloud version of Agenta.</Info>
...
On top of performing semantic search on the formatted prompt fetched from Agenta, the retriever stores the formatted prompt into `internals`.
```python
...
We can also see the list of all `inputs`, `internals`, and `outputs` for each node of the tree, or stage of the workflow.
...
rag.retriever.internals.prompt
rag.retriever.internals.topk
...
Now that we have a RAG application, and that we know how to access all the three types of variables, `inputs`, `internals`, and `outputs`, we can jump to RAG evaluators.
