...
:::tip
For a complete guide on managing test sets with the SDK (including listing, retrieving, and updating), see [Managing Testsets](/1.0/evaluation/evaluation-from-sdk/managing-testsets).
:::
...
```bash
curl -X POST "https://cloud.agenta.ai/api/simple/testsets/" \
...
- [Create test sets from UI](/1.0/evaluation/managing-test-sets/create-from-ui) for manual creation
- [Create test sets from traces](/1.0/evaluation/managing-test-sets/create-from-traces) to capture production data
- [Create test sets from playground](/1.0/evaluation/managing-test-sets/create-from-playground) during experimentation
