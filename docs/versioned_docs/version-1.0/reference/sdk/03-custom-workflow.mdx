...
`chat_template_kwargs` is an optional dictionary for provider-specific chat template options. Agenta passes it through unchanged and does not apply prompt variable formatting to its values. When it is `None`, it is omitted from provider call arguments.
