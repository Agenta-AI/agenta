...
- A **deployment** that we can either [directly invoke](/1.0/prompt-engineering/integrating-prompts/proxy-calls) **or** [fetch the configuration](/1.0/reference/sdk/configuration-management#get_from_registry) to run elsewhere
...
- `DOCS_PATH`: The folder containing the documentation (in our case it's under `/docs`).
- `DOCS_BASE_URL`: The base URL where the documentation can be found (in our case it's `https://agenta.ai/`).
...
For the first, we use the RAG Relevancy evaluator as described in [Agenta's evaluation documentation](/1.0/evaluation/configure-evaluators/rag-evaluators).
...
We set and test an LLM-as-a-Judge evaluator to rate the quality of the assistant's responses the same way. More details on setting up LLM-as-a-Judge evaluators can be found [here](/1.0/evaluation/configure-evaluators/llm-as-a-judge).
...
Agenta provides us with [two endpoints](/1.0/prompt-engineering/integrating-prompts/integrating-with-agenta) to interact with our deployed application:
