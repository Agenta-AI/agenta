---
title: "Capture User Feedback with Trace Annotations"
position: 1
...
<GoogleColabButton notebookPath="examples/jupyter/observability/capture_user_feedback.ipynb">
...
The `evaluator_slug` can be any string. Agenta creates the evaluator on the first annotation and infers its output schema from your data, so it shows up in the UI with aggregated stats. To control the schema (fixed labels, required fields, or value ranges), create the evaluator first through `POST /api/simple/evaluators/` or from the Agenta UI.
...
def annotate(trace_id, span_id, annotation, evaluator_slug):
    ...
    # Annotations are stored as traces. The link back to the instrumented
    # span is expressed via `links.invocation`.
    trace_payload = {
        "trace": {
            "data": {"outputs": annotation},  # Your feedback data goes here
            "references": {"evaluator": {"slug": evaluator_slug}},  # Evaluator reference
            "links": {"invocation": {"trace_id": trace_id, "span_id": span_id}},  # Link to the instrumented span
        }
    }

    # Send the annotation to the Agenta API
    response = requests.post(
        f"{os.environ.get('AGENTA_HOST', 'https://cloud.agenta.ai')}/api/simple/traces/",
        headers=headers,
        json=trace_payload,
    )
