...
:::info
If you want your application to fetch the latest prompt on demand, use [Fetch Prompts via SDK/API](/1.0/prompt-engineering/integrating-prompts/fetch-prompt-programatically).
:::
...
`references` identifies the environment revision that was just committed, plus the environment and variant it belongs to. Each one includes an `id` and, when available, a readable `slug`.
...
Revision read events (`retrieved`, `fetched`, `queried`, and `logged`) carry `references` (the artifact, variant, and revision — each with an `id` and, when available, a `slug`), `count`, and `user_id`. Commit events carry `references`, `user_id`, and optional `message`, but do not include `count`. The `environments.revisions.committed` event additionally carries `state` and `diff` describing the committed configuration, as shown in the examples above.
