...
**Internals**: User-provided internal information such as context variables, intermediate calculations, or evaluation data that aren't part of the primary inputs/outputs. These are set by the user [in the SDK using](/1.0/observability/trace-with-python-sdk/adding-metadata) `ag.tracing.store_internals()`.
...
The decorator automatically captures function inputs and outputs in `ag.data.inputs` and `ag.data.outputs` unless you choose to [mask sensitive data](/1.0/observability/trace-with-python-sdk/redact-sensitive-data).
...
- Learn about [distributed tracing](/1.0/observability/trace-with-opentelemetry/distributed-tracing)
- Explore [Python SDK tracing](/1.0/observability/trace-with-python-sdk/setup-tracing) for easier instrumentation
- See [integration guides](/1.0/integrations/llm-providers/openai/observability) for specific frameworks
