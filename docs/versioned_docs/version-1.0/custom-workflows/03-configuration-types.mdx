...
| `chat_template_kwargs` | `dict`   | `None`            | Provider-specific chat template options passed through unchanged |
...
`chat_template_kwargs` is not prompt-formatted, so values such as `{{provider_flag}}` remain literal provider options. Use only keys supported by the selected model provider.
