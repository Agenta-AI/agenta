...
### Authentication flags reset after plan cancellation

**Scenario**: Organization cancelled their subscription plan

**What happens**:
- Authentication flags (`allow_email`, `allow_social`, `allow_sso`, `allow_root`) are reset to defaults
- Organization reverts to default authentication settings
- Users may lose access if they were using non-default auth methods

**Solution**:
- Renew subscription to restore custom authentication policies
- Or contact organization owner to adjust access
...
- [Enterprise: Organizations](/docs/security/organizations) - Organization management and policies
- [Enterprise: Domain Verification](/docs/security/domain-verification) - Verify email domains for auto-join
