...
- [Okta Integration Guide](/docs/enterprise/guides/okta-integration)
- [Azure AD Integration Guide](/docs/enterprise/guides/azure-ad-integration)
- [Google Workspace SSO](/docs/enterprise/guides/google-workspace-sso)
...
Automatically add users with verified domain emails:
...
**Flow**:
1. New user with `alice@acme.com` authenticates with SSO
2. System sees `acme.com` is a verified domain
3. `auto_join=true` → User automatically added to organization
4. User has immediate access (no invitation needed)

**Requirements**:
- Domain must be verified
- `auto_join=true` must be enabled
- User must authenticate with allowed method (SSO)

## Provider Rotation & Maintenance

### Rotating Credentials

Best practice: Rotate SSO credentials regularly (quarterly or annually)

**Steps**:
1. Generate new credentials in your IdP
2. Update provider in Agenta:
   - Navigate to **Settings** → **Access & Security** → **Providers**
   - Click **Edit** on provider
   - Update Client ID and/or Client Secret
   - Click **Save**
3. Click **Test Connection** to verify
4. Old credentials still work until next user authentication
5. After all users have authenticated with new credentials, revoke old credentials in IdP

:::tip[Zero-Downtime Rotation]
Update credentials in Agenta first, then revoke old credentials in IdP after confirming users can authenticate.
:::

### Replacing a Provider

Migrating from one IdP to another (e.g., Okta → Azure AD):

**Steps**:
1. **Add new provider** (keep old provider active)
   ```json
   {
     "allow_email": false,
     "allow_social": false,
   ...
   ```
   - Providers: `okta` (active), `azure-ad` (active)

2. **Communicate to users**: "Please sign in with Azure AD starting [date]"

3. **Monitor adoption**: Track which provider users are using

4. **Disable old provider** once all users have migrated:
   - Set `okta` to `is_active=false`
   - Users can no longer use Okta

5. **Delete old provider** after grace period:
   - Remove `okta` provider entirely

### Provider Downtime Handling

If your SSO provider has an outage:

**Option 1: Owner Access (if `allow_root=true`)**
1. Organization owner signs in with email/social
2. Owner temporarily enables email authentication
3. Communicate to users: "Use email temporarily"
4. Re-enable SSO when provider is restored

**Option 2: Fallback Provider**
1. Configure backup SSO provider
2. Both providers active
3. Users can choose either provider
4. Redundancy for outages
...
- [Domain Verification](/docs/enterprise/domain-verification) - Verify email domains for auto-discovery
- [Organizations](/docs/enterprise/organizations) - Organization policies and management
...
- [Okta Integration](/docs/enterprise/guides/okta-integration) - Okta-specific setup
- [Azure AD Integration](/docs/enterprise/guides/azure-ad-integration) - Azure AD-specific setup
