...
- Copy the path where `agenta` is located, and export it by running `export agenta=/replace/path/to/agenta`.
- Go to the directory where you tried running `agenta` and it failed, then run `source ~/.bash_profile`. Run `export` to confirm `agenta` has been exported.
