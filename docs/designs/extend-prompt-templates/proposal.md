...
`fallback_llm_configs` item rules:
...
- build candidates from `prompt.llm_config` and `prompt.fallback_llm_configs`
- run one candidate with `prompt.retry_policy`
...
Keep `PromptTemplate.to_openai_kwargs()` primary-only for compatibility. Add an internal helper for candidate-specific kwargs instead of changing that method's return contract.
...
The minimum requirement is that `fallback_llm_configs.items.properties.model` keeps `x-ag-type-ref: "model"` so the frontend can reuse the grouped model selector.
...
- make sure `fallback_llm_configs` can be added, removed, and reordered in the prompt editor/drill-in UI
...
- make sure `retry_policy` and `fallback_policy` appear as prompt root fields, not under `llm_config`
...
- Omitted/null `fallback_llm_configs` means runtime `[]`.
- Omitted/null `retry_policy` means runtime built-in retry defaults.
