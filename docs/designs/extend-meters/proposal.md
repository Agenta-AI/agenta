...
6. **Soft-limit semantics.** `TRACES_RETRIEVED` is `strict=False`. Overshoot is allowed; with `limit=None` everywhere today the check is a no-op.
...
Pattern: after the permission check, call `check_entitlements(key=Counter.EVALUATIONS_RUN, delta=N)`. On `not allowed`, raise `HTTPException(429)`. Then wrap `service.create_*` in `try/except Exception:` — on internal failure, refund with `delta=-N` and re-raise. Domain exceptions raised by the service are *not* refunded — only internal errors.
