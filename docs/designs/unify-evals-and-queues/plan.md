...
5. Add env-backed application configuration for unconditional default queues, e.g. `EVALUATIONS_DEFAULT_QUEUES_FOR_ALL_RUNS`, through the shared `env` object.
...
9. Keep `is_queue` tied to queue-ingest execution semantics; do not use it as the marker that an evaluation has a linked default queue.
