...
A fresh rerun against the current working tree on 2026-05-18 surfaced two additional documentation findings driven by the newly added entitlement behavior: one operator-doc gap for `Flag.AUDIT`, and one cross-doc drift where adjacent access-control docs still describe `events_ingested` as retention-only / unmetered even though this branch now enforces and meters it.
...
### F-013 — [OPEN] `Counter.EVENTS_INGESTED` quota wired with L1/L2; `Flag.AUDIT` added at query side
...
### F-008 — [OPEN] Applications/evaluators commit emission is coupled to `WorkflowsService` being silent
...
### F-009 — [OPEN] Service-layer commit events drop `organization_id`; read events keep it
...
### F-010 — [OPEN] Delta-commit re-entry path is not covered by the exactly-once commit tests
...
### F-011 — [OPEN] `events.md` per-event reference does not document the delta-commit emission rule
...
### F-012 — [OPEN] Webhook event-type list in user docs and TS client can drift from `WebhookEventType`
...
### F-016 — [OPEN] Webhook user docs say commit events carry `count`, but commit payloads intentionally omit it
...
### F-017 — [OPEN] Design docs still preserve pre-AuthScope / pre-metering assumptions after the implementation changed
...
### F-018 — [OPEN] New `Flag.AUDIT` query gate has no direct test coverage
