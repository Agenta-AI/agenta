...
### [DECISION] F42 — subscription test mode (capture-and-skip + `/test` utility)

- **Origin** feature · **Severity** — · **Status** implemented
- **Files** `core/triggers/dtos.py`, `core/triggers/service.py`, `tasks/asyncio/triggers/dispatcher.py`, `apis/fastapi/triggers/{router,models}.py`; web `gatewayTrigger/core/types.ts`, `gatewayTrigger/api/api.ts`, `gatewayTrigger/hooks/useTriggerSubscriptions.ts`, `gatewayTrigger/drawers/TriggerSubscriptionDrawer.tsx`
- **Mechanism** A test subscription is a real persisted subscription with `flags.is_test=true`: it relaxes the bound-workflow requirement and is always `is_active=true`. On a matching inbound event the dispatcher records a `200` delivery whose `data.inputs` is the resolved event context (default `inputs_fields="$"` = whole event) with `data.is_test=true`, and **never invokes** the workflow (capture-and-skip; dedup still applies). The marker lives on both the subscription flags (validation/dispatch) and the delivery data (self-describing).
- **Server invariant** `is_test=true` bypasses `_normalize_references` at create/edit; a non-test create/edit always runs the resolution and rejects (`TriggerReferenceInvalid`) when the binding doesn't resolve — so a production sub can only exist with a valid reference. (Kept as a server-side guardrail even though the UI never sends `is_test` on Save.)
- **`/test` endpoint** `POST /subscriptions/test` (RPC, slashless) wraps create → server long-poll (60s, module constant `_TEST_TIMEOUT_SECONDS`) → delete, tearing down on success, timeout, and error.
- **UX decision (revised)** No test-mode switch in the drawer. A **Test** button (own footer slot) calls `/test` — spinner while in flight, then prints the captured event in a "Captured event" field; the drawer stays open and re-clicking captures again. **Save/Create/Edit** are independent and always require a resolvable binding; they never set `is_test`. Test and Save therefore operate on **separate subscription rows**. Test subscriptions are **hidden from the list** (filtered in `useTriggerSubscriptions` by `flags.is_test`); `/test` tears them down so any visible one is a transient leak.
- **Tests** unit: dispatcher capture-and-skip + dedup; service validation bypass + test-off re-validation + `/test` teardown on success/timeout/error; web zod round-trip for the new flags.
