...
| `stream_id` | streams | `session_streams` | **1:1** (unique `session_id`) | own uuid7 pk; the ephemeral run/liveness mirror of the Redis nest. The business key is `session_id`. |
...
|---|---|---|---|---|
| `turn_id` | **alive + running** lock value | the **current turn** — one execution of the agent loop for this session. Proves alive-lock ownership. | the API in `_start_turn` (uuid7); the runner inherits it. | one per turn; a session sees a *sequence* of `turn_id`s (send/steer each start a new one). **Single name everywhere now** — DTOs, interactions, the wire (`turnId`), and the lock helpers all say `turn_id` (the old `run_id`/`runId` is gone). |
