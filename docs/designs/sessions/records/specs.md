...
Today there is no durable record (audit FUN-1). The PoC proved the shape: the runner
...
## Core decision: records live in the tracing DB, not the core DB

A record is **append-only, high-volume, and retention-governed** — exactly like spans
...
- **No session prefix at the DB level.** The table is just `records` (mirroring how
...
## Service / endpoint surface: `/sessions/records/`, filtered by `session_id`

Under the `sessions` namespace (fixed sub-path), **not** `/sessions/{id}/record` (no id
mid-path). Same shape as `/sessions/states/`, `/sessions/runners/`, `/sessions/mounts/`,
`/sessions/interactions/`. Reads:

- `POST /sessions/records/query` (or `GET ...?session_id=`) — the event log for a session
- one event by id if ever needed: `/sessions/records/{id}` (`id` = the row's uuid7)

So: **DB = `records` (flat, tracing DB); API = `/sessions/records/`, `session_id`
filter.**
...
1. As the runner produces each ACP event it enqueues it to the record ingest — a
   **dedicated queue + worker** (its own Redis-Stream + record worker, same *shape* as
...
On resume the SDK prepends the prior record to the live prompt as a synthetic text
block. If persisted, the *next* resume would replay a record that already contains a
replay → nested, doubling context every turn. The runner must **strip the injected replay
block before storing** so the record only ever holds the real user prompt (PoC
...
Flat table `records`, keyed like the PoC's `session_records` but un-prefixed (the
...
- **Order records by `id`** (tiebreak `created_at, id` if ever needed). No stored
...
tiebreak by random bits — acceptable for a record; they're effectively concurrent). The
...
Read API mirrors the PoC `get_record`, ordered by `id`.
...
- `POST /sessions/records/query` (or `GET ...?session_id=`) — ordered event list (inspect +
  replay source).

## Retention (mirror events/spans exactly)

Records are append-only and retention-governed, so they get the **same retention
machinery events and spans already have** — not an ad-hoc TTL:

1. **Entitlements**: add a records retention quota in the same place as traces. The repo
   ...
   `Counter.RECORDS_*` (and/or reuse the TRACES retention window — Open question 3) so a
   plan defines how long records live.
2. **Retention service**: a records retention flush, mirroring `TracingRetentionService`
   — delete record rows older than the plan's retention, across all plans, wrapped in a
   **Redis lock** (as the spans flush is).
3. **Admin flush endpoint**: `POST /admin/records/flush`, mirroring
...
4. **Cron**: `api/ee/src/crons/records.sh` + `.txt`, mirroring `crons/spans.sh` —
   curls the flush endpoint on a schedule with a long `--max-time` (retention is slow).

## What we drop / change from the PoC

- The ad-hoc `/events` + `session_records` in a single demo Postgres → tracing DB, flat
  `records` table.
- `/invoke`-coupled persistence (PoC already moved off this) → strictly producer-driven.
- No "delete session cascades record" default; deletion is governed by retention, with
...
- **~~C2 cross-DB ownership~~ — WITHDRAWN.** RBAC already covers tracing (`VIEW_SPANS` /
  `VIEW_EVENTS` are tracing permissions); a record read enforces the same way spans/events
  do (project scope + tracing view permission). No special cross-DB story.
- **B3 — cleanup by design.** Records stay on **tracing retention** (entitlement + cron),
...
- **D2 — ingest = a DEDICATED queue + worker** (own record Redis-Stream + worker, NOT
  reusing the spans/events pipeline), not a synchronous write.

## Decided

- **Ingest = DEDICATED queue + worker.** A **dedicated record Redis-Stream + record
...
- **Dedicated retention window.** Records get their **own** retention window — separate from
  spans, separate from events. Three independent windows / `Counter.*.retention` entries / flush
  paths / crons.
- **Payload truncation = per-line max size**, mirroring how spans truncate (cap each record
  event's payload at ingest; no blob-by-reference for v1).

## Open questions (for discussion)

None for v1. (Privacy / explicit "forget this session's record" erase is **deferred** — it
