...
| **Agent mounts (durable per-agent workspace)** | #5215 slice 1 (`POST /mounts/agents/query` — design PR, no code yet) | The config panel's **Mounts section is already stubbed** (`AgentTemplateControl`, Operations group) and our `queryMountFiles`/`readMountFile` are mount-id-generic. Wire-up = add `queryAgentMount(artifactId)` to `session/api` (or a mounts module) → render the mount browser with the returned mount id; empty → "no files yet". Note: #5215's D4/research reference the superseded #5204 FE shape (`MountFilesPanel` in `MountsTab.tsx`, OSS-local fetchers) — our package-level browser is the reuse target, and the user-facing home is the config-panel section, not the (env-gate-bound) inspector. |
