...
- Resolution (applied locally, ruff clean): added two e2e flow tests in `test_evaluation_flows_run.py` — `test_testset_to_mock_evaluator_runs_to_success` (topology `{testset, batch}`, no app; a batch run, so it finalizes to a terminal RUN status — asserts run `success`) and `test_direct_testcases_queue_runs_to_success` (topology `{testcase, queue}` via `POST /simple/queues/` kind=testcases + `/testcases/` batch push → `run_from_batch`). Added `_flow_helpers` helpers: `query_testcase_ids`, `create_testcases_queue`, `add_testcases_to_queue`, `wait_for_scenarios_terminal`. Note on queue mode: a queue run stays OPEN (each batch finalizes its own SCENARIOS, the run never flips terminal), so the queue test asserts completion at the scenario level — `wait_for_scenarios_terminal` polls until each pushed testcase's scenario reaches `success`. This distinction was confirmed empirically: an initial run-level assertion failed with the run pinned at `running`.
