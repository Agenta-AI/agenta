...
| --- | --- | --- |
| 1.6, 4.1 | `uv run pytest oss/tests/pytest/unit/agent_templates -q` (`api/`) | 135 passed; `test_marketplace.py` covers valid tree, missing author (entry + author id in the message), missing and unmapped package folders, unsupported extension schema, modified/added/removed files in a published version against a real git base ref, unknown base ref, API/website parity for every key, author membership, unlisted templates, determinism and staleness |
...
| 4.4 (behavior only) | `pnpm exec vitest run src/components/pages/agent-home/hooks/ src/state/url/template.test.ts` (`web/oss`) | 24 passed; the website consumer now calls `createAgent({name, template})`, repeated consumption creates once, an unknown key shows an unavailable message and creates nothing |
| 1.6, 4.4 | `pnpm test` and `pnpm build` (`web/website`) | 63 passed; build passed; a corrupted author id in the data stops `prebuild` with the regenerate command. The landing CTA reads "Use it for free" and links `https://cloud.agenta.ai/?template=<key>`; every featured key exists in the generated data |
