...
- The window counts from the customer's latest message or button tap by Meta's timestamp (the arrival time when the payload has none), among the 20 latest arrivals in the conversation. A webhook Meta retried for a day therefore neither reopens nor closes the window, unless more than 20 messages arrived after it.
