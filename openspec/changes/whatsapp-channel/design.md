...
- STOP and START apply in the order the customer sent them: every one moves an order fence on the space (the id of the last one seen), even when it changes nothing, and an older, redelivered or delayed one changes nothing. An operator's edit of the space keeps the customer's choice.
