...
Fix: `Railway.create` now retries provisioning while nothing is live for the PR (`RAILWAY_CREATE_ATTEMPTS`, `RAILWAY_CREATE_RETRY_SECONDS`). It is duplicate-safe because the create script's own name-poll already adopts any in-flight background create before returning failure, and the retry is skipped when a live environment indicates a mid-deploy failure rather than a contested name. Three offline tests cover the retry, the no-retry-over-live-env case, and the attempt budget.

Repeat acceptance against the fixed head `db94c5dc` passed: [run 35761621916](https://github.com/Agenta-AI/agenta/actions/runs/35761621916), `result: pass`. Recorded checks: real human comment read and current write permission accepted; real Railway clone with pinned images and readiness smoke; one-hour expiry stored and read back (ready 17:37:21Z, expiry 18:37:21Z UTC); duplicate delivery did not renew the deadline or create a second environment; a stale-generation finalizer left the live preview untouched; the expiry sweep deleted the real environment and verified absence; automatic-mode setup then finish deleted the second real environment and verified absence. `environment_absent: true` at teardown.
