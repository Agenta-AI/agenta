...
These tasks describe future implementation. All remain unchecked. Mahmoud must approve the design and request implementation before work starts. The channels stack has not shipped to production; do not add old-format support, legacy modes, or migration of pre-release installations.
...
- [ ] 2.1 Add address and operation schema with connection, project, and team ownership constraints using repository schema tooling; verify a fresh database and uniqueness tests against two projects without a pre-release-data backfill.
...
- [ ] 3.1 Request the final Slack scopes at first installation and verify granted scope state; test matching hosted and custom setup and refusal to activate a token missing required permissions.
- [ ] 3.2 Bind setup to the authorized project and Slack identity; test a successful fresh installation and rejection of a callback for another workspace or app.
...
- [ ] 4.3 Implement target-specific approval ownership and unaddressed conversation/default selection without a parallel `~slug` path; test an open approval for triage with an explicit research mention and confirm no approval is consumed for triage.
...
- [ ] 5.1 Extend the sender-profile contract directly, update callers, and resolve it from the thread agent; run adapter contract tests to verify Slack identity and unaffected behavior of other adapters.
...
- [ ] 5.3 Expose missing customization permission as a blocked delivery; test no success claim, silent identity substitution, or identity-changing retry after an unknown post outcome.
...
- [ ] 6.3 Add installation selection, roster, handle, and lifecycle states to shared UI; verify desktop and mobile show the same failed, active, retry, and authorization-blocked states.
...
- [ ] 7.3 Exercise fresh setup, partial provisioning recovery, external group drift, and verified cleanup of test groups; prove removal of one deployment leaves the other operational without testing a pre-release upgrade path.
