...
- Built as: paging uses an offset over that total order. A message stored between two pages can shift a later page by one row. Search uses the same `_not_a_copy_of_a_bot_post` filter as read, so it leaves out consumed answers and fetched copies of the bot's posts. After staging QA of v0.121.2, `search_statement` (was `search_inbox_statement`) also matches the bot's final outbox posts in a `UNION ALL`, and `search_space_messages` returns both kinds.
...
- Built as: the Slack coverage reads "Searched messages since the bot joined this channel." and the Telegram coverage reads "Searched only the messages the bot received or sent in this group." Search makes no Slack history or search call. It may refresh the Slack member channel list to know which channels are readable.
