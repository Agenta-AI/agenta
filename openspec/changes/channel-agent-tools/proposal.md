...
Status: approved on 2026-09-24 and implemented on branch `feat/channel-agent-tools`, which targets `release/v0.121.2`. The implementation plan is in [plan.md](plan.md).
...
- Add the channel tools to every run of a connected agent automatically. When the running agent matches an active, verified bot, the agent runtime adds the tools to that run, without changing the saved configuration. This works in every run type: playground, API, channel turns, and automations. The bot settings decide which tools come: turning posting off removes the send tool, and turning reading off removes read and search. An author's own entry for one of the tools wins.
...
- `channel-agent-tool-access`: How the tools reach every run of a connected agent, the send default, the server-bound caller identity, and the checks every call repeats.
...
- **SDK**: four new operations in the platform tool catalog (`sdks/python/agenta/sdk/agents/platform/op_catalog.py`), grouped as `CHANNEL_TOOL_OPS`, and an optional `PlatformOp.default_permission` that carries the send tool's `allow` default. The agent handler adds the channel tools to each run of a connected agent (`_with_channel_tools` in `sdks/python/agenta/sdk/agents/handler.py`).
...
- **API**: new authenticated tool routes under `/api/channels/tools/`, including `POST /api/channels/tools/availability`, which tells the agent runtime which channel tools a run gets, a tool service under `api/oss/src/core/channels/tools/`, three migrations (`oss000000036` to `oss000000038`: a nullable-thread outbox with a `space_id` column, a `sent_at` column on the inbox table, and a full-text expression index), and two Slack adapter methods, for member channels and for older history pages.
- **Web app**: the chat and the tool permission controls know the four tools as platform tools (`web/packages/agenta-chat/src/skin/registry.ts`, `PLATFORM_OPS` in `web/packages/agenta-entity-ui/src/DrillInView/SchemaControls/toolPermission.ts`).
