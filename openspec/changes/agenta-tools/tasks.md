...
- [ ] 1.5 Unit tests: defaults, each setting, missing block, unknown name, author entry wins, build kit entry wins, no duplicate when the build kit sends the same tool, a build kit tool that is deactivated there falls back to the Agenta tools setting, no session ID, no API address, saved configuration unchanged.

## 2. API

- [ ] 2.1 Leave `DEFAULT_BUILD_KIT_OPS` in `api/oss/src/core/workflows/build_kit.py` unchanged. Add a test that pins its list, so the build kit keeps every tool.
...
- [ ] 3.4 Write the Agenta tools copy, and the Build kit copy only if Mahmoud confirms it may change (design, Open Points).
...
- [ ] 4.3 Turn `rename_session` off in Agenta tools: Slack, API and schedule runs stop naming their sessions, and a playground run still follows the build kit.
