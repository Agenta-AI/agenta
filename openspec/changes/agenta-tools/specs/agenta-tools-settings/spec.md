...
The agent's `tools` list SHALL accept an entry of `type` `agenta_tools` with a `tools` map from tool name to value. The allowed values SHALL be `allow` and `ask`. A tool that is not in the map SHALL be off. An empty map SHALL mean every Agenta tool is off. The entry SHALL NOT accept a top-level `permission`. The entry SHALL be saved with the version like any other tool entry. An unknown tool name SHALL be ignored with a warning.
...
#### Scenario: Other values are refused
- **WHEN** the entry sets a tool to `deny` or `off`
- **THEN** the configuration SHALL be refused as invalid, and the error SHALL name the allowed values.

#### Scenario: Unlisted differs from an author's deny
- **WHEN** `rename_agent` is not listed in the entry
...
- **THEN** the saved entry SHALL be `{"type": "agenta_tools", "tools": {}}`, no run outside the playground SHALL offer an Agenta tool, and opening the agent again SHALL NOT add the defaults back.

#### Scenario: Deactivating one tool
- **WHEN** an author sets "Rename this chat" to Deactivate
- **THEN** the draft entry SHALL no longer list `rename_session`, and the row SHALL still be shown, set to Deactivate.
