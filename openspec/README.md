...
The channels stack has not shipped to production. The baseline is a reference for the reviewed code, not a compatibility contract. The proposal does not require legacy modes, preservation of pre-release data, or a production migration path.
...
3. Read the [design](changes/slack-native-agent-handles/design.md) for implementation choices, first-release setup, risks, and the 9-15 engineer-day estimate.
...
`specs/` records the observed baseline. `changes/slack-native-agent-handles/` contains the proposal, design, tasks, and delta specifications. Modified and removed requirement names match the baseline exactly. The new address capability uses `ADDED Requirements`. The routing delta removes the pre-release addressing and precedence requirements and adds native replacements. Other changed behavior uses `MODIFIED Requirements`.
