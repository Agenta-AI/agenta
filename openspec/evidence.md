...
- [Rate limits](https://docs.slack.dev/apis/web-api/rate-limits): Slack answers HTTP 429 with `Retry-After`. Since 2025, commercially distributed apps outside the Slack Marketplace get a much lower limit on `conversations.history` and `conversations.replies`. This must be confirmed for the hosted app before release.
