...
class Forward:
    ...
    async def move_existing_members_to_default_workspace(self, session):
        async for organization in OrganizationDB.find_all():
            default_workspace = await WorkspaceDB.find_one(
                WorkspaceDB.organization.id == organization.id,
                WorkspaceDB.type == "default",
                fetch_links=True,
            )
    ...
    async def update_organization_invitations(self, session):
        async for organization in OrganizationDB.find_all():
            for invitation in organization.invitations:
                invitation.organization_id = str(organization.id)
