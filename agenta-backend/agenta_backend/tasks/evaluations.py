...
def evaluate(app_data, new_evaluation_data):
    ...
    new_evaluation_db = loop.run_until_complete(create_new_evaluation(
        app=app,
    ...
    for variant_id in new_evaluation.variant_ids:
        ...
        deployment = loop.run_until_complete(get_deployment_by_objectid(app_variant_db.base.deployment))
        ...
        for data_point in testset.csvdata:
            ...
            for evaluator_config in new_evaluation.evaluators_configs:
                result = evaluators_service.evaluate(evaluator_config.evaluator_key, data_point['correct_answer'], variant_output)
            ...
            evaluation_scenario = loop.run_until_complete(create_new_evaluation_scenario(
                user=app.user,
                organization=app.organization,
                ...
                outputs=[EvaluationScenarioOutputDB(
                    type="text",
                    value=variant_output
                )],
                results=results
            ))
