...
| **Agent mounts (durable per-agent workspace)** | #5215 slice 1 (`POST /mounts/agents/query` — design PR, no code yet) | The config panel's **"App drive" section is already stubbed** (`AgentOperationsSections`, Storage region — user-facing naming is drive/Storage, not "mount") and our `queryMountFiles`/`readMountFile` are mount-id-generic. Wire-up = add `queryAgentMount(artifactId)` to `session/api` (or a mounts module) → render the mount browser with the returned mount id; empty → "no files yet". Note: #5215's D4/research reference the superseded #5204 FE shape (`MountFilesPanel` in `MountsTab.tsx`, OSS-local fetchers) — our package-level browser is the reuse target, and the user-facing home is the config-panel section, not the (env-gate-bound) inspector. |
...
| Read-write mounts | The browser is read-only; the backend also has write/upload/delete/create-folder + archive/unarchive. Add file management once the agent-mounts surface (#5215, direction now landed as a design PR) is wired | — |
