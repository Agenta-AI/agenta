...
- [x] Records live in the **tracing DB**, flat table `records` (no session prefix),
      API = `/sessions/records/` (sessions namespace) filtered by `session_id` (NOT
      `/sessions/{id}/…`).
- [x] **Ingest = DEDICATED queue + worker** (own record Redis-Stream + record worker,
      NOT reusing the spans/events pipeline).
- [x] **Ordering: uuid7 `id`, no stored `seq`.** Decouples from runner affinity.
- [x] **Dedicated retention window** (records ≠ spans ≠ events — each its own).
- [x] **Payload truncation: per-line max size** at ingest, mirroring spans (no blob-by-ref v1).

## 1. Schema (tracing DB)

- [ ] `records` table: `id` (uuid7 pk, the ordering key), `session_id` (indexed),
...
- [ ] Wire into a **dedicated** record retention flush (own window; see §5).

## 2. DB + service

- [ ] DAO: `append_event(session_id, …)` (plain insert, uuid7 default id) and
      `get_record(session_id)` ordered by `id`. Optional read-time `row_number()` ordinal.
- [ ] Service = `/sessions/records/` (query by `session_id`); read enforces project scope +
...
- [ ] Runner → ingest **enqueues** the event to a **dedicated record Redis-Stream**; a
      **dedicated record worker** consumes and writes the DB. Not a synchronous endpoint
      write; not the spans/events stream.
- [ ] Truncate each event's `payload` to a per-line max at ingest (mirror spans).
- [ ] `POST /sessions/records/query` (or `GET ...?session_id=`) read (inspect + replay).
...
- [ ] Entitlements: add a **dedicated** records retention quota in `api/ee/src/core/access/
      entitlements/types.py` (`Counter.RECORDS_*` with its own `Retention` window — NOT
      reusing TRACES; records/spans/events each have their own).
- [ ] Retention service: records flush mirroring `TracingRetentionService`, Redis-locked.
- [ ] Admin endpoint: `POST /admin/records/flush` mirroring `/admin/spans/flush`.
- [ ] Cron: `api/ee/src/crons/records.sh` + `.txt` mirroring `crons/spans.sh`.
- [ ] Privacy erase path (Open question 4) — explicit per-session record delete.
...
- [ ] Acceptance: run → fetch record → resume → fetch again, assert no nested replay and
...
- **sessions-persistence** consumes `get_record` as the replay source (the persist
...
- Inspect UI for records (frontend follow-up).
