...
describe("DaytonaWorkspaceReader", () => {
  ...
  // The Daytona transport rejects a NUL byte inside a command argument and fails the whole
  // request with an opaque `Stream Error`. On the fail-closed approval path that denied every
  // workspace-file commit on Daytona, live, while the local reader stayed green. The framing
  // still needs NUL separators, so the format asks `find` for them with its own `\0` escape
  // (two characters) and the ARGUMENT stays NUL-free.
  it("asks find for NUL separators without putting a NUL in the argument", async () => {
    const { exec, calls } = fakeExec({});
    const reader = new DaytonaWorkspaceReader(cwd, exec);
    await reader.listImportRoot().catch(() => {});
    await reader.readImportFile("notes.md").catch(() => {});

    assert.ok(calls.length > 0, "the reader ran at least one command");
    for (const argv of calls) {
      for (const arg of argv) {
        assert.ok(
          !arg.includes("\0"),
          `argument carries a NUL byte: ${JSON.stringify(arg)}`,
        );
      }
    }

    const formats = calls
      .filter((argv) => argv[0] === "find" && argv.includes("-printf"))
      .map((argv) => argv[argv.indexOf("-printf") + 1]);
    assert.ok(formats.length > 0, "the manifest walk ran");
    for (const format of formats) {
      if (format === "x") continue; // the depth probe prints a single sentinel character
      assert.equal(format, "%y\\0%m\\0%s\\0%P\\0");
    }
  });
});
