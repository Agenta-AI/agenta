...
@ag.instrument()
async def generate(context:str, messages: List[Message]):
    results = search_docs(context)
    message = await llm(context, results)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("agenta.sdk.decorators.routing:app", host="0.0.0.0", port=804, reload=True)