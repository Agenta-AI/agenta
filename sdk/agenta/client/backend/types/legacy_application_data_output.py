# This file was auto-generated by Fern from our API Definition.

from __future__ import annotations

import typing

import pydantic
from ..core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel, update_forward_refs
from .legacy_application_data_output_headers_value import LegacyApplicationDataOutputHeadersValue


class LegacyApplicationDataOutput(UniversalBaseModel):
    version: typing.Optional[str] = None
    uri: typing.Optional[str] = None
    url: typing.Optional[str] = None
    headers: typing.Optional[typing.Dict[str, typing.Optional[LegacyApplicationDataOutputHeadersValue]]] = None
    schemas: typing.Optional[typing.Dict[str, typing.Optional[typing.Dict[str, typing.Optional["FullJsonOutput"]]]]] = (
        None
    )
    script: typing.Optional[typing.Dict[str, typing.Optional["FullJsonOutput"]]] = None
    parameters: typing.Optional[typing.Dict[str, typing.Optional["FullJsonOutput"]]] = None
    runtime: typing.Optional[str] = None
    service: typing.Optional[typing.Dict[str, typing.Any]] = None
    configuration: typing.Optional[typing.Dict[str, typing.Any]] = None

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow


from .full_json_output import FullJsonOutput  # noqa: E402, I001

update_forward_refs(LegacyApplicationDataOutput, FullJsonOutput=FullJsonOutput)
