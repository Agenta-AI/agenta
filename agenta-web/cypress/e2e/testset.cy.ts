...
describe("create a new testset", () => {
    ...
    it.only("successfully tests test-set exact match evaluation", () => {
        ...
        countriesAndCapitals.forEach((row, index) => {
            cy.get('[col-id="country"]').eq(index + 1).as('countryColumn') // get the first column after the header
            cy.get('@countryColumn').click()
            cy.get('@countryColumn').find('input').type(row.country)

            cy.get('[col-id="correct_answer"]').eq(index + 1).as('answerColumn') // get the first row after the header
            cy.get('@answerColumn').click()
            cy.get('@answerColumn').find('input').type(`The capital of ${row.country} is ${row.capital}.`)
        })
