  ...
  return (
    ...
    >
      {variants.map((cell, index) => {
        ...
        if (!variantsVotesData || variantsVotesData.number_of_votes === 0)
          return null;
        return (
          <div
            key={`variant-${index}`}
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          color: "#fff",
          width: `${
            (scoresData.scores.correct / scoresData.nb_of_rows) * 100
          }%`,
...
const Results: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchData(
      ...
      .then((responseData) => {
        const fetchPromises: Promise<DataType>[] = responseData.map(
  ...
  const columns: ColumnsType<DataType> = [
    {
      ...
      render: (value: any, record: DataType, index: number) => {
        ...
        if (data[index].evaluationType == EvaluationType.human_a_b_testing) {
          ...
          data[index].evaluationType == EvaluationType.auto_exact_match
        ) {
