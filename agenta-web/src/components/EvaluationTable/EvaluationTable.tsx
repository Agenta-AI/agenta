...
const EvaluationTable: React.FC<EvaluationTableProps> = ({
  ...
  const variantData = variants.map((variant: Variant) => {
    const { optParams, URIPath, isLoading, isError, error } = useVariant(
      app_name,
      variant,
    );
  ...
  const setRowValue = (
    rowIndex: number,
    columnKey: keyof EvaluationTableRow,
    value: any,
  ) => {
    const newRows = [...rows];
      ...
      render: (text: any, record: any, rowIndex: number) => (
        <Spin spinning={rows[rowIndex].vote === "loading" ? true : false}>
          <Space>
            <Button
              type={
                record.vote === variants[0].variantName ? "primary" : "default"
              }
              disabled={
                record.evaluationFlow ===
                  EvaluationFlow.COMPARISON_RUN_STARTED || record.vote !== ""
                  ? false
                  : true
              }
            ...
            <Button
              type={
                record.vote === variants[1].variantName ? "primary" : "default"
              }
              disabled={
                record.evaluationFlow ===
                  EvaluationFlow.COMPARISON_RUN_STARTED || record.vote !== ""
