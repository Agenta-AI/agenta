...
const BoxComponent: React.FC<TestViewProps> = ({
  ...
  const [inputParamsDict, setInputParamsDict] = useState<
    Record<string, string>
  >(
    inputParams.reduce(
      (dict, param) => ({ ...dict, [param.name]: param.default }),
      {},
    ),
  );
  const handleInputParamValChange = (
...
const App: React.FC<TestViewProps> = ({ inputParams, optParams, URIPath }) => {
  ...
  return (
    <div>
      <h2 style={{ padding: "0px", marginBottom: "8px" }}>
        2. Preview and test
      </h2>
