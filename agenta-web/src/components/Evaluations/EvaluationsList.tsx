...
export default function EvaluationsList() {
  ...
  const [appEvaluationsList, setAppEvaluationsList] = useState<
    EvaluationListTableDataType[]
  >([]);
  ...
  const onCompleteEvaluation = (appEvaluation: any) => {
    ...
    const evaluationType =
      EvaluationType[
        appEvaluation.evaluationType as keyof typeof EvaluationType
      ];

    if (evaluationType === EvaluationType.auto_exact_match) {
      router.push(
        `/apps/${app_name}/evaluations/${appEvaluation.key}/auto_exact_match`,
      );
    } else if (evaluationType === EvaluationType.human_a_b_testing) {
      router.push(
        `/apps/${app_name}/evaluations/${appEvaluation.key}/human_a_b_testing`,
      );
    }
  };
  ...
  const columns: ColumnsType<EvaluationListTableDataType> = [
    {
      ...
      render: (
        value: any,
        record: EvaluationListTableDataType,
        index: number,
      ) => {
    ...
    },
    {
      ...
      render: (value: string) => {
        const evaluationType =
          EvaluationType[value as keyof typeof EvaluationType];
  ...
  const rowSelection = {
    onChange: (
      ...
      selectedRows: EvaluationListTableDataType[],
    ) => {
