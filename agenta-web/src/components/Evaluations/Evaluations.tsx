...
export default function Evaluations() {
  ...
  const { datasets, isDatasetsLoading, isDatasetsLoadingError } =
    loadDatasetsList(appName);
  ...
  useEffect(() => {
    if (variants.length > 0) {
      const fetchAndSetSchema = async () => {
        try {
          const { inputParams } = await getVariantParameters(
            appName,
            variants[0],
          );
          setVariantInputs(
            inputParams.map((inputParam: Parameter) => inputParam.name),
          );
        } catch (e) {
  ...
  const createNewAppEvaluation = async (
    ...
    return postData(
      `${process.env.NEXT_PUBLIC_AGENTA_API_URL}/api/app_evaluations/`,
      data,
    )
      .then((data) => {
        return data.id;
  ...
  const handleAppVariantsMenuClick =
    ...
    ({ key }: { key: string }) => {
      ...
      const selectedVariant = variants.find(
        (variant) => variant.variantName === key,
      );
  ...
  const onStartEvaluation = async () => {
    ...
    if (selectedEvaluationType === EvaluationType.auto_exact_match) {
      router.push(
        `/apps/${appName}/evaluations/${evaluationTableId}/auto_exact_match`,
      );
    } else if (selectedEvaluationType === EvaluationType.human_a_b_testing) {
      router.push(
        `/apps/${appName}/evaluations/${evaluationTableId}/human_a_b_testing`,
      );
    }
  };
