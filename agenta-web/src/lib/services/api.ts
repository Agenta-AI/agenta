...
export function callVariant(
  ...
  const inputParams = Object.keys(inputParametersDict).reduce(
    (acc: any, key) => {
  ...
  return axios
    .post(
      `${process.env.NEXT_PUBLIC_AGENTA_API_URL}/${URIPath}/generate`,
      requestBody,
      {
        headers: {
    ...
    .catch((error) => {
      if (error.response && error.response.status === 500) {
        throw new Error(
          error.response.data.error + " " + error.response.data.traceback,
        );
      }
...
export const loadDataset = async (datasetId: string) => {
  return fetch(
    `${process.env.NEXT_PUBLIC_AGENTA_API_URL}/api/datasets/${datasetId}`,
    {
      headers: {
        "Content-Type": "application/json",
      },
...
export const loadAppEvaluations = async (app_name: string) => {
  try {
    return await eval_endpoint
      ...
      .then((responseData) => {
        const appEvaluations = responseData.data.map(
...
export const postEvaluationRow = async (evaluationTableId: string, data) => {
  const response = await eval_endpoint.post(
    `${evaluationTableId}/evaluation_row`,
    data,
  );
  return response.data;
};
