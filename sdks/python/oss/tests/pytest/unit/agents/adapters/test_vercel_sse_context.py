...
async def test_redactor_installed_on_first_pull_stays_active(monkeypatch):
    # `_stream_in_redaction_scope` installs the run's redactor on the first pull. Every later
    # chunk, and the upstream's teardown, must still see it.
    from agenta.sdk.agents.handler import _stream_in_redaction_scope
    ...
    async def parts() -> AsyncIterator[Dict[str, Any]]:
        try:
            for index in range(3):
                seen.append(get_active_redactor())
                yield {"type": "text-delta", "delta": str(index)}
        finally:
            seen.append(get_active_redactor())
    ...
    try:
        await _collect(
            mod.vercel_sse_stream(_stream_in_redaction_scope(redactor, parts()))
        )
    finally:
        monkeypatch.delenv("AGENTA_AGENT_SSE_KEEPALIVE_SECONDS", raising=False)
        importlib.reload(sse_module)
    ...
    assert len(seen) == 4
    assert all(observed is redactor for observed in seen)
