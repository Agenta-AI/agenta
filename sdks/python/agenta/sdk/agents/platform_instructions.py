...
def compose_platform_instructions(integration_names: Sequence[str]) -> str:
    """Compose deterministic SDK-owned text, with optional guidance after the common base."""
    guidance = gateway_guidance(integration_names)
    if not guidance:
        return AGENTA_PLATFORM_BASE
    return f"{AGENTA_PLATFORM_BASE}\n\n{guidance}"
