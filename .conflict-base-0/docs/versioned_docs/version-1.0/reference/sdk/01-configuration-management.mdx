...
### `get_from_registry` {#get_from_registry}
...
| prompt.llm_config.chat_template_kwargs | object or null | Provider-specific chat template options passed through unchanged. Omitted when `null` |
...
:::note
You can use the `PromptTemplate` class in the SDK to create and validate a prompt configuration. See [how to commit a variant](/1.0/prompt-engineering/managing-prompts-programatically/create-and-commit#create-a-new-variant).
:::
