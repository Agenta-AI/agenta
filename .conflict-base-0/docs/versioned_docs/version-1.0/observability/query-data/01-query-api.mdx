...
Two endpoints, same query body — choose the shape you want back:

```
POST /api/spans/query     # returns flat spans
POST /api/traces/query    # returns trace trees
```
...
**Spans** (`POST /api/spans/query`): Returns individual spans in a flat list. Use this when you need span-level details.
...
**Traces** (`POST /api/traces/query`): Returns complete traces as hierarchical trees. Each trace groups its spans by `trace_id`. Use this when you need to see the full request flow.
...
Here's a full query that finds successful LLM calls from the last month. The same body works for both `/spans/query` and `/traces/query`:
...
**`count`**: Number of results returned. **`spans`**: Array of spans. **`traces`**: Array of trace trees.
...
Learn about the [Analytics Data API](/1.0/observability/query-data/analytics-data) for aggregated metrics and time-series data.

Explore the [API Reference](/reference/api/category) for complete endpoint documentation.

Check out [Filtering in the UI](/1.0/observability/concepts) to learn about the visual query builder.
