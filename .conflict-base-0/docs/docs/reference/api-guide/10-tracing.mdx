...
The Agenta SDK and OTel auto-instrumentation handle ingestion for you in the common cases. See [Observability](/1.0/observability/overview) for the SDK setup. Use the endpoints below only when you have a custom integration that bypasses the SDK.
...
For programmatic ingestion of nested traces, point any OTel exporter at `POST /otlp/v1/traces`. The Agenta SDK and the OTel auto-instrumentation set this up for you; see [Observability](/1.0/observability/overview) for SDK setup, or the [OpenTelemetry exporter documentation](https://opentelemetry.io/docs/specs/otlp/#otlphttp) if you're configuring a raw OTel collector.
