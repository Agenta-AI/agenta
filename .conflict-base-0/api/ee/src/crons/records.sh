...
echo "--------------------------------------------------------"
echo "[$(date)] records.sh running from cron"
...
RESPONSE=$(curl \
    ...
    "http://api:8000/admin/records/flush" 2>&1) || CURL_EXIT=$?
...
if [ -n "${CURL_EXIT:-}" ]; then
    ...
    if [ "${HTTP_CODE}" = "200" ]; then
        echo "✅ Records retention completed successfully"
    else
        echo "❌ Records retention failed with HTTP ${HTTP_CODE}"
    fi
fi

echo "[$(date)] records.sh done"
