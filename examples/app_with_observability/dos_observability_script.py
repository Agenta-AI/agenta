...
async def create_span(
    trace_id: str, input: Dict[str, Any], output: str, tokens: Dict[str, Any]
):
...
async def start_tracing(requests_to_make: int):
    total_requests = 3
    start_start = time.perf_counter()
    while True:
        if total_requests == requests_to_make:
        ...
        try:
            ...
            time_elapsed = end_time - start_start
            print(
                f" ================= Iteration completed ================= \n- Requests made: {total_requests} \n- Time elapsed: {time_elapsed:.2f} seconds"
            )
        except httpx.RequestError as ex:
            print(f"Backend Error: {ex}")
            requests_to_make = max(1, requests_to_make // 2)  # Reduce load
            await asyncio.sleep(2)


if __name__ == "__main__":
    requests_to_make = 1000
    asyncio.run(start_tracing(requests_to_make))
