...
def generate(event_name: str,
             ...
             wild_tone_prompt: ag.TextParam = wild_tone_prompt) -> str:
    ...
    if tone == "Professional and serious":
        ...
        prompt_template_final = prompt_template.replace("{tone}", professional_tone_prompt)
    elif tone == "Adventurous and enthusiastic":
        prompt_template_final = prompt_template.replace("{tone}", adventurous_tone_prompt)
    ...
    prompt = PromptTemplate(
        input_variables=["vendor_name", "lang", "event_name", "vendor_input", "tone", "answer_length"],
        template=prompt_template_final,
    )
    chain = LLMChain(llm=llm, prompt=prompt)
    output = chain.run(vendor_name=vendor_name, lang=lang, event_name=event_name,
                       vendor_input=vendor_input, tone=tone, answer_length=answer_length)
    return output
