...
class SummarizerConfig(BaseModel):
    summarizer_context_prompt: str = Field(
        default="Act as a professional cinema critic.\nBe concise and factual.\nUse one intro sentence, and one sentence per movie."
    )
    summarizer_instructions_prompt: str = Field(
        default="Summarize the following recommendations about {topic} in the genre of {genre}:\n\n{report}"
    )
...
class GeneratorConfig(BaseModel):
    generator_context_prompt: str = Field(
        default="Given the following list of suggested movies:\n\n{movies}"
    )
    generator_instructions_prompt: str = Field(
        default="Provide a list of {count} movies about {topic} in the genre of {genre}."
    )
...
class MflixConfig(BaseModel):
    retriever_prompt: str = Field(
        default="Movies about {topic} in the genre of {genre}."
    )
