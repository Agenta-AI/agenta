...
| 5 | [architecture.md](architecture.md) | **RFC implemented (steps 1–8)** | Core layer: domain, PMS integration (Protocol + fake), retrieval, clock, AgentDeps. Backend landed in `core/`, 43 service-level tests passing. Runtime adapters (steps 9–10) deferred to library-matrix.md |
| 6 | [library-matrix.md](library-matrix.md) | TODO | What each runtime must implement, side-by-side comparison |
| 7 | [agenta-integration.md](agenta-integration.md) | TODO | Prompt mgmt, eval (SDK + UI), observability, online eval, annotation queue |
...
| 11 | [implementation-status.md](implementation-status.md) | **Done** | Snapshot of what landed for steps 1–8; handoff report |
| 12 | [runtime-pydanticai.md](runtime-pydanticai.md) | **Plan, ready to implement** | First runtime adapter — Pydantic-AI vanilla. Tool surface, streaming model, server route, frontend, test layers |
