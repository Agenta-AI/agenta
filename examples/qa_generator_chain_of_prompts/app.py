...
prompt_1 = (
    "Determine the three main topics that a user would ask about based on this documentation page {context_1}"
)
prompt_2 = (
    "Create 10 Question and Answers based on the following topics {topics} and the documentation page {context_1} "
)
...
ag.config.default(
    ...
    model_1=ag.MultipleChoiceParam(
    ...
    model_2=ag.MultipleChoiceParam(
        "gpt-3.5-turbo", CHAT_LLM_GPT
    ),
...
async def generate(context_1: str) -> str:
    ...
    prompt2 = ag.config.prompt_user_2.format(topics=response.choices[0].message.content, context_1=context_1)
    response = client.chat.completions.create(
        model=ag.config.model_2,
        messages=[
            {"content": prompt2, "role": "user"}],
