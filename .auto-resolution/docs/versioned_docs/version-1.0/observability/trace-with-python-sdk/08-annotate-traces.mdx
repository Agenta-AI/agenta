...
Annotations accept any slug in `references.evaluator.slug`. The first time Agenta sees a new slug, it creates the evaluator for you and infers its output schema from the annotation data. The evaluator then shows up in the UI with its aggregated stats. If you want an explicit schema (fixed labels, required fields, or value ranges), create the evaluator first through `POST /api/simple/evaluators/` or from the UI before you send annotations.
...
:::info
Annotations accept any evaluator slug. Agenta auto-creates the evaluator on the first annotation and infers its output schema from your data. To control the schema (fixed labels, required fields, or value ranges), create the evaluator first via `POST /api/simple/evaluators/` or from the Agenta UI.
:::
