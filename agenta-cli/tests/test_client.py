...
def agenta_client():
    ...
    with patch.dict('os.environ', {'AGENTA_API_KEY': 'mock_api_key', 'AGENTA_HOST': 'https://mock.agenta.ai'}):
        client = Agenta()
    return client
...
def test_get_config_with_caching(agenta_client):
    ...
    with patch.object(agenta_client.client.configs, 'get_config', return_value={"parameters": "something"}) as mock_get_config:
        # Retrieve configuration to store in cache
        response = agenta_client.get_config("base123", "production")
        assert response == {"parameters": "something"}, "First response should match the mock data."
        ...
        response = agenta_client.get_config("base123", "production")
        assert response == {"parameters": "something"}, "Second response should return cached data, not new mock data."
...
def test_get_config_without_caching(agenta_client):
    ...
    with patch.object(agenta_client.client.configs, 'get_config', return_value={"parameters": "something"}) as mock_get_config:
        ...
        response = agenta_client.get_config("base123", "production", cache_timeout=0)
        assert response == {"parameters": "something else"}, "Second response should match the new mock data."
