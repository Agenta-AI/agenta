...
from agenta.sdk.tracing.spans import get_attributes
...
def _parse_to_legacy_span(span: ReadableSpan):
    ...
    legacy_span = CreateSpan(
        ...
        #
        config=get_attributes(span, "metadata").get("config", {}),
        #
        tokens=LlmTokens(
            prompt_tokens=get_attributes(span, "metrics.marginal.tokens").get(
                "prompt", 0
            ),
            completion_tokens=get_attributes(span, "metrics.marginal.tokens").get(
                "completion", 0
            ),
            total_tokens=get_attributes(span, "metrics.marginal.tokens").get(
                "total", 0
            ),
        ),
        cost=get_attributes(span, "metrics.marginal.costs").get("total", 0.0),
        #
        app_id=get_attributes(span, "extra").get("app_id", ""),
        environment=get_attributes(span, "metadata").get("environment"),
        #
