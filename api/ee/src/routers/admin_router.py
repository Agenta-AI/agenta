...
from ee.src.services.commoners import add_user_to_demos
from oss.src.services.api_key_service import (
    create_api_key,
)
...
async def create_accounts(
    ...
    try:
        ...
        for reference, request in entities.project_memberships.items():
            ...
            if request.project_ref.slug not in scopes.projects[request.user_ref.slug]:
                scopes.projects[request.user_ref.slug][request.project_ref.slug] = {}
            # TRACK PROJECT SCOPE
...
async def create_account(
    ...
    try:
        ...
        organization_db = await legacy_create_organization(
            create_org_payload,
            user_db,
        )
