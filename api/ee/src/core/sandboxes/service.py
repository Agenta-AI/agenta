...
class SandboxMeteringService:
    ...
    async def record_usage(self, usage: SandboxUsageDTO) -> SandboxUsageResult:
        ...
        meter_deltas: list[tuple[Counter, int]] = [
            (Counter.SANDBOX_VCPU_CORE_SECONDS, usage.vcpu_seconds),
            (Counter.SANDBOX_VMEM_GIBI_SECONDS, usage.ram_gib_seconds),
            (Counter.SANDBOX_DISK_GIBI_SECONDS, usage.disk_gib_seconds),
            (Counter.SANDBOX_VGPU_CORE_SECONDS, usage.gpu_seconds),
        ]
