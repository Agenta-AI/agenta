...
def annotate(
    ...
    *,
    ...
    try:
        ...
        testset = loop.run_until_complete(
            fetch_testset_by_id(testset_id),
        )

        assert testset is not None, f"Testset with id {testset_id} not found!"

        testset_response = loop.run_until_complete(
            testsets_router.transfer_testset(
                request=request,
                testset_id=testset_id,
            )
        )
        ...
        testcases = testset_response.testset.data.testcases
        ...
        nof_autoevals = len(autoeval_configs)
        # ----------------------------------------------------------------------
        ...
        steps = [input_step, invocation_step]
        ...
        mappings = input_mappings + invocation_mappings
        ...
        raw_metrics = {
            ...
            **{
                ...
                for slug in autoeval_slugs
            },
        }

        autoeval_metrics_keys = {slug: {} for slug in autoeval_slugs}
        # ----------------------------------------------------------------------
        ...
        steps_create_request = EvaluationStepsCreateRequest(
            steps=[
                EvaluationStepCreate(
                    ...
                    retry_id=retry_id,
                    #
                    testcase_id=testcases[idx]["testcase_id"],
                )
        ...
        # ----------------------------------------------------------------------
        log.info(
            "Starting run batches:",
            project_id=project_id,
            run_id=run_id,
            testset_id=testset_id,
            count=len(testcases),
            size=len(dumps(testcases).encode("utf-8")),
        )
        # ----------------------------------------------------------------------
        ...
        invocations: List[InvokationResult] = loop.run_until_complete(
            llm_apps_service.batch_invoke(
                project_id=project_id,
                user_id=user_id,
                testset_data=testcases,  # type: ignore
        ...
        for idx in range(nof_testcases):
            ...
            if not steps_updated:
                # fix evaluator references -------------------------------------
                for jdx in range(nof_autoevals):
                    autoeval_slug = autoeval_slugs[jdx]

                    simple_evaluator_query_request = SimpleEvaluatorQueryRequest(
                        evaluator_refs=[{"slug": autoeval_slug}],
                    )

                    simple_evaluator_response = loop.run_until_complete(
                        evaluators_router.query_simple_evaluators(
                            request=request,
                    ...
                    if simple_evaluator_response.count != 0:
                        ...
                        evaluator_references[autoeval_slug] = {
                            "id": str(evaluator.id),
                            "slug": evaluator.slug,
                        }

                        autoeval_metrics_keys[
                            autoeval_slug
                        ] = get_metrics_keys_from_schema(
                            schema=(
                                evaluator.data.service.get("format")
                                if "agenta" in evaluator.data.service
                                else {}
                            ),
                        )
            ...
            if not steps_updated and steps_ready:
                # edit steps/mappings in run -----------------------------------
                annotation_steps = [
                    {
                        "key": "annotation",
                        "references": {
                            "auto_evaluator": {"id": str(autoeval_config.id)},
                            "evaluator": evaluator_references[autoeval_slug],
                        },
                        "inputs": [
                            {"key": "input"},
                            {"key": "invocation"},
                ...
                annotation_mappings = [
                    {
                        "column": {
                            "kind": "annotation",
                            "name": (metrics_key["path"]),
                            "type": metrics_key["type"],
                        },
                        "step": {
                            "key": autoeval_slug,
                            "path": metrics_key["path"],
                        },
                    }
                    for autoeval_slug in autoeval_slugs
                    for metrics_key in autoeval_metrics_keys[autoeval_slug]
                ]
            ...
            if invocation.result.error:
                ...
                raw_metrics["invocation"]["errors"].append(error is not None)
                raw_metrics["invocation"]["costs"].append(invocation.cost or 0.0)
                raw_metrics["invocation"]["duration"].append(invocation.latency or 0.0)
            # ------------------------------------------------------------------
            ...
            # proceed with the evaluation otherwise ----------------------------
                ...
                for jdx in range(nof_autoevals):
                    ...
                    references = {
                        "auto_evaluator": {"id": autoeval_config.id},
                        "evaluator": evaluator_references[autoeval_slug],
                        "testset": {"id": testset_id},
                    }
                    links = {
                    ...
                    if result.error:
                        log.error(
                            f"There is an error in evaluation {autoeval_config.id} for invocation {invocation.trace_id} so we skip its evaluation"
                        )
                        ...
                        error = result.error.model_dump(mode="json") is not None

                        raw_metrics[autoeval_slug]["errors"].append(error is not None)
                    # ----------------------------------------------------------
                    ...
                    # else, first annotation, then step ------------------------
                        ...
                        for key in metrics:
                            if key not in raw_metrics[autoeval_slug]:
                                raw_metrics[autoeval_slug][key] = []

                            raw_metrics[autoeval_slug][key].append(metrics[key])
    ...
    if run_status != EvaluationStatus.FAILURE:
        try:
            ...
            metrics = process_metrics(
                ...
                autoeval_slugs=autoeval_slugs,
                autoeval_metrics_keys=autoeval_metrics_keys,
            )

            metrics_data = {"outputs": metrics}
            ...
            # log.debug("< metrics", metrics=metrics)
...
def get_metrics_keys_from_schema(schema=None, path=()):
    metrics = []
    ...
    if not path:
        for metric in metrics:
            if metric["path"].startswith("outputs."):
                metric["path"] = metric["path"][len("outputs.") :]
...
def compute_frequency_unique_top(values, top_k=5):
    ...
    _frequency = dict(StatisticsCounter(values))
...
def process_metrics(raw_metrics, autoeval_slugs, autoeval_metrics_keys):
    metrics = dict()

    # invocation metrics -------------------------------------------------------
    duration = raw_metrics["invocation"]["duration"]
    duration_statistics = _process_continuous_metric(metric=duration)

    errors = raw_metrics["invocation"]["errors"]
    errors_statistics = _process_binary_metric(metric=errors)

    costs = raw_metrics["invocation"]["costs"]
    costs_statistics = _process_continuous_metric(metric=costs)

    tokens = raw_metrics["invocation"]["tokens"]
    ...
    # annotation metrics -------------------------------------------------------
    for slug in autoeval_slugs:
        errors = raw_metrics[slug]["errors"]
        errors_statistics = _process_binary_metric(metric=errors)

        if slug not in autoeval_metrics_keys:
            log.warn(f"slug {slug} not found in metrics keys. Skipping.")
            continue

        metrics_keys = autoeval_metrics_keys[slug]

        if not metrics_keys:
            log.warn(f"No metrics keys found for slug {slug}. Skipping.")
            continue
        ...
        for metric_key in metrics_keys:
            ...
            metric = get_metric_from_path(metrics=raw_metrics[slug], path=metric_path)
