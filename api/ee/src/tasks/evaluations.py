    ...
    try:
        ...
        for idx in range(nof_testcases):
            ...
            if not steps_updated:
                ...
                for jdx in range(nof_annotations):
                    ...
                    simple_evaluator_query_request = SimpleEvaluatorQueryRequest(
                        evaluator_refs=[{"slug": annotation_step_key}],
                    )
                    ...
                    if simple_evaluator_response.count != 0:
                        ...
                        annotation_metrics_keys[annotation_step_key] = [
                            {
                                "path": metric_key["path"].replace("outputs.", "", 1),
                                "type": metric_key["type"],
                            }
                ...
                annotation_steps = [
                    {
                        "key": step_key,
                        "references": {
                            "evaluator": evaluator_references[step_key],
                        },
                        "inputs": [
                            {"key": testset_step_key},
                            {"key": invocation_step_key},
            ...
            if invocation.result.error:
                ...
                raw_metrics[invocation_step_key]["errors"].append(error is not None)
                raw_metrics[invocation_step_key]["costs"].append(invocation.cost or 0.0)
                raw_metrics[invocation_step_key]["duration"].append(
            ...
            # proceed with the evaluation otherwise ----------------------------
                ...
                for jdx in range(nof_annotations):
                    ...
                    if result.error:
                        ...
                        raw_metrics[annotation_step_key]["errors"].append(
                            error is not None
                        )
                    # ----------------------------------------------------------
                    ...
                    # else, first annotation, then step ------------------------
                        ...
                        for key in metrics:
                            if key not in raw_metrics[annotation_step_key]:
                                raw_metrics[annotation_step_key][key] = []

                            raw_metrics[annotation_step_key][key].append(metrics[key])
