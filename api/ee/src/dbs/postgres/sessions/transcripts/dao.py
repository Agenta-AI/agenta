"""EE transcripts DAO (retention).

Walks ``projects ⋈ subscriptions`` to find projects on a given plan, then
deletes transcript rows older than the retention cutoff. Lives in EE because
the project-to-plan join goes through the EE ``SubscriptionDBE`` table.

The OSS counterpart (``oss.src.dbs.postgres.sessions.transcripts.dao.TranscriptsDAO``)
...
class TranscriptsRetentionDAO:
    """EE transcripts DAO (retention only)."""
    ...
    async def delete_transcripts_before_cutoff(
        self,
        *,
        cutoff: datetime,
        ...
        """Delete up to ``max_transcripts`` rows with ``created_at < cutoff``
        ...
        async with self.analytics_engine.session() as session:
            ...
            expired = (
                select(
                    TranscriptDBE.project_id.label("project_id"),
                    TranscriptDBE.id.label("id"),
                )
                .where(
                    TranscriptDBE.project_id == any_(project_ids_param),
                    TranscriptDBE.created_at < bindparam("cutoff", value=cutoff),
                )
                .order_by(TranscriptDBE.created_at)
                .limit(bindparam("max_transcripts", value=max_transcripts))
                .cte("expired_transcripts")
            )

            deleted = (
                delete(TranscriptDBE)
                .where(
                    tuple_(TranscriptDBE.project_id, TranscriptDBE.id).in_(
