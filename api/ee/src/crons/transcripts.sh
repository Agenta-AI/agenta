...
echo "--------------------------------------------------------"
echo "[$(date)] transcripts.sh running from cron"
...
RESPONSE=$(curl \
    ...
    "http://api:8000/admin/transcripts/flush" 2>&1) || CURL_EXIT=$?
...
if [ -n "${CURL_EXIT:-}" ]; then
    ...
    if [ "${HTTP_CODE}" = "200" ]; then
        echo "✅ Transcripts retention completed successfully"
    else
        echo "❌ Transcripts retention failed with HTTP ${HTTP_CODE}"
    fi
fi

echo "[$(date)] transcripts.sh done"
