...
async def get_organizations_by_list_ids(organization_ids: List) -> List[OrganizationDB]:
    ...
    async with engine.core_connection() as connection:
        ...
        stmt = select(OrganizationDB).where(
            OrganizationDB.id.in_(organization_uuids),
        )
...
async def get_organization_workspaces(organization_id: str):
    ...
    async with engine.core_connection() as connection:
        stmt = (
            ...
            .options(
                load_only(WorkspaceDB.organization_id),
            )  # type: ignore
        )
...
async def get_default_workspace_id_from_organization(
    ...
    async with engine.core_connection() as connection:
        stmt = (
            ...
            .options(
                load_only(WorkspaceDB.id),
            )
        )

        workspace_query = await connection.execute(stmt=stmt, prepare=True)
...
async def check_user_in_workspace_with_email(email: str, workspace_id: str) -> bool:
    ...
    async with engine.core_connection() as connection:
        stmt = (
            ...
            .join(
                UserDB,
                UserDB.id == WorkspaceMemberDB.user_id,
            )
            .where(
                UserDB.email == email,
...
async def get_project_invitations(project_id: str, **kwargs):
    ...
    async with engine.core_connection() as connection:
        ...
        if kwargs.get("has_pending", False):
            stmt = stmt.filter(
                InvitationDB.used == kwargs["invitation_used"],
            )
...
async def get_project_members(project_id: str):
    ...
    async with engine.core_connection() as connection:
        stmt = (
            ...
            .options(
                joinedload(ProjectMemberDB.user),
            )
        )

        members_query = await connection.execute(stmt=stmt, prepare=True)
...
async def fetch_evaluation_by_id(
    ...
    async with engine.core_connection() as connection:
        stmt = select(EvaluationDB).filter_by(
            project_id=project_id,
            id=uuid.UUID(evaluation_id),
        )
        ...
        stmt = stmt.options(
            ...
            joinedload(
                EvaluationDB.aggregated_results.of_type(EvaluationAggregatedResultDB)
...
async def list_human_evaluations(app_id: str, project_id: str):
    ...
    async with engine.core_connection() as connection:
        stmt = (
            ...
            .filter(
                HumanEvaluationDB.testset_id.isnot(None),
            )
        )
        stmt = stmt.options(
...
async def fetch_human_evaluation_variants(human_evaluation_id: str):
    ...
    async with engine.core_connection() as connection:
        stmt = select(HumanEvaluationVariantDB).filter_by(
...
async def fetch_human_evaluation_by_id(
    ...
    assert evaluation_id is not None, "evaluation_id cannot be None"
    async with engine.core_connection() as connection:
        stmt = select(HumanEvaluationDB).filter_by(
            id=uuid.UUID(evaluation_id),
        )
...
async def fetch_evaluation_scenario_by_id(
    ...
    async with engine.core_connection() as connection:
        stmt = select(EvaluationScenarioDB).filter_by(
            id=uuid.UUID(evaluation_scenario_id),
        )
...
async def fetch_human_evaluation_scenario_by_id(
    ...
    assert evaluation_scenario_id is not None, "evaluation_scenario_id cannot be None"
    async with engine.core_connection() as connection:
        stmt = select(HumanEvaluationScenarioDB).filter_by(
            id=uuid.UUID(evaluation_scenario_id),
        )
    ...
    async with engine.core_connection() as connection:
        if resource_type == "variant":
            stmt = (
                ...
                .options(
                    load_only(EvaluationDB.id),
                )  # type: ignore
            )

            result_evaluations = await connection.execute(stmt=stmt, prepare=True)
            ...
            stmt = (
                ...
                .options(
                    load_only(HumanEvaluationDB.id),
                )  # type: ignore
            )

            result_human_evaluations = await connection.execute(stmt=stmt, prepare=True)
...
async def fetch_eval_aggregated_results(evaluation_id: str):
    ...
    async with engine.core_connection() as connection:
        stmt = select(EvaluationAggregatedResultDB).filter_by(
            evaluation_id=uuid.UUID(evaluation_id)
        )
