...
def update_contact_in_loops(email, properties, max_retries=5, initial_delay=1):
    """
    Update a contact in Loops with user properties, syncing from PostHog.

    Args:
        email (str): Email address of the contact to update.
        properties (dict): Dictionary of properties to update for the contact.
        max_retries (int): Maximum number of retries in case of rate limiting.
        initial_delay (int): Initial delay in seconds before retrying.
    ...
    # Request payload - email is required, plus any additional properties
    data = {"email": email, **properties}
