...
membership/invitation user FKs get explicit ON DELETE behavior. EE billing
FKs are repaired: subscriptions gains its missing FK to organizations and
meters is retargeted from subscriptions to organizations, both CASCADE. Per
...
def upgrade() -> None:
    ...
    # -- BILLING FKS (subscriptions/meters anchor to organizations) --------------
    _drop_fks_on_column("meters", "organization_id")
    op.execute(
        """
        DELETE FROM meters
        WHERE organization_id IS NOT NULL
          AND NOT EXISTS (
                SELECT 1 FROM organizations o WHERE o.id = meters.organization_id
          )
        """
    )
    op.create_foreign_key(
        "meters_organization_id_fkey",
        "meters",
        "organizations",
        ["organization_id"],
        ["id"],
    ...
    op.execute(
        ...
        WHERE NOT EXISTS (
            SELECT 1 FROM organizations o WHERE o.id = subscriptions.organization_id
        )
        """
    )
    _drop_fks_on_column("subscriptions", "organization_id")
    op.create_foreign_key(
