"""Acceptance tests for GET /evaluators/catalog/templates/* endpoints."""
...
class TestListEvaluatorCatalogTemplates:
    def test_returns_200(self, authed_api):
        response = authed_api("GET", "/evaluators/catalog/templates/")
    ...
    def test_templates_is_list(self, authed_api):
        body = authed_api("GET", "/evaluators/catalog/templates/").json()
    ...
    def test_include_archived_returns_more_or_equal(self, authed_api):
        default_count = authed_api("GET", "/evaluators/catalog/templates/").json()[
            "count"
        ]
        with_archived_count = authed_api(
            "GET", "/evaluators/catalog/templates/?include_archived=true"
        ).json()["count"]
...
class TestFetchEvaluatorCatalogTemplate:
    def test_returns_200_for_known_key(self, authed_api):
        response = authed_api(
            "GET", f"/evaluators/catalog/templates/{KNOWN_TEMPLATE_KEY}"
        )
        assert response.status_code == 200
    ...
    def test_count_is_0_for_missing_key(self, authed_api):
        body = authed_api("GET", f"/evaluators/catalog/templates/{MISSING_KEY}").json()
        assert body["count"] == 0
...
class TestListEvaluatorCatalogPresets:
    def test_returns_200_for_known_template(self, authed_api):
        response = authed_api(
            "GET",
            f"/evaluators/catalog/templates/{KNOWN_TEMPLATE_KEY}/presets/",
        )
        assert response.status_code == 200
    ...
    def test_empty_presets_for_unknown_template(self, authed_api):
        body = authed_api(
            "GET", f"/evaluators/catalog/templates/{MISSING_KEY}/presets/"
        ).json()
        assert body["count"] == 0
        assert body["presets"] == []
...
class TestFetchEvaluatorCatalogPreset:
    def test_returns_200_for_known_preset(self, authed_api):
        response = authed_api(
            "GET",
            f"/evaluators/catalog/templates/{KNOWN_TEMPLATE_KEY}/presets/{KNOWN_PRESET_KEY}",
        )
        assert response.status_code == 200
    ...
    def test_count_is_0_for_missing_preset(self, authed_api):
        body = authed_api(
            "GET",
            f"/evaluators/catalog/templates/{KNOWN_TEMPLATE_KEY}/presets/{MISSING_KEY}",
        ).json()
        assert body["count"] == 0
    ...
    def test_count_is_0_for_missing_template(self, authed_api):
        body = authed_api(
            "GET",
            f"/evaluators/catalog/templates/{MISSING_KEY}/presets/{KNOWN_PRESET_KEY}",
        ).json()
        assert body["count"] == 0
