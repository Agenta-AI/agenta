"""One switch, one meaning, on both sides of the ordered-operations rollout.
...
import the API's parser, so it mirrors the truthy set. If the two sets ever drift, a
deployment written `enabled` turns the arm on in the API while the catalog keeps
advertising the legacy surface, and the model sends a shape it was never shown.
...
def test_both_sides_read_the_same_spelling_the_same_way(monkeypatch, spelling):
    ...
    assert _ordered_operations_enabled() == _parse_bool_env(FLAG, False), (
...
def test_an_unset_variable_leaves_the_arm_off_on_both_sides(monkeypatch):
    monkeypatch.delenv(FLAG, raising=False)

    assert _ordered_operations_enabled() is False
    assert _parse_bool_env(FLAG, False) is False
