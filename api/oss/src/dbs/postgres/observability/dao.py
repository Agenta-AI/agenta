        ...
        *,
        ...
        try:
            async with engine.tracing_connection() as connection:
                ...
                count = (await connection.execute(count_query)).scalar()
                # --------
                ...
                # QUERY EXECUTION
                spans = (await connection.execute(query)).all()
                # ---------------
        ...
        *,
        ...
        try:
            async with engine.tracing_connection() as connection:
                ...
                error_bucket_rows = (
                    (await connection.execute(error_query)).mappings().all()
                )
                # ---------------
                ...
                bucket_dtos, count = map_bucket_dbes_to_dtos(
                    total_bucket_dbes=total_bucket_rows,
                    error_bucket_dbes=error_bucket_rows,
                    window=window,
    ...
    async def read_one(
        ...
        *,
        ...
        async with engine.tracing_connection() as connection:
            ...
            span_dbe = result.scalars().one_or_none()
