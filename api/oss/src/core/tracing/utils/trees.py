...
def infer_and_propagate_trace_type_by_trace(
    ...
    A trace is an annotation iff any span in that trace explicitly sets links.
    An empty links list still means "this is an annotation with no link target";
    missing links (None) means invocation.
    """
    if not span_dtos:
        return span_dtos
