...
class HttpHeader(BaseModel):
    """HTTP header configuration for webhook requests"""

    key: str = Field(..., min_length=1, max_length=100, description="Header name")
    value: str = Field(..., max_length=1000, description="Header value")
    is_secret: bool = Field(default=False, description="Whether this is a sensitive value (e.g., Authorization)")
...
class WebhookCreate(BaseModel):
    ...
    webhook_type: Literal["http_webhook", "python_script"] = Field(
        ...
        description="Type of webhook: http_webhook for HTTP callbacks, python_script for Docker script execution"
    )

    # HTTP Webhook fields
    webhook_url: Optional[str] = Field(None, max_length=2048, description="HTTP webhook URL")
    webhook_method: str = Field(default="POST", pattern="^(GET|POST|PUT|PATCH|DELETE)$", description="HTTP method")
    webhook_headers: List[HttpHeader] = Field(
        default_factory=list, description="Custom HTTP headers for the webhook request"
    )
    webhook_body_template: Optional[str] = Field(
        None, description="JSON template for webhook body (supports {{deployment_id}}, {{environment}}, etc.)"
    )

    # Python Script fields
    script_content: Optional[str] = Field(None, min_length=1, description="Python script to execute")
...
class WebhookUpdate(BaseModel):
    ...
    webhook_method: Optional[str] = Field(None, pattern="^(GET|POST|PUT|PATCH|DELETE)$")
    webhook_headers: Optional[List[HttpHeader]] = None
...
class WebhookResponse(BaseModel):
    ...
    webhook_headers: List[HttpHeader]
