...
RESPONSE=$(curl \
    ...
    "http://api:8000/admin/evaluations/runs/refresh?trigger_interval=${TRIGGER_INTERVAL}&trigger_datetime=${TRIGGER_DATETIME}" 2>&1) || CURL_EXIT=$?
...
if [ -n "${CURL_EXIT:-}" ]; then
    ...
    if [ "${HTTP_CODE}" = "200" ]; then
        echo "✅ Runs refresh completed successfully"
    else
        echo "❌ Runs refresh failed with HTTP ${HTTP_CODE}"
    fi
fi
