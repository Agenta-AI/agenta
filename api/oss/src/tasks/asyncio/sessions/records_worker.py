    ...
    async def process_batch(
        ...
        for project_batch in batches:
            ...
            try:
                ...
                # The counter for the late-record guard. `append_many` quarantines a record
                # that arrives for a turn the watchdog has already ended, and logs each one;
                # this is the per-batch total, so how often the guard fires is one grep away
                # rather than a query over the records table.
