...
logger.info("migrations: alembic_version_oss chain")
...
async def _assert_legacy_parked() -> None:
    ...
    if not exists:
        raise RuntimeError(
            "legacy alembic_version table not found; the legacy chain must run first"
        )
    head = await _fetch_value("SELECT version_num FROM alembic_version")
    if head != ALIGN_REVISION:
        raise RuntimeError(
            f"legacy chain is at {head!r}, expected the align revision"
            f" {ALIGN_REVISION!r}; upgrade the legacy chain first"
        )
...
def run_alembic_migration():
    """Run the alembic_version_oss chain to head (legacy chain must be parked at align)."""
    ...
    try:
        ...
        if first_run or env.alembic.auto_migrations:
            ...
            click.echo(
                click.style(
                    "\nalembic_version_oss chain migrations applied.", fg="green"
                ),
                color=True,
            )
        else:
            click.echo(
                click.style(
                    "\nalembic_version_oss chain: auto-migrations disabled, skipping.",
                    fg="yellow",
                ),
                color=True,
            )
        ...
        click.echo(
            click.style(
                f"\nAn ERROR occurred while applying alembic_version_oss chain migrations:"
