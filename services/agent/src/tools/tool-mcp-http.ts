...
/**
 * A parked client tool. The handler returns this sentinel INSTEAD of a JSON-RPC response so the
 * request listener emits NO body and deterministically aborts the in-flight HTTP request: a normal
 * MCP result would let the harness (Claude) settle the call and clobber the pending connect widget
 * before the paused turn is observed. The seam already emitted the `client_tool` interaction and
 * cancelled the prompt (`onPark` -> `destroySession`), so the turn ends `paused`.
 */
...
export interface InternalToolMcpServerOptions {
  /** When set, a `client` tool call is parked through this relay instead of relayed/executed. */
  clientToolRelay?: ClientToolRelay;
  /** Fired by the engine on park/teardown; destroys any in-flight request so none settles late. */
  signal?: AbortSignal;
  log?: Log;
}
 ...
 * notification (no `id`), or the `MCP_PARKED` sentinel for a parked client tool (the listener
...
async function handle(
  ...
  if (method === "tools/list") {
    return {
      ...
      result: {
        ...
        // must SEE them to call them; the runner parks the call in `tools/call` below rather than
        // executing it. Only public metadata (name/description/inputSchema) crosses to the harness
  ...
  if (method === "tools/call") {
    ...
    // `client` tools are browser-fulfilled across a turn boundary: park them through the shared
 ...
 * whatever specs it is given. A `client` spec is parked through `options.clientToolRelay`;
 * `options.signal` (the engine's park/teardown abort) destroys any in-flight request so a parked
 * call never settles late.
 */
export function startInternalToolMcpServer(
  ...
  // parked client tool destroys its OWN response below; the signal is the backstop for any other
  // request still open when the turn ends).
  const active = new Set<ServerResponse>();

  /** Abort a parked request: destroy the socket with no body written, so nothing settles late. */
    ...
    readBody(req)
      .then(async (raw) => {
        ...
        if (response === MCP_PARKED) {
          // Parked client tool: emit NO JSON-RPC result, abort the in-flight request.
          abortParked(res);
          return;
        }
  ...
  // Belt and suspenders: on park/teardown the engine fires this signal; destroy every in-flight
