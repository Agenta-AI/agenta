...
drives the SERVICE's own composition (``oss.src.agent.app``) with the REAL platform resolver,
and stubs only the backend HTTP the resolver reads from. That covers the wiring the seam
tests cannot: the service's composition really carries the resolver, the resolver really
issues its three requests, and the rendered text really reaches the backend.
...
ambiguous request rendered the wrong agent's name through the real service. Every test here
that names an artifact installs a tracing context for exactly that reason.
"""
...
from agenta.sdk.contexts.tracing import TracingContext, tracing_context
...
async def _turn(
    *,
    ...
    tracing_references: Optional[Dict[str, Any]] = None,
    meta: Optional[Dict[str, Any]] = None,
) -> None:
    """One service turn. ``tracing_references`` installs the ambient run context."""
    token = None
    if tracing_references is not None:
        token = tracing_context.set(TracingContext(references=tracing_references))
    try:
        await app._agent(
            request=WorkflowServiceRequest(
                session_id=SESSION_ID, references=references, meta=meta
            ),
...
async def test_the_service_renders_the_facts_it_reads(service, backend_facts):
    ...
    await _turn(
        ...
        tracing_references={"application": {"id": ARTIFACT_A}},
    )
...
async def test_a_rename_shows_on_the_very_next_turn(service, backend_facts):
...
async def test_the_service_ignores_a_forged_session_context(service, backend_facts):
...
async def test_competing_families_render_no_agent_name_through_the_service(
