...
async function executeAllowedRelayedTool(
  ...
  if (spec.call) {
    const callContext: RunContext | undefined = req.toolCallId
      ? { ...runContext, tool: { call_id: req.toolCallId } }
      : runContext;
    const body = assembleBody(spec.call, dispatchArgs, callContext, spec.name);
