/**
 * Session keep-alive identity and continuation policy.
 *
 * Pure derivation only: provider config, canonical fingerprints, credential epochs,
 * conversation matching, and safe project-scoped pool keys. Mutable lifecycle storage
 * belongs in `session-pool.ts`.
 */
