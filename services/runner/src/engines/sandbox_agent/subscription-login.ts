...
export function subscriptionLoginFrom(raw: string | undefined): SubscriptionLogin | undefined {
...
export interface SubscriptionSandboxFs {
  ...
  readFsFile?: (input: { path: string }) => Promise<string>;
  downloadFiles?: (input: { path: string }) => Promise<Buffer | string>;
}
...
async function readSandboxFile(
  ...
  try {
    if (typeof sandbox.readFsFile === "function") {
      return await sandbox.readFsFile({ path });
    }
    if (typeof sandbox.downloadFiles === "function") {
      const content = await sandbox.downloadFiles({ path });
...
export async function pushSubscriptionLogin(
  ...
  try {
    ...
    log(
      `subscription login push failed error=${err instanceof Error ? err.name : "unknown"}`,
    );
  }
}
...
export async function pushBackSubscriptionLogin(input: {
  ...
  try {
    ...
    log(
      `subscription login read-back failed error=${err instanceof Error ? err.name : "unknown"}`,
    );
  }
}
...
export async function reportSubscriptionLoginFailure(
  ...
  try {
    ...
    log(
      `subscription login failure report failed error=${err instanceof Error ? err.name : "unknown"}`,
    );
    return { stale: false };
  }
}
...
export function watchLocalSubscriptionLogin(input: {
  ...
  const read = async (): Promise<void> => {
    ...
    try {
      ...
      log(
        `subscription login watch read failed error=${err instanceof Error ? err.name : "unknown"}`,
      );
    } finally {
      reading = false;
    }
    ...
    () => {
      ...
      void (async () => {
        try {
          ...
          log(
            `subscription login poll failed error=${err instanceof Error ? err.name : "unknown"}`,
          );
        } finally {
          reading = false;
        }
