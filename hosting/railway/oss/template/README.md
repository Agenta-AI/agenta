...
- **Healthchecks:** set only on `api` and `services`, which both serve
  `/health`. The gateway has none on purpose. It proxies `/` to `web`, and the
  web app answers `/` with a 308 redirect to `/w`
  (`web/oss/next.config.ts`). Railway counts a 308 as a failed probe, so a
  healthcheck on `/` never goes green and the gateway deployment of every clone
  ends FAILED. The gateway can carry a healthcheck again once the wrapper image
  serves its own 200 endpoint (for example `location = /healthz`), which needs a
  new `gateway_tag`. Adding a healthcheck to any other service is a deliberate
  change to `template.json`, not silent drift.
